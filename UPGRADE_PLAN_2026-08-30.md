...
| P1 | 统一 D5 覆盖口径（113 vs 119）；章节筛选已修正（147） |
| P1 | 解锁发布依赖：`deploy` 等待 site/pdf/epub 全成功（已改工作树，待推送验收） |
...
4. ⬜ CI 所述变更经远端 Actions 验收（需推送）。

### 阶段 B（短期，09-03 ~ 09-30）：债务清零与指标统一
1. 30 WARN、55+7 引用/拓扑清零并转硬门禁。
2. 依赖锁文件 + 容器 digest + D5 口径统一。
3. legacy 审计候选项分批人工复核（每批提交 + 记录）。
4. 工作树按主题拆分 commits/PR 并推送验收 CI 新依赖图。
