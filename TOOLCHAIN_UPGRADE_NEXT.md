...
> **⚠️ 交叉项根因警示（2026-08-31 立项，见 NEXT_LLM.md 已收口批次）**：
> `Book/` 前缀链接 bug 曾全书 100% 失效、又在一次「全书级流水线重跑」后一字未变地保留下
> 来——**证明该写法活在生成模板/prompt 里，只重跑只会让它原样复现**。凡涉及生成器的
> 后续改造，默认要求：①源头把 `Book/` 前缀从模板/生成口令里去掉；②产物侧挂
> 「Book Link Integrity」卡口（已入 CI quality job，`tools/fix_book_links.py --check`），
> 让任何 reintroduce 即刻 BLOCK。**「改了再跑」≠「改模板」**，两条腿缺一不可。
