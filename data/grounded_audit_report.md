...
- 命题 `ATOM-UB-GRAY-001::prop-1`（observation，可信度 2）判 **IN**：它击败了全部 10 个攻击它的误解（严格可信度优势）
  - 攻击者 `MIS-CONC-001`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
  - 攻击者 `MIS-UB-001`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
  - 攻击者 `MIS-UB-002`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
  - 攻击者 `MIS-UB-003`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
  ...
  - 攻击者 `MIS-UB-012`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
  - 攻击者 `MIS-UB-013`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
  - 攻击者 `MIS-UB-014`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
  - 攻击者 `MIS-UB-015`（误解，可信度 1）判 **OUT**：被 2 条命题击败，如 `ATOM-UB-GRAY-001::prop-1` 等
...
- 命题 `ATOM-MEM-MOVE-002::prop-1`（observation，可信度 2）判 **IN**：它击败了全部 8 个攻击它的误解（严格可信度优势）
  ...
  - 攻击者 `MIS-MEM-002`（误解，可信度 1）判 **OUT**：被 3 条命题击败，如 `ATOM-MEM-MOVE-002::prop-1` 等
  - 攻击者 `MIS-MEM-003`（误解，可信度 1）判 **OUT**：被 3 条命题击败，如 `ATOM-MEM-MOVE-002::prop-1` 等
  - 攻击者 `MIS-MEM-004`（误解，可信度 1）判 **OUT**：被 3 条命题击败，如 `ATOM-MEM-MOVE-002::prop-1` 等
  - 攻击者 `MIS-MEM-005`（误解，可信度 1）判 **OUT**：被 6 条命题击败，如 `ATOM-MEM-MOVE-002::prop-1` 等
  - 攻击者 `MIS-MEM-012`（误解，可信度 1）判 **OUT**：被 3 条命题击败，如 `ATOM-MEM-MOVE-002::prop-1` 等
  - 攻击者 `MIS-MEM-017`（误解，可信度 1）判 **OUT**：被 6 条命题击败，如 `ATOM-MEM-MOVE-002::prop-1` 等
  - 攻击者 `MIS-MEM-019`（误解，可信度 1）判 **OUT**：被 7 条命题击败，如 `ATOM-MEM-MOVE-002::prop-1` 等
