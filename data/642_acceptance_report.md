...
16. **本批未 push**：`origin/master..HEAD` = **58** 个 commit（641 收尾 3 + 642 全部 + 两次自纠），
    push 时机与 CI 观察交人（§六.1）。
17. **`status.json` 的 `last_commit` 约定（口径说明）**：它**无法自引用其自身所在提交的哈希**
    ⇒ 本批记录为**批次末已验证 commit**（`a7bfad34`）；`status.json` 本身那一笔提交的哈希
    以 `git log -- _auto/status.json` 为准。这是既有做法（640c 同形），非本批新造。
