...
5. **是否 push**：~~CI 四 job 现仅覆盖到 `cbd0fbd`(616)，617/618/619 新增工具与单测未被 CI 覆盖。~~
   **【620 D2 更正】** 该判断已过时：实测 `origin/master` = **`d2f412b`**（= 619 收工点），
   即 617/618/619 **已被推送**，其工具与单测**已落入 CI 覆盖范围**（CI 实际运行结果需远程核实）。
   620 开工时本地领先 origin/master 为 **14**（即 620 自身 commit，未 push）。
