...
> **冻结于 2026-07-11 12:20:51 · 仅供历史参考**
>
> 本文件已冻结，不再随正文更新。原声明「由 `tools/gen_crossref.py` 自动生成」
> 已悬空——该生成器只识别旧式 `⟶` 箭头 / 反引号 / `前置·后续` 元数据，而当前
> 正文已大量采用 Markdown 链接 `[第NN章](Book/...)`，重跑生成器会从 732 条依赖边
> 退化到约 103 条（-85%）。
>
> 权威的交叉引用完整性由活跃门禁维护：
> - `tools/crossref_audit.py` — part 级覆盖度与断链
> - `tools/xref_check.py` — 全部可导航引用（Markdown `[..](..)` + ⟶ 箭头式）的目标存在性
>
> 重生成命令已废弃；如需回填 732 条边，请评估选项 B（重写生成器）。

本文件是独立于正文的**导航/速查件**（非 `chXXX.md`，不计入一致性门禁）。

## 0. 速读说明

- **冻结时章节总数**：147
- **冻结时依赖边总数**（出链去重后）：732
- **冻结时有出链的章**：147 ｜ **有入链的章**：146
- **冻结时含策展 `前置/后续` 字段的章**：40
- **断链**：0（应为 0）
