...
matrix:
  ...
  opt: [-O0, -O2]
  arch: [x86-64]
fixture: Examples/atoms/_atom_eval_order.cpp
...
expected:
  run: >-
    输出三个元素，卡内以 `|` 分隔记录（夹具实际 stdout 为三行）：`h | g | f(1,2)`（GCC）或
    `g | h | f(1,2)`（Clang）——**两种都必须判 confirm**，这正是"未指定"的含义；rc=0，
    无 UB 征兆（无崩溃、无 sanitizer 报错）。
  asm: >-
...
falsification: >-
  ...
  **最强证据是"两个编译器给出相反顺序"**：GCC 六个组合全是 `h→g`（右→左），而 CI 的 Clang 是
  ...
  "合法结果集合"。
  **对照（反例说明，不造实验）**：真正属于"未测序标量修改"的例子是 **`i = i++ + ++i`**
  （运算符操作数仍是 *unsequenced*，C++17+ 依然 UB）。**注意不要拿 `f(i++, i++)` 当 UB 例子**：
  C++17（P0145R3）已把函数参数初始化改为 *indeterminately sequenced*（不重叠）→ 它是
  **unspecified**，只有 C++11/14 才是 UB。查标准时**先分清 _unsequenced_ 与
  _indeterminately sequenced_**——本样板曾在人审中被纠正过这一点（执行方用旧版本规则套了新标准）。
depth_layer: asm
drill_note: >-
