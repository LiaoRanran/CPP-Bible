...
- **L2 重启**：工程类 4 项收口后回到主线（L2 已 18 章）。选章改用「纯赝品」精准判据
  ...
  不误伤）——ch37 以 14 块居首，人工复核后确认 **13 块为合法事件追踪、仅 #59 为真赝品**
  （`cout << "ok: 配对 new[]+delete[] 或 vector"`，纯散文零验证）。
- **ch37 #59 真机实证替换**：新增 `Examples/_ch37_new_delete.cpp`（替换全局
  operator new/delete 计数），GCC 15.3.0 `-std=c++23 -O2` 实测——裸 `new[]+delete[]`
  live=0、忘记释放 live=1（泄漏可见）、`unique_ptr<int[]>`/`vector` 自动归零、
  `push_back`×1000 **分配 11 次/8188 字节**（vs 一次性构造 1 次）——`reserve()` 的
  意义变成可验证数字。块数 7534 不变。
- **方法论沉淀**：自动密度指标必须人工复核再动手（与 prose_density 误伤深章同型）；
  ⚠️ 本地全量编译必须在**标准工具链**（mingw1530）下判定——WSL gcc 13.3.0 缺
  `<print>` 会产生 ch125/126 的平台假阳性回归。
- 门禁：compile_gate PASS（0 regression）/ exempt_audit PASS / gen_metrics ✅（114）/
  consistency / whitespace / terminology / dangling 全 RC=0。

### 2026-09-05（续二）编译器基线双轨规则 + 矛盾头部修正（工程③首批）
- **CONVENTIONS §1.1 新增「编译器基线双轨」**：当前标准基线 GCC 15.3.0（`mingw1530_64`，标签 `[实现·GCC15]` 与新取证为准）vs 历史取证基线 GCC 13.1.0（`mingw1310_64`，既有 `Examples/*.asm`/libstdc++ 摘录/基准输出版本与路径必须原样保留，改写即伪造取证）；表述规范 + 同章双代摘录的行号归属要求。
- **修正 4 处矛盾头部**（只加不改，取证事实零改动）：ch41 立场分层/源码根、ch42 平台行、ch78/ch79 标准基行——统一「取证基线 13.1.0／当前标准基线 15.3.0」双标表述，消除「本机=13.1.0」的当前环境歧义。
- 其余 69 文件取证头部的 13.1.0 为真实产物记录，按 §1.1 保留；后续新取证一律 15.3.0。
