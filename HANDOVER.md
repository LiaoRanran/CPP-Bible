...
+ **30 章真深耕启动**（`prose_density` 量化基线 + ch22 样板章验收 + 标题截断 536 处清零进门禁）。

## 当前状态总表（2026-09-02 实测）

| 维度 | 数据 | 状态 |
|------|------|------|
| 章节 | 147 章 / 16 part / 151 文件 / 240,748 行 | ✅ |
| cpp 代码块 | 7,534 | ✅ |
| asm 块 | 497（锚定 203 = 40.8%，**DRIFT=0**） | ✅ 证据真实 |
...
| 极致打磨 15 条 | **11/15** | 🟡 见诚实边界 |
| CI | `.github/workflows/ci.yml` 七 job | ✅ |
| Git | 有 remote，master 已推送，工作树干净 | ✅ |

### 「极致打磨」15 条看板（达标 11/15）

已达标：`chapter_levels` 147 · `one_liner` 147 · `analogy` 147 · `pitfall` 147 · `exercise_zero_chapters` 0 ·
`exercise_median` 5 · `extra_css_lines` 488 · `material_features` 16 · `changelog_lag` 0 · `gcc_hardcodes` 0 ·
`asm_anchor_rate` 40.8%（09-02 真机锚定推进后达标，诚实上限见 `gen_metrics` 看板说明）。
...
| `unverified` | 296 | 120 | ≈192（130 微架构 + 56 声明头 + 6 C++26 假设，0 个可本机复测数据声明）。294→297 系 ch109 新增 ① 节对 ARM 弱内存行为与 ~ns 经验值**如实**标注 `[UNVERIFIED]`，按红线不为凑指标摘除；09-02 深耕清编造后回落至 296 |
| `root_docs` | 41 | 10 | 有意保留（根目录元文档互链，见 `RELEASE.md`） |
| `glossary_lines` | 507 | 800 | 自动生成（`gen_indexes.py` 文件头明令勿手改；随术语沉淀自然增长） |
...
### 六、30 章真深耕启动（09-02 下午追加：诊断 → 样板 → 清污 → 落档）

> 外部锐评定调后的转向：停止「形式完备」推进，转入「少而准」——30 章真深耕，深度靠实证凿，不靠常识流。

1. **量化基线（`tools/prose_density.py`，`9cd0487`）**：叙述密度测量工具——段落行/正文行口径，
   全书 65.5%、最贫血 20 章平均 48.9%（按全部行含代码块计，纯叙述仅 ~32%，即「深度贫血」的量化证据）。
2. **样板章 ch22 验收（`dbc1af7`）**：字段卡→因果叙述，密度 **52.4%→65.5%**；随行清掉编造编译器符号
   （`Sema::ActOnAutoDecl`/`Sema::ActOnDecltype`→`[实现-推断]` 标注）、12 处围栏误闭合、徽章前缀缺失
   （`· ++14`→`· C++14`）。范式四要点（因果叙述/清编造/实证锚定/格式随行）经实战验证。
3. **标题截断污染清零（`60fb2c8` + `9807292`）**：全书 **536 处**示例标题生成期截断（未闭合括号/断词/
   悬挂标点），新工具 `tools/caption_truncation_audit.py` 按三级置信度（HIGH/MED/LOW）标题补全 + 引注补全
   + 最近标题优先匹配修复 535 处、人工重建 1 处，全量清零；`--check` 进 preflight 门禁（11 项检查）防回归。
4. **名单与范式落档**：30 章名单（A 机制主线 12 / B 性能并发 8 / C 源码精读 5 / D 教学骨干 5，
   `prose_density --top 40` 数据驱动）+ 单章验收五条 + 节奏红线，落 `CONTENT_DEPTH_ROADMAP.md` 顶部
   「当前主线」节；SOP-5 落 `NEXT_LLM.md`。
5. **提交链（09-02 下午，7 个）**：`9cd0487`(prose_density) → `dbc1af7`(ch22 样板) → `60fb2c8`(标题截断清零)
   → `9807292`(截断门禁进 preflight) + 本批文档落档提交。推送后 pre-push 11/11 全过，CI 已触发。
...
- 剩余待办：**30 章深耕为当前主线**（名单/范式/验收见 `CONTENT_DEPTH_ROADMAP.md` 顶部节 + `NEXT_LLM.md` SOP-5，
  下一章 ch69_constexpr）、㉒.2 模板同构（风格问题，不批量动）、CI 待确认全绿（PERF_CONFLICT 已于 09-02 复核收口，见「五」）。
