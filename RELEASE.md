...
> 版本：1.0.0（已发布）
> 发布日期：2026-07-12（本地竣工）｜ 2026-07-13 补强续推 + 站点重建｜ 2026-07-14 编译门禁上线 + 跨平台修复 + 去封面
> 维护者：项目作者 + Hy3（后续打磨扩写）
...
| 竣工-3 | CI 全 job 实跑（quality/compile/site/pdf/epub/deploy） | ✅ | run #15（98ecd85）全绿，含编译门禁 |
| 竣工-4 | PDF / EPUB 成品验证 | ✅ | CI 出 PDF/EPUB 成功，去封面后构建干净 |
| 竣工-5 | 去重审计 | ✅ | `deduplication_audit.py` → DUP%=0.0 / WTR%=0.00（147 章） |
| 竣工-6 | 本文件 RELEASE.md | ✅ | — |

**说明**：竣工-3 / 竣工-4 已于 2026-07-14 经 CI run #15（commit `98ecd85`）实证全绿（含新上线的编译门禁），PDF / EPUB 成品正常产出。本地健康度已达"可宣布 1.0 已发布"标准。
...
|------|------|------|
| 规模 | 章数 | 147 |
| 规模 | 代码行（Book/） | 146,657（~14.7 万行） |
| 规模 | cpp 代码块 | 6,840（可编译 cpp 块） |
| 门禁 | `consistency_check.py` | **100/100**（ERROR=0 / WARN=0） |
| 门禁 | `compile_all.py` + `compile_gate.py` | 147 章 / 112 通过 / 66 豁免（0 真实语法/类型回归） |
| 密度 | `density_audit.py` v3 | **24.2/30**（shallow=0，密度深化封顶） |
| 断言 | `run_cpp_assertions.py` | **FAIL=0**（编译期 349 + 运行期 96 PASS） |
| 交引 | `crossref_audit.py` | **1190 引用 / 0 断链** |
| 质量 | `deduplication_audit.py`（v4） | **17.8/30 均分**（全部 16 part ≥15，v4=13 池已清零） |
...
1. **CI 已实跑**：2026-07-14 run #15（98ecd85）六 job 全绿（含编译门禁）；PDF / EPUB 由 CI 正常产出。本机仍无 xelatex，本地构建需在 WSL / CI 进行。
...
_**2026-07-13 铺路轮**（继续推进项目）：三批 15 章附录 G 注入真实工业引用（含 GitHub permalink）+ 底层深度分析，每附录 ≥10 行实质性内容。v4=13 池 14→0 彻底清零，v4 均分 17.1→17.8/30。注入章：batch1（ch99/ch154/ch107/ch80/ch142）、batch2（ch162/ch32/ch60/ch64/ch67）、batch3（ch50/ch17/ch137/ch145/ch68）。全部 INS 为经实证的真实项目引用（LLVM/Qt/Chromium/Boost/Abseil/Eigen/WebKit/DPDK/folly 等），无关键词堆砌注水。commit `352622a→...`（后续提交）。_

_**2026-07-14 发布收尾轮**（闭环 + 上线发布物料）：① **CI 编译门禁上线**：`compile_all.py --main-only` → `compile_gate.py`（双层豁免：显式清单 + 错误模式），新真实 `SYNTAX`/`TYPE_MISMATCH` 立即变红。② **跨平台回归修复**（本地 mingw 软过、Linux gcc13 暴露）：`ch30` 补 `<csignal>`、`ch35`/`ch36` 补 `<cstdint>`、`ch62` `W<int>` 成员 `double`→`char`（LP64 `static_assert` 失败，LLP64 巧合掩盖）；门禁 WINDOWS 模式补 `winsock2.h|ws2tcpip.h`。commit `98ecd85` 推送触发 run #15，六 job 全绿（门禁按设计拦住 8 处跨平台回归后修复闭环）。③ **EPUB 去封面**：移除 `--epub-cover-image` 与 `assets/cover.png`（独立作品，无外部封面依赖）；EPUB/PDF 经 CI 实证构建干净。④ **豁免清单元数据化**：`gen_compile_exempt.py` 从审计报告生成 `compile_exempt.json`，66 块 0 UNCLASSIFIED。⑤ **密度审计 v3 封顶**：全量均分 24.2/30，shallow=0 —— 密度深化不再是主线，转向质量收尾。⑥ **发布物料补齐**：新增根目录 `README.md`（项目定位/结构/门禁/构建命令）、`CHANGELOG.md`（v1.0.0 里程碑）；本文件升 `1.0.0（已发布）`，竣工-3/4 标 ✅，质量基线补编译门禁与密度 v3 行，规模数字更新为实测 146,657 行 / 6,840 cpp 块。⚠️ 安全：本次推送仍用约定视为已泄漏的 PAT，待全部任务结束后须吊销重建，remote 保持无 token。_
