...
P1（内存/对象模型）**不是白纸**：57 例精品实证里，对象模型主题已覆盖大半（虚调用、RTTI、EBO、SSO、shared_ptr 计数、虚继承 thunk、optional/variant/any/function 布局、全部 STL 容器内存模型均已真机实证）。原四个核心缺口"文字讲了、真机证据缺"，**本轮已全部补齐真机实证**（`ASM-41-make_shared` / `ASM-41-esft` / `ASM-50-mi` / `ASM-47-deleting_dtor`），它们是对象模型里最能揭示底层真相的点。
...
| C3 | **make_shared 单次分配 vs `shared_ptr(new)` 两次分配** | ✅ | ASM-41-make_shared |
| C4 | **enable_shared_from_this：控制块内嵌 weak_this 的初始化时机** | ✅ | ASM-41-esft |
...
**汇总**：27 条结论，✅ 23 条真机实证（含本轮补齐的 4 条核心缺口）、🟡 4 条章内片段未锚定、❌ **0 条缺口**。

---

## 2. 四个优先缺口（✅ 本轮已全部实证，证据归档 `_asm_demo/`）

> 每条给出"要揭示的底层事实 + 验证方法"，弱模型照此写 `_asm_demo/*.cpp`、跑 objdump、归档 `.cpp/.s`，再更新 `_asm_demo/INDEX.md` 与 STATE 计数。

### ✅ P1-GAP-1：make_shared 单次分配（已实证 → `ASM-41-make_shared`）
...
### ✅ P1-GAP-2：多继承对象布局（已实证 → `ASM-50-mi`）
...
### ✅ P1-GAP-3：enable_shared_from_this（已实证 → `ASM-41-esft`）
...
### ✅ P1-GAP-4：虚析构 deleting destructor（已实证 → `ASM-47-deleting_dtor`）
