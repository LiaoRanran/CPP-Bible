...
### S6 的补充：L3 自动变异测试（`tools/mutation_fuzz.py`）——**发现器，不是门禁**

| 项 | 内容 |
|---|---|
| 谁 | Gatekeeper（机器）：对**真实卡**自动批量变异（M1–M7 七类算子），看门禁有没有反应 |
| 与 S6 的边界 | `poison_drill` = **人工写死**的载荷，测"已知攻击还有没有效"（回归锁，漏网即 CI 红）；`mutation_fuzz` = **自动生成**的载荷，找"毒样例还没覆盖的**新**逃逸"（发现器，`escaped` 是**产物不是红灯**，默认 exit 0） |
| 自动判定 | 三分类 `blocked / escaped / n_a`（`n_a` 与 `malformed` **不进拦截率分母**——把 n_a 当 blocked 凑率是自欺，见 539 B2 / 543 P1） |
| 产物 | `data/mutation/*.json`（逐变体 + 复现命令）+ `data/mutation/README_v1.md`（首份**全量**基线：83 卡 × 7 算子 = 1188 变体，escaped 227 / 严格拦截率 64.3%） |
| 何时跑 | **不进 CI**（一次性全量 ≈ 11 分钟）；由人在"该体检了"时跑 `cppbible mutation --all`，或按卡/按算子小批复现 |
| 处置 | 逃逸**不阻断**：逐条定性后交人裁决（真洞 → 出修复批；形态约定 → 出 warn 规则；变异点无效 → 记口径） |

> 口径提醒（548 实测）：**"逃逸"要分三种**——真逃逸、畸形变体（`malformed`，已单列）、
> **变异点无效**（变异落在门禁根本不读的字段/注释上，改前改后门禁都不会有反应）。
> 第三种看起来是"逃逸"，实为**对门禁的无效提问**，不许算进"门禁漏了 N 条"。
