...
| 项 | 内容 |
|---|---|
| 谁 | **Author**（Agent/人）只产出原子卡与证据卡；**RedTeam** 只做证伪（KPI=挑出错，产出攻击工单）；**Gatekeeper** = `gate_engine` + `atom_evidence_replay` 机器复算；**Golden 认可权唯人**（`verified_by: human:*`） |
| 环节 | 写入 → 机器复算 → 人工签收 → 入库（三段分离，同一执行者不得跨越签收段） |
| 手段 | `S1-AUTHOR-SELF-VERIFY` 规则：`status=verified` ⟹ frontmatter 含 `verified_by: human:*`（Agent 写不出合法签收——写了也不被机器认） |
| 自动判定 | 机器查字段前缀（gate_engine，block 级） |
| 处置 | **阻断**：无人工签收的 verified 直接红 |
...
| 手段 | 六指标全部机器复算：block_findings / warn_findings / atoms_total / evidence_total / verified_atoms / replay_confirm（ADR-0005 快照-漂移底座） |
| 自动判定 | block/warn 上升、原子/证据/verified/复算通过数下降 → **恶化即红**；改善提示 `sync` 把进步锁进快照 |
...
| 手段 | 注入三类毒样例 + 一个阴性对照：**P1 假论断**（verified 无证据无签收 → S1+S2+VERIFIED-BOUND 必须拦）、**P2 过期工件**（sha256 不符 → replay 必须拦）、**P3 缺反例**（→ EV-FALSIFICATION 必须拦）、**阴性**（干净卡必须放行，防恒红） |
...
- 3 个毒样例 = `poison_drill.py` 的 P1/P2/P3（**机器化，每次 CI 现场攻击**）；
