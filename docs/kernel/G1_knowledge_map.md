...
**待确认**：cpp 块本脚本 7572 vs `metrics.json` 7515，差 57。可能原因：metrics 的 `blocks.cpp` 统计口径排除了某些围栏（如 ```cpp title 变体或空块）。**不阻塞 G1**，列入 G2 前核对项（谁多谁少要定死，否则后续"一手实证覆盖率"分母会漂）。
