...
## 六、548：从"手动 40 变异"到"自动全量基线"（并给出 L3 的定位）

499/500 的变异是**手工构造、按卡逐个看**（5 张源卡 × 8 变异）。539 Part B 起改为
`tools/mutation_fuzz.py`：七类算子（M1 字段删除 / M2 路径变形 / M3 断言弱化 / M4 恒真注入 /
M5 claim 自标 / M6 YAML 变形 / M7 数值哈希篡改）对**全部真实卡**自动批量变异，
沙箱内逐变体跑门禁（+ 必要时 replay），判决三分类。

**548 前的状态**：只有 `--limit 2` 的 smoke（够证明机器能跑，不足以说"库有多硬"）。
**548 补上首份全量基线**（方法/口径/复跑命令见 `data/mutation/README_v1.md`，
机器可读原报告 `data/mutation/full_baseline_v1.json`）：

| 项 | 数 |
|---|---|
| 卡 / 算子 / 变体 | 83 张（56 证据 + 27 原子）/ 7 类 / **1188 个** |
| blocked / escaped / n_a | 729（严格 615）/ **227** / 232（malformed **0**） |
| 严格拦截率 / 含 warn 处置率 | **64.3%** / 76.3% |
| 全量耗时 | 11 分钟（548 Part 0 提速后；改前同口径外推 ≈46 分钟） |

**逃逸四类定性**（别把"无效提问"算成门禁漏网）：

1. **M2 跨平台路径异体（207 条，最大头）**：`Examples/x.cpp` → 全大写 / 加 `./` / 换反斜杠，
   Windows 上三种写法都打得开（NTFS 大小写不敏感 + 两种分隔符都收）⇒ 门禁无反应，
   同一张卡到 Linux CI 就是 `No such file`。**548 Part 2 已收口**：加
   `CARD-PATH-NOT-CANONICAL`（warn 级、存量 0 命中、毒样例 P69）⇒ 141 条转 blocked(warn_only)，
   余 66 条的变异点落在**注释/正文**（门禁不读的地方）——属"变异点无效"，不是新洞。
2. **M3 区间断言降级 / 去 `-Werror`（11 条）**：`contains_in → contains` 后"必须在 symbol
   区间内"的约束丢失；命令去 `-Werror` 后"零诊断"不可判定。**已知清单**（543 已定性），
   修法受 541 实测约束（直接扩 gate 会撞存量）⇒ 留给"单点互斥判定"。
3. **M6 块式 → flow（8 条）**：`matrix:` 块式列表写成 `{a, b}` 后两个解析器都照样读，
   门禁对**形态**零要求 ⇒ 是"允许 flow 写法"还是"要求块式"需人裁决。
4. **M1 删 `negative_controls`（1 条）**：阴性对照缺失无闸门（只有别的规则管"有没有"）。

**L3 的定位（写死，防误用）**：`mutation_fuzz` 是**发现器**——`escaped` 是产物不是红灯，
不进 CI、不阻断提交；它回答"门禁对自动生成的攻击有多硬"，不回答"库能不能发"。
红线：**提速/简化不得牺牲跨卡规则**（每个变体仍是全库扫描、全量 diff，绝不按卡裁剪）。
