...
- gate 规则 `EV-ZERO-DIAG-WERROR`（**block**，472 P1-1 由 warn 升格）强制本约定：命中措辞而无
  `-Werror` 即报（防回归）。升格依据：warn 级只"可见化"，卡照样 confirm 直推 verified（v5 E10a/b
  实证）；且该判据在无 `-Werror` 时不可机器判定 ⇒ 不可复算判据不得放行。存量 56 卡 0 命中，零误伤。
