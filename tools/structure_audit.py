#!/usr/bin/env python3
"""结构审计：示例头星级格统一（star）+ H2 数量基线（h2）。

与 l2_state 同一方法论：先把存量债显性化并冻结基线，门禁只防恶化，
...
H2 基线：`h2-sync` 把每章 `^## ` 计数写入 tools/structure_state.json；
`h2-check` 只在「H2 数量增加（恶化）」时 exit 1，减少（合并/拆分改善）提示 sync。

用法：
    python tools/structure_audit.py check --star      # 星级门禁（CI 硬门禁）
    python tools/structure_audit.py fix-star          # 方括号 → span 归一（字节安全）
    python tools/structure_audit.py scan --star       # 只盘点不落盘
    python tools/structure_audit.py h2-sync           # H2 基线快照
    python tools/structure_audit.py h2-check          # H2 防恶化门禁
    python tools/structure_audit.py check --star --json out.json
"""
...
ROOT = HERE.parent
BOOK = ROOT / "Book"
STATE_PATH = HERE / "structure_state.json"
...
def h2_sync() -> str:
    ...
    STATE_PATH.write_text(
        json.dumps({"schema": "cppbible-structure/1.0", "h2": snap},
...
def main(argv: Sequence[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="结构审计（星级格 / H2 基线）")
