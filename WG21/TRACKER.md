...
| P2093 | `std::print` | `__cpp_lib_print` | ✅ 202211 | ✅ 18 | ✅ 19.37 | GCC 实测 202211 | ch131 |
...
| P1899 | `std::mdspan` | `__cpp_lib_mdspan` | ❌ `<undef>` | ✅ 18 | ✅ 19.39 | GCC 15.3 实测 UNDEF | ch82 |

## C++26 关键提案（进行中，随标准演进变动）

| 提案 | 特性 | feature-test macro | GCC | Clang [DOC] | MSVC [DOC] | 备注 | 章节 |
|------|------|-------------------|:---:|:---:|:---:|------|------|
| P2996 | 静态反射（reflection） | `__cpp_reflection` | ❌ | 🚧 实验 | ❌ | 里程碑特性，编译器均在起步 | ch74 |
| P2300 | `std::execution`（Sender/Receiver） | 未定名 | ❌ | 🚧 | ❌ | 参考实现 stdexec；注意 `__cpp_lib_execution`（实测 201902）是 **C++17 并行算法执行策略**，与 P2300 无关，勿混淆 | ch114 |
...
| P2900 | Contracts（合约） | `__cpp_contracts`（拟） | ❌ | 🚧 实验 | ❌ | C++26 方向，反复进出标准 | ch121 |
