...
| 在飞的活？ | 内容侧「学习目标→问题驱动论证」打磨——已收口 **58 章**（历史9 + OO6 + STL17 + 模板12 + 语言5 + 现代4 + 性能3 + 其他2），见「已收口批次（2026-09-01）」；**仅剩 2 个「学习目标」章未处理：ch31（运算符重载，part03_language）+ ch109（内存屏障/栅栏，part09_concurrency）** |
| 剩余待办？ | 打磨剩余 2 个「学习目标」章 ch31/ch109；工具链升级剩余项见 `TOOLCHAIN_UPGRADE_NEXT.md`（mypy 类型 triage 等）；内容侧**优先审 8 个 UNVERIFIED 章**（ch02/05/16/132/148/149/150/165），见下文健康度评估；ch163 网络章是否补 POSIX 对照（低优先） |
...
### 内容打磨：学习目标 → 问题驱动论证（58 章，分 9 批提交，勿重做）
...
- **下一批（收尾）**：仅剩 2 个「学习目标」章未打磨：ch31（运算符重载，part03_language，标题带 `<span class="badge badge-std">标准</span>`）+ ch109（内存屏障/栅栏，part09_concurrency）。完成后本打磨专项收官。
