...
| 当前阶段？ | **30 章真深耕（叙述密度治理）进行中**（2026-09-02 定调，主线见 `CONTENT_DEPTH_ROADMAP.md` 顶部节）；L2/L3 已收官，CI 七 job 真绿 |
| 在飞的活？ | 🔶 **30 章深耕**——样板 ch22 已验收（52.4%→65.5%，`dbc1af7`），名单 30 章与验收标准已落 `CONTENT_DEPTH_ROADMAP.md`「当前主线」节；SOP 见本文件 SOP-5 |
| 剩余待办？ | ①30 章深耕（主线，串行/小批量，禁一波流）②㉒.2 模板同构（风格问题，不批量动）③mypy 类型 triage（需 `uv sync --extra dev` 重建 `.venv`，本机高风险，见 `TOOLCHAIN_UPGRADE_NEXT.md`）。~~PERF_CONFLICT 7 条~~ 已于 09-02 逐条复核固化豁免（`b10e5cb`）；~~标题截断污染~~ 已清零 536 处并进门禁（`60fb2c8`+`9807292`） |
...
### SOP-5：30 章深耕——叙述密度治理（当前主线）
> 名单 + 验收标准 + 节奏红线的唯一事实源：`CONTENT_DEPTH_ROADMAP.md` 顶部「当前主线」节。
> 样板参照：`Book/part03_language/ch22_auto_decltype.md`（52.4%→65.5%，commit `dbc1af7`）。

1. 测基线：`python tools/prose_density.py <章文件>`，记录改前密度。
2. 逐专题把「字段卡」（定义/历史/编译器行为式罗列）改写为**因果叙述**：先立判断、再给依据
   （标准条款/真机证据/一致性论证），段落间有承接。**不是删列表凑数字**——保留有信息量的清单，
   消灭的是「填空式并列」。
3. 清编造随行：编译器符号/预期输出/性能数字，不确定即 `[实现-推断]`/`[UNVERIFIED]`/「示意」。
4. 格式修复随行：围栏误闭合（```cpp 结尾）、徽章前缀（`· ++14`→`· C++14`）、`——`误用为冒号。
5. 验收五条（全过才算完）：密度 ≥60% · 字段卡清零 · 禁编造 · ASM 锚定只升不降（DRIFT=0）·
   preflight 11/11 + quality 16/16。
6. 提交节奏：**每章独立提交**；串行或 ≤3 章小批量。🔴 禁止 30 章并行一波流（L0）。
...
### ✅ 内容打磨：学习目标 → 问题驱动论证（60 章，分 10 批提交，已收官，勿重做）
...
- **批次**：
  - `fd6897a` ch121 试点 → `bc4fd45` 历史 9 章（ch01, ch04–ch10）→ `e2ffc76` OO 6 章（ch47–52）→ `dc8ed07` STL/现代 4 章（ch80/82/83/115）→ `65b3bf1` STL 容器 4 章（ch77/84/85/86）→ `f80c7cd` 模板板块 12 章（ch60–64/66–72）→ `5cf5362` STL 板块 10 章（ch76/78/79/87/89–94）→ `957870a` 语言 5 章（ch22/24/29/30/32）→ `008c265` 现代 4 章（ch116/120/122/123）→ `c246651` 性能 3 章（ch152/153/158）→ **`c53848b` 收尾 2 章（ch31 运算符重载 + ch109 内存屏障/fence）**。
- **内容准确性修正（禁虚构红线）**：ch85 原声称「C++20 透明哈希」——透明哈希对 unordered 容器**并未进入标准库**（`std::hash` 非 transparent），已改为诚实表述；ch83 消除反引号内 `Book/` 前缀引用。
- **门禁**：每批本地 preflight 均 17/17 通过；CI 最近 6 run（10dc21e/dc8ed07/e2ffc76/65b3bf1/5cf5362 等）已验证/推送，CI 待确认。
- **收官状态（`c53848b` 后）**：全书 147 章已无「① 学习目标」节。改名为「① 我们真正要回答的问题」后，另扫出并修掉 1 处悬空回指：ch76:1311「见 ① 学习目标」→「见 ① 我们真正要回答的问题」（`_scan_lx.py` 式扫描可复现，正则 `①\s*学习目标`）。
- **配套修复**：`README:5` 写死 cpp 块 7531 → 7525（前序提交引入的数字漂移，CI 硬门禁 `gen_metrics.py --check` 项）；`d0d5dda` 修站点欢迎页 part 表标题全空（`DEFAULT_PART_TITLES` 兜底字典定义了却未接上 `parse_part_titles()`，而 INDEX.md 是项目文档索引、通篇无 `Part` 字样）。CHANGELOG 已补 2026-09-01 条目（`4d934f3`，changelog_lag 53→0）。
...
- **全书健康度评估（外部 441 样本分层编译 + 全量扫描，结论需记录）**：
  ...
  - **验证体系初具雏形**：74/147 章带显式 `[VERIFIED]/[UNVERIFIED]` 标记（65 已验证 / 8 未验证）。**下一步优先审 8 个 UNVERIFIED**：ch02（标准化组织）、ch05（C++14）、ch16（IDE）、ch132（LevelDB/RocksDB）、ch148（Git 工作流）、ch149（CI/CD）、ch150（测试策略）、ch165（进阶路线图）。
    → **审计已收口（2026-09-01）**：ch05 翻转（新增 `_asm_demo/ch05_generic_lambda*` 真机 GCC 15.3.0 证据，-O0 见泛型 lambda 独立实例符号 / -O2 见零调用内联，`book_asm_freshness` 0 漂移）；其余 7 章逐章扫描（数字断言/基准词正则 + 人工复核）均为外部工具经验值或已显式标注「示意，非本机实测」，无机器可验断言，UNVERIFIED 依红线保留。验证横幅现 73 章（66 VERIFIED / 7 UNVERIFIED）。
...
| `CONTENT_DEPTH_ROADMAP.md` | **内容深化主线**（顶部「当前主线」= 30 章深耕名单/范式/验收；其后五阶：汇编实证矩阵 / 实战项目线 / 陷阱体系 / 人文温度 / 内容形态补全） | ✅ |
| `tools/prose_density.py` | 叙述密度测量（报告型）：全书/单章密度 + 最贫血章排名（`--top N`）；30 章深耕的量化基线与验收工具 | ✅ |
| `tools/caption_truncation_audit.py` | 示例标题截断审计：检测/修复生成期截断（未闭合括号/断词/悬挂标点）；`--check` 已进 preflight 门禁 | ✅ |
