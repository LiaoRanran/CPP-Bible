...
def test_stats_in_out():
    out = m610.collect_defense_chain_stats({})
    assert (out["in"], out["out"], out["undec"]) == (114, 7, 0)


def test_defeating_edges():
    out = m610.collect_defense_chain_stats({})
    assert out["defeating_edges"] == 17 and out["total_edges"] == 388


def test_no_defenders_no_attackers():
    out = m610.collect_defense_chain_stats({})
    assert out["no_defenders"] == 7 and out["no_attackers"] == 4
    assert out["credibility_distribution"] == {"high": 0, "medium": 114, "low": 7}
...
def test_agrees_with_608_grounded():
    ...
    assert (a["in"], a["out"], a["undec"]) == (b["in"], b["out"], b["undec"]) == (114, 7, 0)
    assert a["total_edges"] == b["candidate_edges_total"] == 388
...
def test_container_has_all_three_keys():
    ...
    assert d["defense_chain_stats"]["defeating_edges"] == 17
