...
锁十件事：
  ...
  * I3 CLI：--check 包含 4 个不变量 / --list 包含 lock_consistency
  * metrics 接入：invariants 字段存在 / 4 个不变量全 true / all_pass=true
...
# ── CLI 包含 I3 ─────────────────────────────────────────────────────────────────
def test_cli_list_has_4_invariants():
    proc = subprocess.run([sys.executable, str(TOOL), "--list"],
                          capture_output=True, text=True, timeout=10)
    assert proc.returncode == 0
    for inv in ("artifact_restore", "build_reproducibility", "sandbox_isolation", "lock_consistency"):
        assert inv in proc.stdout


def test_cli_check_has_4_results():
...
def test_metrics_invariants_has_4_keys():
    import tools.metrics_collector as mc
    s = mc.collect(with_heavy=False)
    inv = s["invariants"]
    assert set(inv.keys()) == {"artifact_restore", "build_reproducibility",
                                "sandbox_isolation", "lock_consistency"}
