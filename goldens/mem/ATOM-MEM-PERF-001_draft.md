...
| 维度 | 自评 | 说明 |
|---|---|---|
| rubric 总分 | **4/5** | 五重剖面齐全；量化证据双层互证（volatile 计数 + 汇编 `_Znay` 调用点）。留待人审一点：论断"快多少"目前以
  字节搬运量与分配次数刻画，未跑 `-O3` 基准计时（性能类按 M2 §2 建议加 `-O3` 与 `Benchmarks/` 对照，可在原子化前补）。 |

### 4 分锚定依据
1. **量化而非口号**：heap 移动 0 分配（8 字节指针）vs 值类型移动==拷贝（32 字节），用 volatile 计数 + 汇编调用点双层互证。
2. **自带证伪对照**：值类型"无收益"组与 heap"有收益"组同 TU 对比，唯一变量是"是否持有动态资源"。
3. **可操作判据**：三条"不该用 move"的判据（纯值 / const 源 / return 局部），接住 MIS-MEM-001。
