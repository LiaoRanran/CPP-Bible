...
矩阵：`EV-UB-001` 覆盖 `{GCC 15.3, 13.1, 8.1} × {-O0, -O2}` 6 组 ✓；
`EV-UB-002` 覆盖 `-O2`（主档）+ `-O0` / `-O2 -fno-strict-aliasing`（人工补跑）✓。
...
- **Clang 列**：本机与 WSL 均无 Clang；CI 的 Clang 仅作交叉验证（`continue-on-error`）。
  按 M2 §2 永久边界，**MSVC 以标准条文代替**；Clang 列本轮以 **CI Gray-zone 步骤留痕**补齐，
  若 CI 侧不可用则**降级为"标准条文 + `unspecified` 语义"并标"部分达成"**，
  登记豁免票（`owner: human:liaoranran`，30 天）——**具体路径见文末"Clang 列处置"**。
...
claim_boundary:
  standard: [C++17, C++23]
  compilers: [GCC 15.3.0, GCC 13.1.0, GCC 8.1.0]
  opt: [-O0, -O2]
...
superiority: >-
  ...
  unspecified 侧 6 组矩阵全部一致（教"一致 ≠ 可依赖"），UB 侧用
...
| 证据 | 观测 | 结论 |
|---|---|---|
| `EV-UB-001`（6 组矩阵：GCC 8.1/13.1/15.3 × -O0/-O2） | 全部输出 `h` → `g` → `f(1,2)`，rc=0，sanitizer 无报错 | 属 unspecified（有合法集合、不崩） |
...
**关键教学点（EV-UB-001 的真正价值）**：6 组矩阵横跨 GCC 8.1 → 15.3（7 年版本）+ 两档优化，
**全部一致**——很容易让人误以为"标准规定了右→左"。事实相反：标准给的是**合法结果集合**，
这批实现恰好都选了右→左。**一致 ≠ 可依赖。**
...
2. **"一致 ≠ 可依赖"的教法**：用 6 组跨 7 年版本的一致结果，反过来说明"这不是标准保证"，
   是否比单纯念标准条文更有说服力？
...
- **路线**：先在 CI 的 Clang 侧跑样板 B 夹具（`Examples/_atom_eval_order.cpp` /
  `_atom_strict_alias.cpp`）——若编译通过则留痕实测，符合 M2 "GCC + Clang 双编译器"边界；
  若不通过（已知全库 Clang 编译 audit 目前 exit 2），则**降级为"Clang 列以标准条文 +
  `unspecified` 语义代替"**，登记豁免票（`owner: human:liaoranran`，30 天到期），标注"**部分达成**"，
  等 Clang 兼容性修复后补实测。**本机不装 Clang。**
- CI 的 `Gray-zone Matrix` 步骤已用 `clang++`（若 runner 有）编译运行 `_atom_eval_order.cpp`；
  本轮为其补 `::notice::` 输出，使结果可经 check-runs API 公开读取（否则拿不到 job 日志）。
- **结果记录**：见本文件提交后的 CI 运行与 `docs/kernel/G4_selfcheck.md` 的"Clang 列"条目。
...
| 行家挑不出硬伤 | ❌ | ✅ | 三处定性均给标准条款；两组实验双平台复核（Windows sha 轨 / WSL 断言轨 + sanitizer 真跑 4/4）；夹具三次自身 UB 全部修掉并留痕；未覆盖项（Clang/MSVC）明标"部分达成" |
