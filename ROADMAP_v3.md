...
### §10.6 Phase APP 应用层增强（习题重写 / 工业深挖 / 用法演绎，2026-07-15 启动）
...
**Batch APP1（2026-07-15 完成）**：ch20/27/41/47/77/78/93/96 全覆盖 APP-A + APP-C（习题重写 + 用法演绎附录）。一致性门禁 147 章 0/0 维持。
**Batch APP2（2026-07-15 完成）**：ch21/31/39/42/43/44/50/51 全覆盖 APP-A + APP-C。其中 ch42（附录E `-fstrict-aliasing` 开关汇编）、ch50（[实现] MI vtable this 调整 thunk 汇编）含既有真机 ASM 附录，注入脚本采用"仅替换习题块 + EOF 追加演绎"双模式，ASM 零破坏。一致性门禁 0/0 维持，8 章 cpp 块 GCC13.1 -O2 编译校验通过。APP-A/APP-C 累计覆盖 16/147 章。
...
**Batch APP11（2026-07-16 完成）**：part05_oo 5 章 ch45/46/48/49/52 全覆盖 APP-A + APP-C。通用模板习题（max_safe/integral add/constexpr fact 与 OO 主题错配）替换为章主题对齐难度阶梯（★★/★★★/★★★★）；每章追加「用法演绎」附录；双门禁校验：CI 模拟（无 PRELUDE main-only，-fsyntax-only）0 新增回归，独立全链接校验（无 PRELUDE，include-hoist，真全链接）35 注入块 GCC 13.1 -O2 -Wall -Wextra 0 fail（OO 章无需 -pthread/-latomic）；一致性门禁 147 章 0/0 维持。preserve 模式：ch48 保留尾随「## 附录 E：typeid/dynamic_cast 真实汇编」、ch52 保留「## 附录 E：EBO 字节偏移」汇编实证（原书既有 ASM，独立校验器仅校验注入区）；ch48 演绎2 错误块（非多态 dynamic_cast 编译错误）用 ```text 围栏。所有 ```cpp 注入块自包含可编译且显式写全 `#include`。APP-A/APP-C 累计覆盖 65/147 章（批次数累加；唯一覆盖 62 章）。
**Batch APP12（2026-07-16 完成）**：part03_language 5 章 ch24/26/29/30/32 全覆盖 APP-A + APP-C。通用模板习题（max_safe/integral add/constexpr fact 与语言主题错配）替换为章主题对齐难度阶梯（★★/★★★/★★★★）；每章追加「用法演绎」附录；双门禁校验：CI 模拟（无 PRELUDE main-only，-fsyntax-only）0 新增回归，独立全链接校验（不裹命名空间，无 PRELUDE，include-hoist，真全链接）39 注入块 GCC 13.1 -O2 -Wall -Wextra 0 fail（语言章无需 -pthread/-latomic；不含 main 块用 -c 仅编译）；一致性门禁 147 章 0/0 维持。preserve 模式：ch24 保留尾随「## 附录：enum class 真机汇编实证」、ch32 保留「## 附录：std::initializer_list 真机汇编实证」汇编实证（原书既有 ASM，独立校验器按 `真机汇编实证` 尾精确截断仅校验注入区）；修复：ch24 演绎2 补 operator& 定义使 has() 自包含、ch26 演绎2 错误示范令 Node 继承 enable_shared_from_this 使 shared_from_this 合法、standalone TAIL_RE 由 `^## 附录` 收紧为 `^## 附录：.*真机汇编实证` 避免误截断注入区。所有 ```cpp 注入块自包含可编译且显式写全 `#include`。APP-A/APP-C 累计覆盖 70/147 章（批次数累加；唯一覆盖 67 章）。
**Batch APP13（2026-07-16 完成）**：part02_toolchain 8 章 ch11/12/13/14/15/16/17/18 全覆盖 APP-A + APP-C。通用模板习题（max_safe/integral add/constexpr fact 与工具链主题错配）替换为章主题对齐难度阶梯（★★/★★★/★★★★）；每章追加「用法演绎」附录；双门禁校验：CI 模拟（无 PRELUDE main-only，-fsyntax-only）0 新增回归（8 处既有正文 MULTI_FILE/MODULE/GUARDED_COMPILE 豁免），独立全链接校验（不裹命名空间，无 PRELUDE，include-hoist，真全链接）36 注入块 GCC 13.1 -O2 -Wall -Wextra 0 fail（工具链章无需 -pthread/-latomic）；一致性门禁 147 章 0/0 维持。8 章既有工业附录均位于习题锚点之前，简单替换（锚点→EOF）天然保留，无 preserve 截断需求；所有 ```cpp 注入块自包含可编译且显式写全 `#include`；修复：ch11 演绎2 为 old_init 补同 TU 定义使链接自包含、ch17 练习2 修正 offsetof(Packed, int)→offsetof(Packed, i)。APP-A/APP-C 累计覆盖 78/147 章（批次数累加；唯一覆盖 75 章）。
**Batch APP14（2026-07-16 完成）**：part01_history 5 章 ch03/04/05/06/07 全覆盖 APP-A + APP-C（C++98/03→C++20 特性章）。通用模板习题（max_safe/integral add/constexpr fact 与历史语境错配）替换为章主题对齐难度阶梯（★★/★★★/★★★★）；每章追加「用法演绎」附录；双门禁校验：CI 模拟（无 PRELUDE main-only，-fsyntax-only）0 新增回归，独立全链接校验（不裹命名空间，无 PRELUDE，include-hoist，真全链接）25 注入块 GCC 13.1 -O2 -Wall -Wextra 0 fail（全含 main 全链接）；一致性门禁 147 章 0/0 维持。特性选择受 GCC 13.1 -std=c++23 可编译约束（未选 C++23/26 特性，含 concepts/ranges 均 C++20 已完备特性）；5 章既有工业附录均位于习题锚点之前，简单替换（锚点→EOF）天然保留。所有 ```cpp 注入块自包含可编译且显式写全 `#include`。APP-A/APP-C 累计覆盖 83/147 章（批次数累加；唯一覆盖 80 章）。
**Batch APP15（2026-07-17 完成）**：part07_stl 8章 习题重写+用法演绎 (list/array/string/span/map/set/unordered/adapters)。通用模板习题替换为章主题对齐难度阶梯（★★/★★★/★★★★）；每章追加「用法演绎」附录；双门禁校验：CI 模拟（无 PRELUDE main-only，-fsyntax-only）0 新增回归，独立全链接校验（不裹命名空间，无 PRELUDE，include-hoist，真全链接）注入区 GCC 13.1 -O2 -Wall -Wextra 0 fail（无需 -pthread/-latomic）；一致性门禁 147 章 0/0 维持。简单替换(习题锚点→EOF, 工业附录在锚点前已保留)。所有 ```cpp 注入块自包含可编译且显式写全 `#include`。APP-A/APP-C 累计覆盖 91/147 章（批次数累加；唯一覆盖 88 章）。
...
**C. 旧残留清理（已完成）**
- 跟踪文件 `tools/compile_report.json` 此前留存 mingw1310（gcc 13.1.0）旧审计（3560 块 / 66 失败），属误导残留，已清除。
- 覆盖为权威 gcc15 全量审计：mingw1530 gcc 15.3.0、`-std=c++23 -O0 -fsyntax-only`、`--main-only`，**147 章 / 3720 块 / 65 失败 / partial=False**（与 `_audit_gcc15.json` 同源 canonical 审计）。
- 过程插曲：后台并发一度出现两个 `compile_all.py` 写入进程竞写同一 `--json`（PID 61704 + 70316），已 `taskkill /T /F` 终止竞写、备份污染文件、改从已门禁验证的 canonical `_audit_gcc15.json` 直接填充（避免重跑被重模板块拖慢 30–60 分钟，结论等价）。
- `compile_gate.py` 复核：**65 失败 = 65 豁免，新增回归 0、清单冗余 0**。
- 注：本会话后续追加的 6 个例维度补强块（ch51/ch52/ch50/ch67/ch69/ch145）均经 gcc15.3.0 `-std=c++23 -O2 -Wall -Wextra` 实测零警告通过，属已知可编译，不计入上述审计快照的失败集。

**D. 书内 asm 围栏新鲜度审计（已完成）**
- 阶段3 `_verify_asm_evidence.py` 已证存储工件与 gcc15.3.0 一致；本项补查最后一环：Book 正文 `asm` 围栏展示的用户函数是否仍定义于对应 `_asm_demo` 工件。
- 新增 `tools/book_asm_freshness.py`：c++filt 双侧 demangle + 滤 libstdc++/CRT/编译器内部符号 + **只比用户函数定义标号**（忽略书内策展伪标号与库调用）。
- 全量审计 18 个含围栏且引用工件的章：**初版误报 109 个**（mangled/demangled 错位、库符号、策展伪标号噪声），修正方法学后定位 **1 个真缺陷** `ch82_span.md`：
  - 该围栏为 Linux/gcc13 捕获旧汇编，mangling 与 MinGW-w64 x64 ABI 不符；
  - `sum_span` 标号 `_Z8sum_spanNSt6spanIKiiEE` → 真实 `_Z8sum_spanSt4spanIKiLy18446744073709551615EE`（动态 extent，`size_t`=`unsigned long long`）；
  - `sum_ptr` 标号 `_Z7sum_ptrPKim`（`m`=unsigned long，Linux 约定）→ 真实 `_Z7sum_ptrPKiy`（`y`=unsigned long long，MinGW-w64 x64 的 `size_t` 实为 `unsigned long long`）；
  - caption `g++ 13.1` → `GCC 15.3.0`（红线③）。
- 修正后全量重跑：**0 真过期**；`consistency_check` 100/100。

**结论**：本书在 GCC 15.3.0 工具链下**实证完整、编译诚实、书内汇编新鲜、无版本漂移**；A/B/C/D 四条独立证据链相互印证。
