...
> 标准基: C++23 / GCC 13.1 / 预计阅读: 70min / [第152章　性能模型与测量学](../part14_perf/ch152_perf_model.md) [第154章　缓存优化与数据局部性（C++/硬件）](../part14_perf/ch154_cache_opt.md) / 难度: ★★★★☆｜层级：L3 专家
...
!!! note "类比：性能反模式清单 = 机场禁带名单"
    性能反模式清单可以**类比**为机场禁带物品名单——它告诉你「这里常被坑」，是嫌疑名单不是定罪判决；最终要用测量学定案，而不是见「按值传大对象」就慌。
    换个角度：反模式清单也**类似于**活文档——语言每加一个特性（string_view / move / concepts）既消灭一批旧坑也埋下新坑（协程的隐式堆分配），清单因此不断划掉又补上。

    > 失效边界：教条式 micro-optimization（见清单就改）和纯测量派（漏掉显而易见大头）都偏；反模式随标准演进，旧清单在新标准下可能已由语言层面消除（如 constexpr / string_view），照搬旧书单会优化错地方。

> **一句话结论**：性能反模式与陷阱：过早优化、测了噪声、忽视分配与拷贝、误信微基准——先测量再优化、避免凭感觉是铁律。

## ① 我们真正要回答的问题 <span class="badge badge-exp">经验</span>

[第152章　性能模型与测量学](../part14_perf/ch152_perf_model.md)
[第154章　缓存优化与数据局部性（C++/硬件）](../part14_perf/ch154_cache_opt.md)

性能反模式常被当成"背一份陷阱清单"的记忆任务，但**它真正的本质是一份"值得怀疑的嫌疑名单，不是定罪判决"**——它告诉你"这里常被坑"，但每个结论最终都要用测量学定案，否则就会陷入"教条式 micro-optimization"。反模式清单本身还是**活文档**：C++11 的 move 让"按值返回大对象"由慢变快，`constexpr`/`string_view`/`span` 又消除一批旧坑、而协程埋下隐式堆分配的新坑。本章不重复"过早优化是万恶之源"的口号，而要带着这六笔账往下读：

1. **"过早优化是万恶之源"到底在反对什么？反模式清单该怎么用？** Knuth 原意是"约 97% 的时间该忽略微小效率"，它反对的是两个极端：盲目优化与盲目不优化（`优化 50% 代码最多 2×、优化 95% 才到 20×`，Amdahl 限定必须只打热路径）。所以反模式是"嫌疑名单"而非"定罪"，修复只在热路径上做并用测量验证、别在冷路径上为修反模式付代码复杂度代价。本章 ⓪ 历史动机 + ⑰ FAQ（如何确认热路径、冷路径要不要修）给出判据。
2. **分配与拷贝这两类最经典的 C++ 反模式，本质是什么？** 不必要的堆分配（小数据也走 `vector`/`new`，应栈数组或 `std::array`）；隐式拷贝与临时对象（按值传大对象触发拷贝，改 `const&`）；`reserve` 缺失（`push_back` 末尾反复扩容 O(n)·log n 次重分配的代价）；移动语义未触发（该 `std::move` 却没显式 move，或元素不 `noexcept` movable 让扩容退回到安全移动路径）。本章 ② 堆分配、③ 隐式拷贝、⑪ reserve 缺失、⑫ 移动语义未触发逐条给出 ❌/✅ 对照与代价量化（malloc/free 约 50–100ns）。
3. **类型擦除与虚调用的代价到底在哪儿？标准把这些消灭了吗？** 虚函数间接调用走 vtable，`-O2` 下 GCC 会做「推测去虚拟化」（比对 vtable 槽位命中就直接内联、否则退回 `jmp rax`）——现代优化已经把"纯虚调用开销"大幅压缩，真正贵的是**分支误预测**叠加（虚调用 +5ns、波及误预测再 +15ns）。`std::function` 类型擦除每次调用 +10ns，改用模板零擦除开销。本章 ⑤ 虚函数（含 GCC 15.3 真实去虚拟化反汇编）、⑩ `std::function` 都给出量化与改写。
4. **与缓存、分支、I/O 相关的一类反模式，为什么"感觉不到慢"反而更危险？** `false sharing`（不同核写同一 cache line 互相踢，`alignas(64)` 隔离）、缓存不友好遍历（列优先跨行 vs 行优先连续内存，见附录 D5 的 2D 行/列优先基准）、异常在热路径、`std::endl` 每行 flush（每次 +1 syscall ≈1μs，但 I/O bound 场景测不出来）。这些反模式**取决于负载与微架构**——`endl` 在 STDERR（无缓冲）上就没有额外开销，必须测量对应场景。本章 ⑦ ⑧ ⑨ ⑭ 各节 + ⑯ FAQ 把"何时真慢/何时无所谓"讲清。
5. **微基准与"优化"本身的陷阱是什么？数字会骗人吗？** `std::regex` 每次调用都重新编译正则（应 `static const` 只在编译期构造一次）；过度模板实例化让翻译期代码膨胀；分支预测失败（`v[i]=i%2` 不排序则乱序触发误预测，约 15–20 周期）。所有量级数字（L1→L3 约 40 cycles、L3→RAM 约 200 cycles）都**强依赖具体 CPU/编译器/标志**，不可跨架构移植。本章 ⑨ ⑬ ⑭ + ⑲ 性能数据参考给出代价表，附录 D5 用真实基准兜底。
6. **判定该不该改的最终依据是什么？修复的黄金判据怎么落地？** 六条铁律：先测量再优化、`reserve` 再填、大对象传 `const&`、move 构造标 `noexcept`、昂贵对象 `static`、用 `'\n'` 而非 `std::endl`；诊断手法是 `perf record -g` 看 top 10、`perf c2c` 检测 false sharing。标准演进也在系统消灭反模式（P1144 trivially relocatable 让扩容变 memmove、P2300/`std::execution` 消异步开销）；Meyers 若今天重写，`string_view` 与 move 能单独占一章。本章 ⑱ 最佳实践 + ⑳ 源码阅读路线 + ⑮ 跨语言对比给出完整判据。
...
- **跨模块延伸**：[第160章 从零实现内存池（C++）](../part15_cases/ch160_mempool.md)）
...
**常见陷阱 / 最佳实践**：性能反模式的根因多为三类——① 隐式堆分配（小对象也 `new`）；② 缓存不友好（链表/指针追逐）；③ 分支不可预测（数据依赖分支）。对照 [ch153](../part14_perf/ch153_cpu_micro.md) 的微架构与 [ch156](../part14_perf/ch156_compiler_opt.md) 的优化器，先量后改（`-O2` + benchmark），不靠直觉。
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(a) 绝对耗时（随机器而变，仅作量级参考）">
  ...
  <text x="640" y="224.2" text-anchor="end" font-size="10.5" font-family="Georgia, serif" fill="#C44E52">基线 3.79ms</text>
  <rect x="188.0" y="228.2" width="64.0" height="71.8" fill="#9A9A9A"/>
  <text x="220.0" y="222.2" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#9A9A9A">3.79ms</text>
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">行优先（顺序访问，缓存友好）</text>
  <rect x="468.0" y="99.8" width="64.0" height="200.2" fill="#C44E52"/>
  <text x="500.0" y="93.8" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">41.15ms</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">列优先（跨步访问，缓存失效反模式）</text>
</svg>
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(b) 相对倍数（可移植信号：基准=1.00×）">
  ...
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">行优先（顺序访问，缓存友好）</text>
  <rect x="468.0" y="171.6" width="64.0" height="128.4" fill="#C44E52"/>
  <text x="500.0" y="165.6" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">10.84×</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">列优先（跨步访问，缓存失效反模式）</text>
</svg>
