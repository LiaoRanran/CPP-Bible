...
!!! note "类比：CPU 微架构 = 餐厅后厨"
    流水线 / 乱序执行可以**类比**为餐厅后厨——多道菜（指令）同时在不同工位（取指 / 译码 / 执行）推进，看似你点的菜一个接一个上，其实后厨在并行。分支预测更**好比**服务员猜你下一道点啥，猜对就提前备料，猜错（误预测）整桌重排。
    换个角度：C++ 内存模型也**类似于**厨房「哪些步骤必须按序」的硬规矩——你用 memory_order 声明哪些重排不可接受，其余交给硬件自由发挥。

    > 失效边界：微架构不是「免费并行」——分支误预测惩罚约 15~20 周期、false sharing 在不同芯片表现迥异（同段 C++ Intel 快 AMD 慢一倍）；Spectre 更证明微架构细节会泄漏数据，不能只当性能问题看。
...
CPU 微架构常被当成"研究 CPU 的硬件课"，但**它真正的本质是"理解源代码最深的认知陷阱之一是'顺序执行'这个大脑默认模型"**——你写的代码不是"一条接一条被执行"，而是先被拆成微指令，再被流水线、分支预测、乱序执行重新排列，很多"显然更聪明"的写法反而因喂不饱这个真实执行引擎而更慢。本章不重复"什么叫指令集"的入门句，而要带着这五笔账往下读：

1. **为什么"顺序执行"的直觉是性能最大的敌人？流水线怎么让代码"失真"？** 一条指令被拆成取指→译码→执行→访存→写回多阶段，不同指令的阶段可重叠；超标量（一周期发射多条）与乱序执行（Pentium Pro 起）进一步打破程序序，让吞吐远高于"一条跑完再跑下一条"，也让程序员脑中的顺序模型失真。本章 ⑤ 一条指令的微架构旅程 + ⑥ 执行单元与缓冲把流水线与 OoO 的吞吐机制讲清。
2. **分支预测怎么"消解"控制依赖？`[[likely]]`/`[[unlikely]]` 到底做了什么、不做什么？** 分支预测器先猜再执行，猜错就付**误预测惩罚**（现代 x86-64 约 15~20 周期）。`[[likely]]`/`[[unlikely]]`（C++20，`[dcl.attr.likelihood]`）**不消除分支**，只生成偏向热路径的基本块布局（热路径紧挨、冷路径跳走，减取指/`I-cache` 压力）；真正去分支要写无分支代码（位运算/`cmov`）。本章 ⑩ 汇编分析用 `-O2` 真实的 `cmov` 反汇编、⑬ 源码分析（`gcc/predict.c` 概率传播）讲清机制边界。
3. **重排序缓冲（ROB）怎么做到"乱序执行 + 按序退休"？语义为何仍看似顺序？** 指令按序进入 ROB、可乱序执行，结果暂存，仅当"前面所有指令都已安全退休"才提交；异常/分支误预测时，ROB 中程序序在后的指令被丢弃——这正是 OoO 能既并行又保语义正确（含异常）的关键。本章 ⑧ 生命周期图 + ⑨ OoO 重排示意（程序序 vs 执行序）把这一机制钉死。
4. **依赖链怎么限制 ILP？为什么"循环体够短"反而慢？** 即使核心每周期能发射多条指令，一条长依赖链也被其**关键路径延迟**钉死——`s = s + x` 的循环，每轮都等上一个 `s` 就绪，吞吐被钉在 1 加/周期；类比如 ⑦ 依赖链 vs 无关链（`s0/s1/s2` 分路可同周期发射到不同 ALU）。对策是拆链（多累加器）。本章 ⑦ 内存图 + ⑩ 汇编的串行 `add rax,[rdi]` 依赖、⑲ 实测对比（4 路累加通常快 2~4×）给证据。
5. **这些机制的实际代价与易错判据是什么？怎么测、怎么改才不踩坑？** 误区连串：盲目贴 `[[likely]]` 反而误导预测器；误以为 `[[likely]]` 消除分支；把 `-O0` 结果当真；裸 `rdtsc` 被乱序错位（需 `lfence`）、跨核 TSC 不同步不能直接比。判据是"先测后改"——数据不可预测且热用无分支，数据高度可预测时 `cmov` 反因两路都算更慢，**以测量为准**；绑核 + 关 SMT + 固定频率保证可复现。本章 ⑯ 易错点 + ⑰ FAQ + ⑱ 最佳实践（查表/拆链/数据可预测化）+ ⑲ 实测对比给出完整判据。
...
| 关联章节 | 场景 | 组合方式 |
|---|---|---|
| [第154章](../part14_perf/ch154_cache_opt.md) | 无锁队列/计数器 | 本章提供概念，第154章提供实现 |
| [第152章](../part14_perf/ch152_perf_model.md) | 向量化计算/图像处理 | 本章提供概念，第152章提供实现 |
| [第152章](../part14_perf/ch152_perf_model.md) | 数据处理管道/排行榜 | 本章提供概念，第152章提供实现 |
| [第154章](../part14_perf/ch154_cache_opt.md) | 计时器/性能测量 | 本章提供概念，第154章提供实现 |
...
> 交叉引用：性能模型见 [ch152](../part14_perf/ch152_perf_model.md)；基准见 [ch151](../part13_engineering/ch151_benchmark.md)。
