...
### 0.2 关键转折（编年）
`[史]` 这条脉络是"从定性到定量、从经验到可测量"的三步：1967 年 Amdahl 定律第一次用数学量化了"并行加速受限于串行部分"，逼着工程师先定位瓶颈再动手；2009 年 Berkeley 的 Williams、Patterson 等人提出 Roofline 模型，把"算力上限 vs 带宽上限"画成一张可读的图，让"卡在哪"变成一眼可判的事；此后"测量驱动"逐步替代"拍脑袋"成为性能工程共识——模型给方向，测量给距离，二者合起来才构成闭环。
...
0.2 编年止于 Roofline 与"测量驱动"成为共识，此后真正的变化是**模型从论文搬进了日常工具链**：Intel **Advisor** 与开源的 **llvm-mca / opt-viewer** 把 Roofline 与"每条指令吞吐/延迟"直接叠到源码上，开发者在 IDE 里就能看见"这段代码离算力天花板多远"；可观测性工具（Python `scalene`、Linux `perf` 的 `stat` / `record`）让"内存带宽受限还是算力受限"可被实测区分，正好呼应 0.3"模型框天花板、测量看现实距离"的双向立场。`<span class="badge badge-history">史</span>`

标准层面也给了锚点：WG21 推动的 **`std::hardware_*` 常量**（如 `hardware_destructive_interference_size`）把"缓存行/伪共享"这类模型参数以标准方式暴露，性能模型第一次有了可移植的语言级锚点——尽管标准只保证抽象机语义、不保证 cycles（㉒.4）。`<span class="badge badge-history">史</span>`

`<span class="badge badge-comment">评</span>` 现代性能工程把"先建模再测量"做成闭环：模型告诉你该往哪看，测量告诉你现实离天花板多远，二者缺一都是盲调。`<span class="badge badge-anecdote">轶</span>` 圈内笑谈：有人用 Amdahl 定律算出"并行化能加速 50×"，真上 64 核却只快 8×——因为串行那 2% 在真实负载里被放大成了 IO 等待。模型没撒谎，是输入画像不全。
...
本章是 part14 的方法论总纲，它站在五块前置知识的肩膀上，缺一块就建不起模型：**内存层次与 cache（ch154）**提供带宽/延迟的硬件来源（含 false sharing），**CPU 微架构（ch153）**决定单条指令的成本（流水线、乱序执行、分支预测），**编译器优化（ch156）**会改写你"以为"测到的代码（`-O2/-O3/LTO/PGO`），**Benchmark 方法论（ch151）**给出测试框架、Fixture 与统计报告，而 **SIMD（ch155）**直接改变算术强度——把每指令处理的数据量翻倍，等于把 Roofline 上的点往右推、可能从带宽斜坡一侧推到算力屋顶一侧。读下面之前，这几块概念要先就位。
...
模型建成之后要落到两处：**Compiler Explorer 实战（ch157）**把测量出的慢函数贴进 CE 直接看汇编，验证"模型预测"与"编译器实际生成"是否一致；**性能反模式（ch158）**则用本章模型去识别那些"以为快其实慢"的写法。至于 **SIMD（ch155）**与**编译器优化（ch156）**，它们是建模之后的**具体加速手段**——先知道瓶颈在算力还是带宽，才知道该不该上 SIMD。换句话说，本章给的是"往哪使劲"的判断力，后续章给的是"怎么使劲"的工具箱。
...
> **示例 4** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 内存图：带宽与延迟的硬件来源 <span class="badge badge-platform">平台</span>
...
> **示例 5** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 内存图：带宽与延迟的硬件来源 <span class="badge badge-platform">平台</span>
...
> **示例 6** <span class="badge badge-exp">难度 ★☆☆☆☆</span> · 生命周期图：一次测量的时间线 <span class="badge badge-impl">实现</span>
...
> **示例 19** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 面试题 <span class="badge badge-exp">经验</span>
...
> **示例 20** <span class="badge badge-exp">难度 ★☆☆☆☆</span> · 面试题 <span class="badge badge-exp">经验</span>
...
> **示例 21** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 易错点 <span class="badge badge-exp">经验</span>
...
> **示例 22** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 易错点 <span class="badge badge-exp">经验</span>
...
> **示例 23** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 易错点 <span class="badge badge-exp">经验</span>
...
**Q：`rdtsc` 和 `steady_clock` 哪个更准？**

两者量的不是同一个东西。`rdtsc` 读 CPU 的 TSC 周期计数器，分辨率可达亚纳秒（约 0.3ns 量级），但**单位是周期而非时间**——你得自己除以频率换算，而频率受 Turbo/变频影响，且多核间 TSC 未必同步（需 `rdtscp` 或绑核）。`steady_clock` 是 OS 提供的纳秒单调时钟，免换算且保证单调递增，代价是单次调用有成本（本机 D5 实测分辨率 100ns，⑨ 单次约 20–40ns `[微架构·x86-64][UNVERIFIED]`）。**工程首选 `steady_clock`**；只有需要 cycle 级精度、且能自行处理频率与绑核时，才用 `rdtsc`（附录 H 量化方法）。

**Q：单次结果能报吗？**

不能。单次读数同时被三类噪声污染：时钟量化（本机 100ns）、调度/OS 抖动、冷启动（页错误与 cache 冷）。至少要 warmup 后重复数十次，取**中位数**而非均值（中位数抗离群），并报告 MAD 或方差（示例 C21）。若被测区间极短（短于 100ns），还应把它放进循环测总量再除以次数，把计时开销摊薄（易错点 C17）。

**Q：为什么 `-O3` 有时比 `-O2` 慢？**

因为 `-O3` 的激进优化各自都有代价：过度循环展开会胀 I-cache（取指变慢）、提高寄存器压力可能触发溢出（spill）、自动向量化在对齐不佳或别名不明时反而要加运行时检查与兜底路径。这些代价在特定代码上可能超过收益。所以**优化级别不是越高越好，必须实测**——这也正是 ⑱ 强调"用与目标一致的优化级别测量"的原因（⟶ ch156 逐项看各优化标志的实际影响）。

> **示例 24** <span class="badge badge-exp">难度 ★★☆☆☆</span> · <span class="badge badge-exp">经验</span>
...
> **示例 25** <span class="badge badge-exp">难度 ★★★☆☆</span> · <span class="badge badge-exp">经验</span>
...
下面五条不是并列的"注意事项"，而是一条完整测量链的五个环节，顺序不能颠倒：

**先建模，再动手**（对应原第 5 条，但它是逻辑上的第 1 步）。用 Roofline/Amdahl 明确瓶颈在算力还是带宽（或串行占比多少），否则优化可能全落在非瓶颈部分——Amdahl 早已说明：改进非瓶颈部分不会转化为整机加速。

**用与目标一致的优化级别测量**。`-O2/-O3` 必须与发布配置相同，并固定在安静环境（关 turbo/降频干扰可选）——`-O0` 的"慢"不代表发布版慢（易错点 C18）。

**warmup 后再采样**，剔除冷启动：页错误、cache 冷、动态频率爬升都会污染前若干次读数（易错点 C16）。

**用 `volatile`/`black_box` 接住结果**防 DCE——否则 `-O2` 可能把整个被测循环删掉，测出 0ns（⑩ 有真实反汇编证据）。

**多次重复取中位数 + MAD**，报告方差而非单次：中位数抗调度抖动（离群的 200 被压住，示例 C21），MAD 量化稳定性。"warmup → 防 DCE → 多次取中位"三者缺一，数字就不可复现。
...
> **示例 28** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 性能分析：从模型到数字 <span class="badge badge-exp">经验</span>
...
> **示例 29** <span class="badge badge-exp">难度 ★☆☆☆☆</span> · 性能分析：从模型到数字 <span class="badge badge-exp">经验</span>
...
> **示例 30** <span class="badge badge-exp">难度 ★☆☆☆☆</span> · 性能分析：从模型到数字 <span class="badge badge-exp">经验</span>
...
> **示例 31** <span class="badge badge-exp">难度 ★☆☆☆☆</span> · 性能分析：从模型到数字 <span class="badge badge-exp">经验</span>
...
> **示例 32** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 性能分析：从模型到数字 <span class="badge badge-exp">经验</span>
...
> **示例 33** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 性能分析：从模型到数字 <span class="badge badge-exp">经验</span>
...
> **示例 34** <span class="badge badge-exp">难度 ★★☆☆☆</span> · 跨语言对比：基准与剖析生态 <span class="badge badge-std">标准</span>
...
> **示例 35** <span class="badge badge-exp">难度 ★☆☆☆☆</span> · 跨语言对比：基准与剖析生态 <span class="badge badge-std">标准</span>
...
### ㉒.3 生产踩坑：建模与测量的误用
这四条其实是同一个错误的两面：**模型告诉你"上限在哪"，但不会告诉你"你现在在哪"**——把二者混为一谈就会踩坑。

**用错模型（把访存密集当计算密集）**。最典型的是狂加 SIMD 却卡在 cache miss：SIMD 抬高的是算力侧，而瓶颈若压在带宽斜坡上，加算力等于把屋顶抬高却仍被斜坡压着（Roofline）。正确顺序是先画 Roofline 定位落在哪条线下，再决定攻算力还是攻带宽。

**只看平均值不看分位**。均值好看而 p99 爆炸，用户体感就是"偶发卡顿"——因为尾延迟来自偶发的缺页、GC、锁竞争或 NUMA 远端访问，这些事件在均值里被摊平了。服务端必须报分位数（⑫）。

**把单次测量当结论**。未去冷启动、未多次取中位数，结果被噪声主导（第151章 ⑥⑬）；本机 D5 实测分辨率 100ns（附录 H 量化方法），短区间的单次读数基本就是量化噪声。

**混淆加速比与绝对值**。绝对值（毫秒）随机器、负载、编译器版本而变，只有"相对加速比"才可能跨机器复现（第151章 D5）。所以报数必须带上基线对比，否则数字既不可比、也无法做回归门禁。
...
与其罗列"谁用了什么工具"，不如看清这些项目**为什么**需要性能模型——它们面对的问题不同，选的建模手段也随之分野。按"模型回答的是哪个问题"分三类：

**第一类：给编译器/库自己用的代价模型（决定"生成什么代码"）**。编译器无法实测每个候选方案，只能在编译期用解析模型预估代价：LLVM 的 cost model 判断某个循环该不该向量化、展开因子取多少，Eigen 用同类 cost 模型选展开因子，Blink 用性能模型指导合成。这类模型的硬约束是"快且保守"——它嵌在编译流程里，宁可估错也不能拖慢编译，所以只用算术强度、寄存器压力这类可静态计算的指标，而不会真的去跑计时。

**第二类：量化"这版改动快了还是慢了"的回归基准（决定"能不能合入"）**。这类项目把基准做成 CI 门禁：Google Benchmark 是 C++ 微基准的事实标准（自动 warmup、统计、置信区间），folly 用 benchmark 库量化吞吐，fmt 与 spdlog 用基准量化格式化与异步日志开销，RocksDB 用 `db_bench` 量化读放大，ClickHouse 用 `EXPLAIN PIPELINE` 估算执行成本，Redis 用 `redis-benchmark` 建模 QPS，gRPC 用 benchmark 建模 RPC 时延，DPDK 用 `testpmd` 建模包转发速率。它们共同的方法学纪律正是本章 ⑯/⑱ 那套——warmup、防 DCE、多次取中位数、报相对加速比，因为绝对毫秒在 CI 机器之间根本不可比。

**第三类：在线采集与端到端剖析（决定"线上哪里慢"）**。Chromium 用 perfetto 采集性能时间线，Unreal 用 stat 命令监控帧时间，WebKit 的 WTF 与 Mozilla 的 SpiderMonkey 用基准量化分配与 GC 成本，V8 用 d8 基准建模 JS 性能；时钟底座则由 Boost.Chrono 与 Qt 的 `QElapsedTimer` 这类高精度计时设施提供。这类关注的是"真实负载下的尾延迟与帧预算"，所以报分位数（p99）而非均值（⑫）。

三类并不互斥：成熟项目往往同时具备编译期 cost model（选代码）+ CI 基准（防回归）+ 线上剖析（找热点）。要上哪一套，取决于你问的是"生成什么""能不能合入"还是"线上哪里慢"——这正是 ㉒.2 说的"先知道上限再动手"。
...
`[微架构·x86-64][UNVERIFIED]` 延迟层级是 Roofline 那条"斜坡"的物理来源：L1 约 1ns（64 字节行）、L2 约 4ns、L3 约 12ns、主存约 100ns（与 ⑦ 同数量级）。把它放进 Roofline 看就明白了：一次主存访问的约 100ns 里，一个 3GHz 核本可执行约 300 条指令——所以"少访存一次"往往比"少算几次"值钱得多，这正是低算术强度内核该优先改数据布局（ch154）而不是加算力的根因。

带宽与算力天花板同样只给量级：DRAM 数十 GB/s（本章正文与 ⑲ 的 axpy 实测以约 50 GB/s 作对比基准），AVX2 单核算力约 256 GFLOP/s（示意值，随 CPU 代际大幅变化）。**真正可比的是二者的比值而非绝对值**——机器平衡点 `balance = 峰值算力 / 峰值带宽`，算术强度低于它时落在 Roofline 左侧（带宽受限），高于它才撞上算力屋顶（㉒.1）。所以"低于 8 FLOP/字节 即带宽受限"这类判据只对给定 `balance` 成立，换台机器就要重算（练习 5 的红线）。

### 量化方法（与本章 D5 实测对齐）

`[微架构·x86-64][UNVERIFIED]` 时钟分辨率是这一层最容易写错的数：**本机（Windows / MinGW）D5.1 实测 `steady_clock` 分辨率为 100 ns**（QueryPerformanceCounter 量化到系统时钟粒度），而非某些资料笼统给出的 1ns；Linux 下经 vDSO 的 `clock_gettime(CLOCK_MONOTONIC)` 分辨率可达 ns 级，但单次调用仍有约 20–40ns 成本（⑨）。`rdtsc` 直接读 TSC，约 0.3ns 量级，代价是要自己换算频率，且受变频/Turbo 与多核 TSC 同步影响（⑰）。

`[微架构·x86-64][UNVERIFIED]` perf 采样约 0.2us/事件，cache-miss 计数经 MSR 读出；单次测量抖动约 5ns 量级。这里的关键区分是：抖动虽小，**分辨率才是短区间的真正敌人**——本机 100ns 的量化粒度下，任何短于 100ns 的差异在单次读数中不可分辨，必须靠多次重复取统计（D5.2 铁律）。取 4096 次中位数是常见经验做法，目的正是把量化误差与调度噪声压到可忽略。

### 编译器与标准

生成侧：GCC 15.3.0 / Clang 19 在 `-O3 -mavx2` 下产出上示向量化代码；`__builtin_expect` 用于指导分支布局（ch153）。语言侧：`__cplusplus` 在 C++23 下为 `202302L`；C++20 的 `std::span` 作为零拷贝视图，能在不复制数据的前提下传递连续区间，从**减少访存量**的角度直接抬高算术强度——这是语言设施改变 Roofline 落点位置的直接例子。
...
## 附录 D5：真实基准与性能分析 — 性能测量学：steady_clock 分辨率与计时开销（GCC 15.3.0）

> 测试环境：AMD Ryzen 9 7940HX（16C/32T）；本机 MinGW-W64 GCC 15.3.0；`g++ -O2 -std=c++20`；`std::chrono::steady_clock`。本附录目的：用主控实测锁死本机时钟的**分辨率**与**单次计时对开销**，为全书 D5 的方法学基线（为何取中位、为何用 volatile sink、为何报加速比）提供量化依据。**绝对毫秒随机器而变，加速比才是可移植信号。**
...
| 单次计时对开销（起点 `now()` + 终点 `now()`） | 100.4 ns |
