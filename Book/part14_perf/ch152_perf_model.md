...
> 测试环境：AMD Ryzen 9 7940HX（16C/32T）；本机 MinGW-W64 GCC 15.3.0；`g++ -O2 -std=c++20`；`std::chrono::steady_clock`。本附录目的：用主控实测锁死本机时钟的**分辨率**与**单次计时对开销**，为全书 D5 的方法学基线（为何取中位、为何用 volatile sink、为何报加速比）提供量化依据。
