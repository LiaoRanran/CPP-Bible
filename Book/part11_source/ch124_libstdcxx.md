...
### 0.1 起源（谁·何时·为何）
GCC 由 Richard Stallman 于 1987 年发起（最初只是 GNU C Compiler）[史]，但早期它几乎没有现代意义上的 C++ 标准库：先是有一段松散的 `libg++` 时期，代码大量以预标准 C++ 写成，连异常、命名空间、模板的标准语义都还在漂移。真正的痛点很清楚——1998 年 ISO C++98 标准正式发布后 [史]，旧的 C++ 运行时根本无法干净地实现"标准规定的那套容器与算法"，而且它还与 GPL、与 GCC 内部耦合得死死的，难以独立演进。于是社区需要一次彻底重写，而非打补丁。

### 0.2 关键转折（编年）
- 1987：GCC 问世，C++ 前端随后加入 [史]。
- 1998：C++98 标准发布，旧实现暴露出结构性落后 [史]。
- 2001：GCC 3.0 伴随 **libstdc++-v3**（第三代、也是我们今天用的版本）整体重写登场，头文件搬进 `bits/`，模板实现与链接期符号彻底分离 [史]。这是 libstdc++ 定型的里程碑。
...
- **跨模块延伸（part07 STL）**：[第85章　unordered_map / unordered_set：哈希开链集合](Book/part07_stl/ch85_unordered.md)—— STL 哈希容器开链实现的源码落点
- **跨模块延伸（part07 STL）**：[第87章　bitset：编译期定长位集](Book/part07_stl/ch87_bitset.md)—— 编译期定长位集的源码落点
- **跨模块延伸（part10 现代）**：[第123章　Compile-Time 编程范式总览](Book/part10_modern/ch123_ct_programming.md)—— 编译期编程范式是阅读 STL 源码的元编程底座
- **跨模块延伸（part10 现代）**：[第122章　PMR 与多态分配器](Book/part10_modern/ch122_pmr.md)—— PMR 多态分配器是 STL 容器内存后端的现代实现
