...
!!! note "类比：libc++ = 为新派厨房重写的菜谱"
    `libc++` 可以**类比**为 Clang/LLVM 家另开的一家厨房，同样照 C++ 标准做菜。它与 libstdc++ 更**好比**两种「不同流派的同菜系餐厅」——菜名相同，厨艺取向不同（libc++ 更积极采用 C++11 起的新特性）。

    > 失效边界：libc++ 与 libstdc++ 二进制不兼容；把两边都编进同一可执行文件会触发 ODR 违规，链接期或运行期崩溃。
