...
- **量化金融/交易系统**：头部券商与对冲基金的低延迟交易系统（撮合、风控、行情分发）几乎以 C++ 为底座，对内存布局、无锁队列与缓存亲和度极致敏感——这正是 ch165"4 个从零项目"里"高频/系统层"战场的真实雇主，岗位 JD 普遍要求 concrete 的并发与内存基础而非特性背诵。
- **自动驾驶/机器人**：自动驾驶栈（感知-决策-控制）与机器人中间件 ROS 2 的 C++ 客户端库 `rclcpp` 以 C++ 为主，岗位要求 concrete 的 RAII/ownership/实时基础——路线图"先打 RAII/ownership 底、再按需追特性"的取向在此最被验证，也是 C++ 几乎不可替代的战场之一。
