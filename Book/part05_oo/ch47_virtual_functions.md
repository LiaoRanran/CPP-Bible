...
- **音频插件生态（VST3 / Steinberg）**：VST3 SDK 用纯虚接口（如 `IAudioProcessor::processBlock`）让宿主加载第三方音频插件，虚函数表是插件 ABI 稳定的基石——「虚函数即二进制稳定扩展点」的工业主场。
- **科学计算 / 有限元（deal.II）**：有限元库 deal.II 用虚函数表达「可替换的有限元/求解器/网格细化策略」，是科学计算里虚函数承载可扩展算法的案例。
