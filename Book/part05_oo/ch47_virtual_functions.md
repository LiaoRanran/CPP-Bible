...
`virtual` 是大多数 C++ 程序员"早就会用、却说不清它到底让编译器替你做了什么"的关键字。本章不重复"虚函数能让子类覆盖"这种入门句，而要把这五笔账一次算清：

1. **一次虚调用，CPU 到底多付出了什么？** 不是"一点性能损耗"就能带过的。它多解了一次 `vptr → vtable → 函数指针` 的间接寻址，这在你没开去虚化时**每次调用都发生**。本章 ⑩ 用 MinGW GCC 15.3 反汇编给你看一次虚调用的全部指令与两次取值（load vptr、load slot），让你对"虚 vs 非虚"的差异有第一手的量级感。
2. **为什么"基类析构必须 virtual"几乎是一条铁律？** 本质是**对象生命周期**问题：当基类指针指向派生类对象并 `delete p` 时，若析构不虚，走的将是静态类型（基类）的析构——派生类的资源永不被释放，这是 UB 且静默。本章 ⑧ 生命周期 + ⑩ 汇编会把它讲透。
3. **多重继承下一次基类指针怎么"修"才能指对派生类？** `this` 在不同基类子对象间需要偏移调整，编译器生成 **thunk（跳板函数）**来做修饰；虚继承还要再引入 `vbptr` 一层。这是 Itanium/MSVC ABI 差异最集中的地方，也是 ⑰ FAQ 与 ⑫ 工业案例反复出现的坑。
4. **动态分派 vs 静态分派（CRTP），差距到底多大？** 本章 ⑲ 用 microbenchmark 量化；关键是**去虚化**（`final` + PGO / LTO）如何把"虚"重新变成"不虚"。别只听"C++ 虚调用很慢"的传言，要看数据和触发去虚化的条件。
5. **libstdc++/libc++/MS STL 的 vtable 为什么长得不一样？** 这不是实现细节洁癖，而是 ABI 契约——同一份源码跨编译器链接时，vtable 布局不一致会直接崩。本章 ⑬ 源码分析对照 Itanium 与 MSVC 两套 vtable 的真实差异。

带着这几笔账往下读，每一节都会回到它们：⑦ 的 ASCII 内存图/对象布局给你 vptr 到底插在哪，⑮ 面试题把这五笔账转成高频追问，⑱ 最佳实践告诉你哪些场景该躲开动态分派。
...
[第 45 章　C++ 面向对象总览与对象模型基础](../part05_oo/ch45_oop_object_model.md)（对象模型基础）—— vtable 指针即虚函数的对象模型落点
[第76章　STL 架构与迭代器概念](../part07_stl/ch76_stl_arch.md)（STL 架构与迭代器概念）—— STL 抽象基类大量用虚接口
...
| 关联章节 | 场景 | 组合方式 |
|---|---|---|
| [第48章](../part05_oo/ch48_rtti.md) | 键值查找/缓存 | 本章提供概念，第48章提供实现 |
| [第48章](../part05_oo/ch48_rtti.md) | 独占所有权/工厂模式 | 本章提供概念，第48章提供实现 |
...
- **同模块接续**：[第48章 RTTI 与 typeid/dynamic_cast：运行时类型查询](../part05_oo/ch48_rtti.md)—— dynamic_cast/typeid 依赖虚函数表的存在
- **同模块接续**：[第49章 虚继承与菱形继承：共享虚基类](../part05_oo/ch49_virtual_inheritance.md)—— 虚继承下的虚函数调用涉及虚基类 this 调整
- **同模块接续**：[第50章　多重继承与对象模型（Multiple Inheritance）](../part05_oo/ch50_multiple_inheritance.md)）—— 多重继承的虚函数调用可能存在二义性，需显式消歧
