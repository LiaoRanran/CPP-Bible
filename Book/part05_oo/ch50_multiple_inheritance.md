...
> 汇编证据：MinGW GCC 13.1.0，`-std=c++23 -O2 -S -masm=intel` 真实输出（见 `Examples/_asm_mi.cpp` → `_asm_mi.asm`）
...
```
x64 / Itanium ABI / GCC 13.1.0，struct D : B1, B2 { int x=1; }
...
> 以下为 `struct D : B1, B2 { int x=1; }`（x64 / Itanium ABI / GCC 13.1.0）`D` 对象 vtable 的符号与一段 `dynamic_cast<B2*>(d)` 生成的 this 调整 thunk 反汇编，用于把 `D*` 偏移到 `B2` 子对象：
...
_ZTV1D:                                 ; D 的完整 vtable（_ZTV = vtable符号）
  ...
  .quad 0x0000000000000010              ; B2 次 vtable 的 top_offset = +16
  .quad _ZTI1D
