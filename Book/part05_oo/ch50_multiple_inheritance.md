...
多重继承在不少 C++ 教程里被一句"尽量别用"打发，但它是理解对象模型与 ABI 的试金石。本章不教"什么时候忍痛用 MI"，而要把这五笔账算清：

1. **多继承和单继承在对象布局上的本质差别是什么？** 单继承的对象只有一个 vptr；多继承里**每一层非首基类都会带一具独立的 vptr**，对象里藏着多个 vtable 指针。这正是"一个对象有多个『我』"的物理真相——本章 ⑦ 的 ASCII 布局图直接把这个多 vptr 结构画出来。
2. **`this` 为什么会"跑偏"，跑偏后又怎么"修"回来？** 当基类指针要把地址指到派生类完整对象时，需要做**偏移调整**；编译器生成 **thunk（跳板函数）**来做这件事。这段汇编你看懂了，就彻底不再怕"多继承指针比较诡异"的传闻。本章 ⑩ 给出 thunk 的真实指令形态，⑮ 面试题常考它。
3. **能不能从一段 vtable 二进制反推出对象模型？** 能。vtable 里的 `top_offset`（指向对象首）、`typeinfo` 槽、`thunk` 槽共同编码了类型信息。能读懂这张表，等于拿到读懂任意库 vtable 的钥匙。本章 ⑬ 源码分析带你逐槽拆解。
4. **名字冲突、菱形歧义，多继承里到底怎么裁决？** 两个基类各有一个同名成员时是**硬冲突**（连虚继承都救不了）；菱形继承的真正解法在 ch49 的虚继承。先分清"冲突"和"歧义"两个不同的问题，Linux 内核风格的多接口混入才用得心安。
5. **什么时候该用多继承，什么时候该用组合/CRTP？** 默认纪律是**能用组合就不用多继承**，能用 CRTP（静态多态，ch51）就避开运行时 this 调整。本章 ⑱ 给你一条可执行的选型判据，而不是一句"尽量别用"。

带着这几笔账往下读，每一节都会回到它们：⑰ FAQ 把多继承的经典疑问一网打尽，⑲ 性能分析量化多 vptr + thunk 的真实成本，⑯ 易错点提醒你名字隐藏与构造顺序的坑。
...
| 关联章节 | 场景 | 组合方式 |
|---|---|---|
| [第49章](../part05_oo/ch49_virtual_inheritance.md) | 泛型库/编译期计算 | 本章提供概念，第49章提供实现 |
| [第49章](../part05_oo/ch49_virtual_inheritance.md) | 性能基准/回归检测 | 本章提供概念，第49章提供实现 |
| [第51章](../part05_oo/ch51_crtp.md) | 内存管理/PMR定制 | 本章提供概念，第51章提供实现 |
| [第45章](../part05_oo/ch45_oop_object_model.md) | 静态多态/编译期接口 | 本章提供概念，第45章提供实现 |
...
- **同模块接续**：[第 45 章　C++ 面向对象总览与对象模型基础](../part05_oo/ch45_oop_object_model.md)—— 多重继承对象含多个基类子对象，布局直观
- **同模块接续**：[第 46 章　封装与继承深度：访问控制、三种继承、切片、构造/析构、名字隐藏、override/final、NVI](../part05_oo/ch46_encapsulation_inheritance.md)—— 多重继承是封装/继承的进阶形态
- **同模块接续**：[第47章 虚函数与虚表（vtable）：动态多态的发动机](../part05_oo/ch47_virtual_functions.md)：动态多态的发动机）—— 多重继承的虚函数调用可能二义，需显式限定
- **同模块接续**：[第48章 RTTI 与 typeid/dynamic_cast：运行时类型查询](../part05_oo/ch48_rtti.md)—— 多重继承下 dynamic_cast 跨分支依赖虚基类
- **同模块接续**：[第49章 虚继承与菱形继承：共享虚基类](../part05_oo/ch49_virtual_inheritance.md)—— 菱形继承=虚继承+多重继承
- **同模块接续**：[第52章　空基类优化 EBO（Empty Base Optimization）](../part05_oo/ch52_ebo.md)）—— EBO 在多重继承基类中仍有布局收益
