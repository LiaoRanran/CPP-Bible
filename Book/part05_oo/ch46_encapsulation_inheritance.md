...
- **企业 C++ 框架（POCO C++ Libraries）**：POCO 用接口继承（如 `Poco::Runnable`、`Channel`）表达可替换组件，public 继承承载框架契约——「继承即框架成员资格」的工业案例。
- **游戏引擎（id Tech / DOOM Eternal）**：id Software 的现代引擎用 entity-component（组合）而非深继承树表达游戏对象，印证第 ⑰ 节「组合优于继承」在大型游戏团队里的落地。
