...
!!! note "类比：封装与继承 = 从 Simula 借来的两把钥匙"
    封装与继承可以**类比**为「两把从 Simula 借来的钥匙」——封装解决「保护好内部」（把能碰 / 不能碰写进类型系统），继承解决「复用与特化」（把 is-a 沿层次传下去）。封装更**好比**给模块上锁：private 是编译期契约，不是运行期铁墙，friend / 反射仍能越界。
    换个角度：「组合优于继承」在工程上反复被验证，也**类似于**与其「生来就是某种子类」（继承、脆弱基类），不如「装上某种能力」（组合、可替换）——层次一深，继承就成牵一发而动全身的蛛网。

    > 失效边界：切片（slicing）是值语义继承的永恒陷阱——派生按值赋给基类变量会切掉派生部分，标准从不替你拦；移动语义与继承交互还会悄悄退化为拷贝；override / final 只是工程纪律的「编译器帮盯」，而非语言强制封装。
...
- L1 缓存行 `64` 字节；false sharing 使跨核写放大到 ≈ 100ns
...
- `[core:C.127]`（T3）C++ Core Guidelines 规则 C.127 —— 本地 `docs/references/external/vendor/CppCoreGuidelines/CppCoreGuidelines.md`
- `[book:effective-cpp:item32]`（T4）Effective C++（Meyers，55 条） —— 提取文本 `docs/references/external/books/effective-cpp.txt`
