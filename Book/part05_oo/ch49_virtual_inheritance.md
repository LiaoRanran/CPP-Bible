...
- **标准库 `iostream` 体系**：`std::ios` 被 `istream`/`ostream` 虚拟继承，再汇合到 `iostream`——这是 C++ 标准库内部使用虚继承解决菱形、保证 `cin`/`cout` 只有一份 `ios` 状态的经典实例（第 ⑫ 节）。
- **大型框架的接口继承**：Unreal/Qt 中部分「被多接口同时继承的基类」会用虚继承避免重复子对象，但使用面很窄，仅限确有共享基类的场景。
- **CORBA / COM 风格的接口聚合**：早期分布式对象框架的「多接口继承同一根（如 `IUnknown`）」常用虚继承保证根唯一，是菱形问题的真实工业来源。
- **GUI 控件树**：某些 MFC/旧框架里「控件同时是窗口 + 是控件 + 是某基类」的菱形，曾靠虚继承消解，但现代实践多改为组合（ch46）。
- **通信框架（ACE — Adaptive Communication Environment）**：ACE 在 reactor/service configurator 等体系里用虚继承处理「被多接口共享的基类」，是真实大量使用虚继承的企业级 C++ 框架（尽管现代推荐组合）。
- **GUI 工具包（wxWidgets）**：wxWidgets 的部分控件体系让某基类被多接口虚继承以保证根唯一，是桌面 GUI 里虚继承仍有落地的工业案例（与 MFC 同源思路）。
