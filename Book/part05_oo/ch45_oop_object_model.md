...
- **JavaScript 引擎（V8 / hidden class）**：V8 用「hidden class / map」在 C++ 之上构建二级对象模型，对象属性访问靠自身维护的 map 而非 C++ 虚表——这是「在 C++ 对象模型之外自建对象模型」的工业标杆（V8 本体即 C++ 编写）。
- **SQLite / 存储引擎页布局**：SQLite 的 B-tree 页内记录（record format）与 WAL 帧的字节布局，依赖对「字节偏移 + 对齐 + 序列化」的精确控制，与 C++ 标准布局类的 `offsetof` 思想遥相呼应——存储格式的「对象模型」。
