...
> "集成地狱"曾是每个发布日的噩梦——直到有人决定：别等到发布日才集成。

### 0.1 起源（谁·何时·为何）
早期团队习惯"各自闷头开发数周，发布前一次性合并"，结果冲突堆积如山、集成以天计。`[史]` 1991 年 Grady Booch 在《Object-Oriented Design with Applications》中正式提出"持续集成（continuous integration）"概念；1990 年代末，Kent Beck 的极限编程（XP）把它推成日常实践——开发者频繁（甚至每天）合入主干，并立即构建、测试。痛点很直白：合并越晚，冲突越贵。

### 0.2 关键转折（编年）
- 2001：ThoughtWorks 发布 CruiseControl，第一批自动化 CI 服务器；`[史]`
- 2005/2011：Hudson 诞生、后因商标争议于 2011 年改名 Jenkins，成为开源 CI 事实标准；`[史]`
- 2011 / 2019：Travis CI、GitHub Actions 先后把 CI 搬进"配置文件即代码"的云时代。`[史]`

### 0.3 设计哲学之争
CI 的核心理念是"让集成失败尽早、自动化、可重复"，反对"靠人肉跑一遍编译"的侥幸。`[评]` 对 C++ 这类编译重、链接慢、平台耦合强的语言，CI 的价值被放大：本机能编过不代表 CI 矩阵（多编译器 / 多平台 / 多警告等级）能过。把"能编译、零警告、测试通过"固化成门禁，正是把质量从个人自觉变成系统保证。
...
!!! note "类比：CI 流水线 = 出厂自动质检线"
    CI 流水线可以**类比**为工厂的自动质检线——每辆车（提交）下线都过一遍检测，不合格不许出厂；本机编过只是「我这儿能跑」，CI 矩阵（多编译器 / 多平台 / 多警告等级）才是「到处都能跑」。它更**好比**红绿灯替代交警指挥——个人遵守靠自觉会漏，规则固化进机器就零遗漏。
    换个角度：-Werror 与 Sanitizer 进门禁，也**类似于**测速摄像头——不是要罚你，而是把「能编过」窄化成「真合格才能合入」，把质量从个人自觉变成系统强制。

    > 失效边界：CI 绿 ≠ 没 bug——它只验证「配置里写了的检查」（能构建、零警告、测试通过），验证不了设计正确性与需求真伪；缓存配错会命中旧产物让「绿色」持续撒谎（文中轶事），故缓存正确性核验仍是红线。
