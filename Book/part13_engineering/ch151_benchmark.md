...
> "快"不是一个形容词，而是一个需要被反复测量的数字——否则你测到的，往往是优化器的幻觉。

### 0.1 起源（谁·何时·为何）
性能度量最早靠"跑一遍看表"的土法，直到它变得不可信。`[史]` 1982 年 UNIX 引入 `prof`，随后 GNU `gprof` 给出函数级耗时火焰；但采样粗糙、无法看清微架构层面的瓶颈。更深的痛点是：手写的微基准常常被优化器整段删掉（死代码消除），或受缓存预热、时钟分辨率污染——你以为在测算法，其实在测噪声。硬件性能计数器（perf，2009 年并入 Linux 内核）才让"到底花在哪"第一次变得可观测。`[史]`

### 0.2 关键转折（编年）
- 1982：UNIX `prof` / GNU `gprof` 普及函数级 profiling；`[史]`
- 2009：Linux `perf` 把 CPU 硬件计数器（cache miss、分支预测失误）交给普通开发者；`[史]`
- 2015：Google 开源 Benchmark 库，带来统计稳健的微基准框架。`[史]`

### 0.3 设计哲学之争
基准之争本质是"信不信直觉"。`[评]` 一派主张"先测再优化"，用统计方法（多次采样、剔除离群）对抗噪声；另一派止步于"看起来应该快"。C++ 尤其危险：编译器会重排、内联、消除你的代码，未经严谨度量的"优化"常常负收益。本章立场正是——没有可复现数字的优化，只是猜测。
...
!!! note "类比：微基准 = 称体重"
    微基准可以**类比**为称体重——刚吃完饭（缓存预热）、站在斜坡（噪声）、秤没校准（时钟分辨率）都让数字骗人；要固定环境、多次取稳才有可比数字。优化器删掉你以为在测的代码，更**好比**评委偷偷撕了你的考卷——你以为在测算法，其实整段被死代码消除，跑的是空循环。
    换个角度：持续基准（continuous benchmarking）把数字接进 CI 长期盯防，也**类似于**体检档案——单个数字不可信，一串可对比的数字才暴露趋势。

    > 失效边界：一个基准数字不可信，一串可对比的数字才可信；虚拟机 / 后台备份等看不见的噪声源能让数字差 30%；跨平台 / 跨编译器数字不可直接比（微架构差异），且基准只能度量「开销」，度量不了「是否真的需要这段优化」。
...
- `[book:optimized-cpp:ch3]`（T4）Optimized C++（Guntheroth） · ch3 —— 提取文本 `docs/references/external/books/optimized-cpp.txt`
- `[cppcon:<benchmark>]`（T6）复现/案例源 —— 在线
