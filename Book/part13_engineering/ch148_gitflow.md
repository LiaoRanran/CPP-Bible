...
!!! note "类比：Git = 每人一份完整档案柜"
    Git 的分布式可以**类比**为每个人的完整档案柜副本——离线也能工作、丢失一台不影响全局；SVN 像只有中央一台保险柜，柜子一锁全队停摆。长期分支更**好比**借出去很久的书——拖越久，归还时和原书架差得越多，合回（merge）越痛。
    换个角度：对 C++ 这种构建慢、耦合强的语言，长命分支的合并代价也**类似于**大装修后的重新接线——一次大合并常伴漫长重编与 ABI 对账。

    > 失效边界：分布式 ≠ 无冲突——Git 只是把冲突从「中央锁死」变成「合并时显式解决」，仍要人来处理；trunk-based 要求小批量高频合入，若团队没有强 CI 兜底，主干会被频繁打断；重型 Git Flow 对按分钟发布的微服务是过度工程，分支策略必须匹配发布节奏与团队规模。
