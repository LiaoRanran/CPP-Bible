...
> 把缺陷消灭在合入之前，是软件工程里"性价比最高的一道闸门"。

### 0.1 起源（谁·何时·为何）
代码审查的雏形可追溯到大厂对"缺陷越晚发现越贵"的痛感。`[史]` 1976 年，IBM 的 Michael Fagan 正式提出"Fagan 检查法"——一套结构化的、以会议形式逐行走查代码的流程，把 review 从"随便看看"变成可度量、可培训的工程活动。彼时一次发布动辄数月，缺陷流到客户手里代价惊人，团队急需一道"合入前的质检"。

### 0.2 关键转折（编年）
- 1976：Fagan 发表结构化代码检查法；`[史]`
- 2008：Google 内部工程实践将"所有变更必须经至少一人 CR 才能合入"制度化，成为业界范本；`[史]`
- 2008–2011：Gerrit、GitHub Pull Request 把重审查变成轻量、异步、在线的工具流。`[史]`

### 0.3 设计哲学之争
重型 Fagan 检查（全员会议、逐行念稿）与轻量工具化 CR 之争，本质是"投入强度 vs 频率"的权衡。`[评]` 现代 C++ 团队普遍选择后者：用工具做格式 / 静态检查的前置过滤，把人的注意力留给逻辑、生命周期与别名这类只有人能判断的缺陷——尤其 C++ 的 UB 往往编译器不报，正是最该被 eyes 拦下的类别。
...
!!! note "类比：代码审查 = 出版前的编辑"
    代码审查可以**类比**为出版前的编辑——作者对自己代码的盲点视而不见，第二双眼睛才能拦下错别字与逻辑硬伤；尤其 C++ 的 UB 编译器不报，正是最该被人眼拦下的类别。它更**好比**机场安检——机器（clang-tidy / Sanitizer）先过一遍可机械判断的（格式、UB、数据竞争），人只查需要判断力的（动机、设计）。
    换个角度：现代 review 的「机器预筛 + 人工裁决」分工也**类似于**餐厅后厨——洗切（机械可判的）交给流水线，掌勺调味（需判断力的）留给厨师。

    > 失效边界：review 不是找茬仪式也不是万能药——它拦得住已知模式的缺陷，拦不住「需求本身就错了」；AI review bot 只能预筛不能裁决，过度依赖会让人退化成「反正有人看」的松懈；重型会议走查成本高，轻量异步才是现代答案。
...
UB（数据竞争、越界、空引用）无法靠测试稳定复现：线程竞争落在同一 `0x0040` 缓存行（false sharing）时吞吐骤降；越界写污染相邻 `8` 对象。`C++20` `consteval` / `constexpr` 把越界、除零等推到编译期求值报错，提前消除一类 UB。
...
- `[book:swe-google:ch9]`（T4）Software Engineering at Google · ch9 —— 提取文本 `docs/references/external/books/swe-at-google.txt`
