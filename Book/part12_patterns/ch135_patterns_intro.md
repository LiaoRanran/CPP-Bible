...
- 航空航天：NASA 的 **cFS（core Flight System）** 与 JPL 的 **F Prime（F´）** 用「状态机 + 命令模式 + 发布订阅」组织航天器飞行软件，是模式在极端可靠约束下的工业落地（见 <https://fprime.jpl.nasa.gov>）。
- 金融/交易：**彭博 Terminal** 的插件与数据订阅体系、以及投行定价/风控引擎对 **Visitor + Strategy** 的运用，被广泛报道采用观察者/中介者来插拔不同衍生品结构与定价模型 [据记载]。
