...
> 事故现场：迭代器失效是 vector 最高频的生产事故——「遍历中 push_back」或「保存着元素的引用、之后再 push_back」时，某次扩容把整块内存搬到新地址，旧迭代器/指针/引用仍指向已释放的旧块（[std-cpp23] 规定重分配后所有指向原元素的引用、指针、迭代器全部失效）。表现不是崩溃而是**静默错乱**：你通过旧迭代器写进去的字节落在已 free 的内存上，被后续分配覆盖；排这类 bug 时日志里没有崩溃、没有断言，只有"数据偶尔不对"。防御就一句话：任何可能触发扩容的操作之前，先 `reserve` 买够容量，或改用下标而非迭代器。

!!! note "类比：vector = 可伸缩书架"
    `vector` 可以**类比**为一排会自己加层的书架：尾部放书是 O(1)，放满了书柜整体换更大的再搬一次家。更**好比** Excel 里会自动增加行数的表格区域。

    > 失效边界：搬家（扩容）后旧地址作废——所有指向原元素的迭代器/指针/引用全部失效；且「换大柜子」的代价摊还到每次是 O(1)，但单次扩容本身是 O(n) 复制。

## ① 我们真正要回答的问题 <span class="badge badge-std">标准</span>

[第 80 章　std::array：连续定长序列](../part07_stl/ch80_array.md)
[第 81 章　std::string：字符序列](../part07_stl/ch81_string.md)

`std::vector<T>` 常被当成"会用就会用"的默认容器，但它其实是 **98% 的 C++ 程序里性能和安全 bug 的同一来源**——不是 vector 错，而是没人搞懂它"什么时候把元素搬走、什么时候让引用失效"。本章不重复"vector 能 push_back"这种入门句，而要带着这八笔账往下读：

1. **三指针模型的 `size()` 和 `capacity()`，物理上差在哪？** `vector` 用 `_M_start` / `_M_finish` / `_M_end_of_storage` 三根指针管理"已有元素"与"已申请容量"两条边界。`size()` 是 `_M_finish - _M_start`，`capacity()` 是 `_M_end_of_storage - _M_start`——两者一先一后，正是"扩容"这个词的物理位置。本章 ⑦ ASCII 内存图 + ⑧ 生命周期把这条边界画清。
2. **GCC 2 倍扩容、MSVC 1.5 倍扩容——为什么厂商会吵这个？** 2 倍增长越小越浪费、越大越好摊还；1.5 倍（黄金比附近）有个隐藏好处：**新容量大概率能复用刚释放的旧内存块**。这不是实现细节洁癖，而是实打实的分配器行为差异。本章 ⑤ Mermaid + ⑩ 汇编给出两套策略的实测对比。
3. **`push_back` 是 O(1) 吗？凭什么？** 单个 `push_back` 是 O(1) 摊还——数学上是因为"几何增长"让每次均摊成本恒为常量。这个证明是理解所有"为什么数组扩容不卡顿"的钥匙。本章 ⑲ 性能用拼接的摊还分析把证明过程走一遍。
4. **一次 `push_back` 触发扩容后，为什么你的引用/指针/迭代器全"报废"了？** 扩容 = 申请新内存 + 把所有元素搬过去 + 释放旧内存，旧地址从此作废。**你不知道哪次 push_back 会扩容**，所以挂了引用就埋雷。本章 ⑨ 展示一次扩容的全过程，⑯ 易错点列出"挂着迭代器推元素"这类经典事故。
5. **`reserve`/`shrink_to_fit` 到底在管什么、该怎么用？** `reserve` 提前把容量买够，让后续 `push_back` 全部避开扩容；`shrink_to_fit` 反过来把多余容量归还（还可能失效所有迭代器）。本章 ② 前置 + ⑰ FAQ 给出一套"预估高频增长就 reserve"的实践。
6. **`allocator` 的四步分离（allocate/construct/destroy/deallocate）为什么存在？EBO 又凭什么能压缩空分配器？** 分配内存与构造对象被刻意拆开，正是为了支持"raw 内存 + placement new"这类机制；而后面的 EBO 让"无状态分配器"不占一字节。本章 ⑪ STL 联系 + 源码分析说明这四步与类型安全的关系。
7. **libstdc++ 的 `_M_realloc_insert` 为什么战战兢兢？规模一扩错就全错。** 扩容搬移必须保证异常安全：如果元素移动可能抛异常，就退回报异常安全的内存复制（经 `move_if_noexcept` 判定）——这正是 `noexcept` 移动决定性能的关键一环。本章 ⑬ 源码逐行读 `bits/vector.tcc` 这段，⑰ 回顾移动语义那条线。
8. **`vector<bool>` 是个什么问题？** 它是标准库里最声名狼藉的特化：位压缩让它每个元素只占 1 bit，换来的是"`bool*` 永远指不进去、返回的是代理对象"——一堆意想不到的坑。本章 ⑯ 易错点把它当压轴反面教材，⑳ 跨语言对比再补一刀。
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(a) 绝对耗时（随机器而变，仅作量级参考）">
  ...
  <text x="74" y="241.5" text-anchor="end" font-size="10.5" font-family="Georgia, serif">6.25</text>
  <line x1="80" y1="176.0" x2="640" y2="176.0" stroke="#ececf0" stroke-width="1"/>
  <text x="74" y="179.5" text-anchor="end" font-size="10.5" font-family="Georgia, serif">12.5</text>
  <line x1="80" y1="114.0" x2="640" y2="114.0" stroke="#ececf0" stroke-width="1"/>
  <text x="74" y="117.5" text-anchor="end" font-size="10.5" font-family="Georgia, serif">18.75</text>
  <line x1="80" y1="52.0" x2="640" y2="52.0" stroke="#ececf0" stroke-width="1"/>
  <text x="74" y="55.5" text-anchor="end" font-size="10.5" font-family="Georgia, serif">25</text>
  <text x="20" y="176" text-anchor="middle" font-size="12" font-family="Georgia, serif" transform="rotate(-90 20 176)">绝对耗时 (ms)</text>
  <line x1="80" y1="217.5" x2="640" y2="217.5" stroke="#C44E52" stroke-width="1.2" stroke-dasharray="5 4"/>
  <text x="640" y="213.5" text-anchor="end" font-size="10.5" font-family="Georgia, serif" fill="#C44E52">基线 8.32ms</text>
  <rect x="141.3" y="217.5" width="64.0" height="82.5" fill="#9A9A9A"/>
  <text x="173.3" y="211.5" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#9A9A9A">8.32ms</text>
  <text x="173.3" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 173.3 314.0)">push_back ×4M（无 reserve）</text>
  <rect x="328.0" y="258.7" width="64.0" height="41.3" fill="#DD8452"/>
  <text x="360.0" y="252.7" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#DD8452">4.16ms</text>
  <text x="360.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 360.0 314.0)">push_back ×4M（预先 reserve）</text>
  <rect x="514.7" y="80.2" width="64.0" height="219.8" fill="#C44E52"/>
  <text x="546.7" y="74.2" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">22.16ms</text>
  <text x="546.7" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 546.7 314.0)">erase-remove 删除 4M 中一半（偶数值）</text>
</svg>
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(b) 相对倍数（可移植信号：基准=1.00×）">
  ...
  <text x="173.3" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 173.3 314.0)">push_back ×4M（无 reserve）</text>
  <rect x="328.0" y="275.2" width="64.0" height="24.8" fill="#DD8452"/>
  <text x="360.0" y="269.2" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#DD8452">0.50×</text>
  <text x="360.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 360.0 314.0)">push_back ×4M（预先 reserve）</text>
  <rect x="514.7" y="167.9" width="64.0" height="132.1" fill="#C44E52"/>
  <text x="546.7" y="161.9" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">2.66×</text>
  <text x="546.7" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 546.7 314.0)">erase-remove 删除 4M 中一半（偶数值）</text>
</svg>
