...
> 前置：[第19章　变量、存储期、链接与 ODR（工业级深度版）](../part03_language/ch19_variables.md)（对象生命周期与存储）· [第47章 虚函数与虚表（vtable）：动态多态的发动机](../part05_oo/ch47_virtual_functions.md)（clock 是空基类，理解 `is_clock` 概念）· [第 39 章　RAII 与 Rule of Zero/Three/Five](../part04_memory/ch39_raii_rule.md)（RAII 计时器）。
> 后续：[第91章 文件系统 filesystem](../part07_stl/ch91_filesystem.md)（`last_write_time` 返回的 `file_time_type` 即 `chrono::time_point`）· [第152章　性能模型与测量学](../part14_perf/ch152_perf_model.md)（基准测量的时间学基础）· [第151章 基准测试与性能度量（C++）](../part13_engineering/ch151_benchmark.md)（基准方法论）。
...
!!! note "类比：chrono = 带单位的量具"
    `std::chrono` 可以**类比**为一把带单位的量具：秒、毫秒、纳秒是「不同的米与英尺」，不能偷偷混加。`duration` 更**好比**一个明确标了单位的数值，绝不把米当秒用。
...
- **类型与模板参数**：`duration<Rep, Period>` 是两个模板参数；`ratio<Num,Den>` 是编译期有理数。见 `[第60章　模板基础与实例化（Template Basics & Instantiation）](../part06_templates/ch60_template_basics.md)`。
- **`constexpr` 与编译期计算**：单位换算、日历字段在编译期完成。见 `[第69章　编译期计算：constexpr / consteval / constinit](../part06_templates/ch69_constexpr.md)`。
- **RAII**：计时器用构造/析构自动记录区间。见 `[第 39 章　RAII 与 Rule of Zero/Three/Five](../part04_memory/ch39_raii_rule.md)`。
- **异常安全**：`system_clock::now()` 不抛；时区查找失败抛 `std::chrono::nonexistent_local_time` 等。见 `[第 40 章　异常安全（Exception Safety）](../part04_memory/ch40_exception_safety.md)`。
- **比值与整数溢出**：`Rep` 类型（如 `int64_t`）需足够宽，否则长时长溢出。见 `[第19章　变量、存储期、链接与 ODR（工业级深度版）](../part03_language/ch19_variables.md)`。

---

## ③ 后续依赖

- **filesystem（第91章）**：`last_write_time()` 返回 `file_time_type = time_point<file_clock, ...>`；比较文件新旧就是比较 `time_point`。`[第91章 文件系统 filesystem](../part07_stl/ch91_filesystem.md)`。
- **性能测量（第152章）**：所有 benchmark 都建立在 `steady_clock` 上。见 `[第152章　性能模型与测量学](../part14_perf/ch152_perf_model.md)`。
- **基准方法论（第151章）**：反复测量、warm-up、统计分布。见 `[第151章 基准测试与性能度量（C++）](../part13_engineering/ch151_benchmark.md)`。
- **并发超时（第103–105章）**：`try_lock_for(d)`、`wait_for(d)` 接收 `duration`。见 `[第93章　线程与异步：thread / future / async](../part07_stl/ch93_thread_async.md)`。
- **格式化（第131章 fmt/spdlog）**：时间戳格式化常借助 `std::format` 与 chrono 的 `operator<<`。见 `[第131章　fmt / spdlog 格式化与日志（C++）](../part11_source/ch131_fmt_spdlog.md)`。
...
- **与 `ratio`（编译期有理数）**：`duration` 的 `Period` 就是 `ratio<Num,Den>`（如 `milli = ratio<1,1000>`，见 `文件：bits/chrono.h 行号：905`）。`[第68章　模板元编程 TMP 基础（递归 / 分支 / 循环）](../part06_templates/ch68_tmp.md)`。
- **与 filesystem（第91章）**：`file_time_type` 是 `time_point<file_clock, ...>`；`last_write_time` 的比较本质是 `time_point` 比较。`[第91章 文件系统 filesystem](../part07_stl/ch91_filesystem.md)`。
- **与 `format`（第8章）**：C++20 起 `std::format("{:%Y-%m-%d}", sys_days{...})` 直接格式化日期。`[第08章　C++23：标准库大修](../part01_history/ch08_cpp23.md)`。
- **与 ranges（第90章）**：`views::take` + `steady_clock` 可做定长时间窗口采样。`[第90章　ranges 与 views：惰性求值与管道组合](../part07_stl/ch90_ranges.md)`。
- **与 `optional`/`expected`（第88章）**：超时可表达为 `expected<Result, timeout_error>`。`[第88章　optional / expected / variant：可空与可辨别联合](../part07_stl/ch88_optional_variant.md)`。
...
5. **`system_clock::time_point` 与 `file_time_type` 能直接相减吗？** 不能——前者是 `system_clock`，后者是 `file_clock`，类型不同；需经纪元换算或都转 `file_time_type`。`[第91章 文件系统 filesystem](../part07_stl/ch91_filesystem.md)`。
...
**Q：`file_time_type` 与 `system_clock::time_point` 能互转吗？** A：C++20 提供 `clock_cast`（如 `std::chrono::clock_cast<system_clock>(file_time)`），因为 `file_clock` 与 `system_clock` 有固定的纪元偏移。见 `[第91章 文件系统 filesystem](../part07_stl/ch91_filesystem.md)`。`[标准]`
...
**Q：`clock_cast` 和 `duration_cast` 有何不同？** A：`duration_cast` 只换单位、不换纪元；`clock_cast` 在不同 `clock` 的 `time_point` 间转换（会按纪元偏移换算），如 `system_clock` ↔ `file_clock`。`file_clock` 的纪元与 `system_clock` 不同，故必须用 `clock_cast`（见 `[第91章 文件系统 filesystem](../part07_stl/ch91_filesystem.md)`）。`[标准]`
...
- `[经验·量级]`：`steady_clock::now()` 单次约 20–30 ns（vDSO/QPC，无需真正陷入内核）；`duration_cast` 在 `-O2` 下被常数折叠为一条 `imul`/`idiv`。因此"每次循环读一次时钟"在纳秒级工作的微基准里会**显著污染结果**——基准时应先测"空转 now()"的开销并扣除（见 `[第151章 基准测试与性能度量（C++）](../part13_engineering/ch151_benchmark.md)`）。
...
- `[经验]`：更稳健的统计用中位数或去极值均值（去掉最大/最小），因为最坏一次会被调度器拖累（见 `[第151章 基准测试与性能度量（C++）](../part13_engineering/ch151_benchmark.md)`）。
...
| 关联章节 | 场景 | 组合方式 |
|---|---|---|
| [第91章](../part07_stl/ch91_filesystem.md) | 多态插件/框架扩展 | 本章提供概念，第91章提供实现 |
| [第91章](../part07_stl/ch91_filesystem.md) | 泛型库/编译期计算 | 本章提供概念，第91章提供实现 |
| [第93章](../part07_stl/ch93_thread_async.md) | 日志格式化/序列化 | 本章提供概念，第93章提供实现 |
| [第91章](../part07_stl/ch91_filesystem.md) | 资源管理/事务回滚 | 本章提供概念，第91章提供实现 |
| [第91章](../part07_stl/ch91_filesystem.md) | 错误恢复/不可恢复错误 | 本章提供概念，第91章提供实现 |
...
- **同模块相邻**：[第91章 文件系统 filesystem](../part07_stl/ch91_filesystem.md)—— filesystem 时间戳依赖 chrono
- **同模块相邻**：[第94章　stop_token 与协作取消 <span class="badge badge-std">标准</span>](../part07_stl/ch94_stop_token.md)—— stop_token 的超时基于 chrono 时长
- **同模块相邻**：[第76章　STL 架构与迭代器概念](../part07_stl/ch76_stl_arch.md)—— chrono 是该架构外的标准库组件
- **跨模块前置**：[第122章　PMR 与多态分配器](../part10_modern/ch122_pmr.md)—— PMR 可定制 chrono 的内存分配
...
- `[cppref:cpp/chrono]`（T1）cppreference `cpp/chrono` —— 离线 `C:\Users\ASUS\Desktop\cppb参考资料\cppreference\`
