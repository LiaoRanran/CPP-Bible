...
> 前置：[第19章　变量、存储期、链接与 ODR（工业级深度版）](../part03_language/ch19_variables.md)（存储期与对象） · [第60章　模板基础与实例化（Template Basics & Instantiation）](../part06_templates/ch60_template_basics.md)（模板与实例化） · [第67章　Concepts 与 requires —— C++20 的编译期约束](../part06_templates/ch67_concepts.md)（C++20 概念）。
> 后续：[第77章　vector：扩容、失效、allocator 协作](../part07_stl/ch77_vector.md)（vector 与三指针） · [第84章　set / multiset：红黑树有序集合](../part07_stl/ch84_set.md)（有序容器） · [第85章　unordered_map / unordered_set：哈希开链集合](../part07_stl/ch85_unordered.md)（哈希容器）。
...
> 事故现场：把 `>=` 传给 `std::sort` 当比较器是 UB——标准 [alg.sorting] 要求 comp 必须是**严格弱序**，而 `>=` 对相等元素返回 `true`，不满足"comp(a,a) 恒为 false"这条自反判据（[std-cpp23]）。违反后快排的分区与递归会越过区间边界读写，表现为随机崩溃或相邻内存被踩坏——排小数组时"看起来能跑"，数据一多就炸，是最难复现的一类线上事故。判据一句话：需要降序就传 `>`，永远别用 `>=`；任何自写的比较器，先自问 `comp(x, x)` 是否返回 `false`。

!!! note "类比：STL = 乐高标准件 + 一支乐队"
    STL 可以**类比**为一套乐高标准件：容器是不同形状的积木，算法是通用工具，迭代器是统一的卡扣接口。更**好比**一支乐队——容器是乐器，算法是指挥，迭代器是把它们连起来的乐谱连线。
...
- 模板基础与实例化：[第60章　模板基础与实例化（Template Basics & Instantiation）](../part06_templates/ch60_template_basics.md)。
- 类型萃取 Type Traits：[第65章　类型特性 Type Traits —— 编译期类型自省与分发](../part06_templates/ch65_type_traits.md)。
- C++20 Concepts：`std::forward_iterator` 等是概念，见 [第67章　Concepts 与 requires —— C++20 的编译期约束](../part06_templates/ch67_concepts.md)。
- 指针即随机访问/连续迭代器：[第20章　引用（reference）vs 指针（pointer）：语义本质、底层实现与生命周期战争](../part03_language/ch20_reference_pointer.md)。

## ③ 后续依赖

- `vector` 三指针与失效：[第77章　vector：扩容、失效、allocator 协作](../part07_stl/ch77_vector.md)。
- 有序/哈希容器迭代器失效差异：[第84章　set / multiset：红黑树有序集合](../part07_stl/ch84_set.md)、Book/part07_stl/ch85_unordered.md。
- 算法分类与复杂度：[第95章　STL 算法分类与复杂度（C++）](../part08_algorithms/ch95_algo_overview.md)。
- ranges 与投影：[第90章　ranges 与 views：惰性求值与管道组合](../part07_stl/ch90_ranges.md)。
...
- `[实现·GCC15]`：连续迭代器展开后等价于指针遍历，编译器可自动**向量化**（[第155章　SIMD / AVX 向量化（C++/硬件）](../part14_perf/ch155_simd.md)）；而 `list` 迭代器因指针跳变无法向量化。
- `[经验]`：热路径遍历优先 `vector`/`array`（连续迭代器），这正是性能敏感代码的铁律。

## ⑪ STL 联系

- 容器提供 `begin()/end()`，算法消费迭代器区间——二者通过迭代器解耦（[第77章　vector：扩容、失效、allocator 协作](../part07_stl/ch77_vector.md)）。
- 仿函数（lambda/`std::less`）作为算法第三参数，实现策略注入（[第71章　策略设计 Policy-Based Design](../part06_templates/ch71_policy.md)）。
- 适配器：`back_inserter` 把"赋值"改成"push_back"；`reverse_iterator` 反转方向（[第90章　ranges 与 views：惰性求值与管道组合](../part07_stl/ch90_ranges.md)）。
- 分配器被容器在底层使用，普通算法不直接接触（[第 38 章　分配器（Allocator）模型与 PMR](../part04_memory/ch38_allocator.md)）。
...
4. 区间算法优先 `std::ranges::xxx`（C++20），可读性更好（[第90章　ranges 与 views：惰性求值与管道组合](../part07_stl/ch90_ranges.md)）。
...
| `unordered_*` | rehash 使所有失效；否则不失效 | 仅被删迭代器失效；引用仍有效 | 见 [第85章　unordered_map / unordered_set：哈希开链集合](../part07_stl/ch85_unordered.md) |

- `[平台·x86-64]`：连续迭代器遍历可被编译器向量化为 AVX 加载（[第155章　SIMD / AVX 向量化（C++/硬件）](../part14_perf/ch155_simd.md)），单遍可快数倍；非连续迭代器每步一次缓存缺失。
...
| [第77章](../part07_stl/ch77_vector.md) | STL算法回调/异步任务 | 本章提供概念，第77章提供实现 |
| [第77章](../part07_stl/ch77_vector.md) | 泛型库/编译期计算 | 本章提供概念，第77章提供实现 |
| [第71章](../part06_templates/ch71_policy.md) | 向量化计算/图像处理 | 本章提供概念，第71章提供实现 |
| [第84章](../part07_stl/ch84_set.md) | 数据处理管道/排行榜 | 本章提供概念，第84章提供实现 |
...
> 交叉引用：容器见 [ch78](../part07_stl/ch78_deque.md) 等；分配器见 [ch38](../part04_memory/ch38_allocator.md)。
...
- **同模块核心**：[第77章　vector：扩容、失效、allocator 协作](../part07_stl/ch77_vector.md)—— vector 是该架构下连续内存容器的典型实现，迭代器类别为随机访问
- **同模块核心**：[第78章　deque 与分段连续 <span class="badge badge-std">标准</span>](../part07_stl/ch78_deque.md)—— deque 的分段缓冲体现同一架构下的另一种迭代器模型
- **同模块核心**：[第79章　list / forward_list <span class="badge badge-std">标准</span>](../part07_stl/ch79_list.md)—— list/forward_list 的节点迭代器满足同一套迭代器概念
- **同模块核心**：[第90章　ranges 与 views：惰性求值与管道组合](../part07_stl/ch90_ranges.md)—— ranges 在该架构之上叠加惰性管道与视图
- **跨模块前置**：[第 38 章　分配器（Allocator）模型与 PMR](../part04_memory/ch38_allocator.md)模型与 PMR）—— allocator 是 STL 容器的可插拔内存后端，架构依赖它切分内存
- **相邻主题**：[第115章　移动语义与右值引用](../part10_modern/ch115_move.md)—— 移动语义是该架构值传递的零拷贝基石
- **相邻主题**：[第122章　PMR 与多态分配器](../part10_modern/ch122_pmr.md)—— PMR 多态分配器是该架构的现代内存后端
