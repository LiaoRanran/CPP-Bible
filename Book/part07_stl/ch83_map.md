...
> 标准基：ISO/IEC 14882:2023 (C++23) / 预计阅读：90 分钟 / 前置：[第76章　STL 架构与迭代器概念](../part07_stl/ch76_stl_arch.md)（STL 架构）、[第20章　引用（reference）vs 指针（pointer）：语义本质、底层实现与生命周期战争](../part03_language/ch20_reference_pointer.md)（引用与指针）、[第80章　array 与固定数组](../part07_stl/ch80_array.md)（array）/ 后续：[第84章　set / multiset：红黑树有序集合](../part07_stl/ch84_set.md)（set/multiset）、[第85章　unordered_map / unordered_set：哈希开链集合](../part07_stl/ch85_unordered.md)（unordered_map）、[第90章　ranges 与 views：惰性求值与管道组合](../part07_stl/ch90_ranges.md)（ranges）/ 难度：★★★★☆｜层级：L2 进阶
...
- 后续：C++11 引入 `unordered_map`（[第85章　unordered_map / unordered_set：哈希开链集合](../part07_stl/ch85_unordered.md)）作为"无序但平均更快"的对照方案；C++17 补 `try_emplace`/`insert_or_assign` 等更安全的接口。
...
!!! note "类比：map = 按键排序的电话簿"
    `std::map` 可以**类比**为一本按姓名排序的电话簿（红黑树实现）：查人要走「翻一半再翻一半」的路。它更**好比**图书馆按索书号排好的书架。

    > 失效边界：查找是 O(log n) 而非 O(1)，且没有哈希；红黑树带来每次插入/删除的旋转开销，若只求「按键去重 + 快速查」且无需有序，`unordered_map` 通常更合适。

## ① 我们真正要回答的问题 <span class="badge badge-std">标准</span>

[第 77 章　vector：动态数组](../part07_stl/ch77_vector.md)
[第 85 章　unordered_map：哈希表](../part07_stl/ch85_unordered.md)

`std::map<Key, T>` 常被一句话介绍成"有序 key-value"，但同是键查找，`map` 与后来居上的 `unordered_map` 走的是完全不同的两棵树。本章不重复"map 会自动排序"这种入门句，而要带着这七笔账往下读：

1. **`_Rb_tree` 到底是什么，为什么 map 的"排序 + 平衡"靠它一次付清？** `map` 不是自己写平衡逻辑，而是包了一层标准库的红黑树 `_Rb_tree`。本章 ⑥ UML 拆开这层关系，⑦ 的 ASCII 布局画出节点与哨兵头 `_M_header` 的真实结构，⑬ 源码分析则带你读 `libstdc++` 的实现。
2. **"插入不移位、删除不失效"——节点稳定性的代价是什么？** 你之所以能用迭代器挂着遍历同时改结构，是因为 `map` 的每个元素都是独立分配的节点，插入/删除只改指针。这是它与 `vector` 最本质的差别（`vector` 扩容会移走一切）。本章 ⑧ 生命周期专门解释这套承诺与边界。
3. **map 的迭代器失效规则，到底和 vector 差在哪？** 一句话：`map` 里只要不删那个元素本身，你的引用/指针/迭代器就都还在——而 `vector` 一次 `push_back` 触发扩容就可能全部失效。本章 ② 前置 + ⑧ 把两套规则并排对照，⑯ 面试题则转成高频追问。
4. **`lower_bound`/`upper_bound`/`equal_range` 凭什么都是 `O(log N)`？** 不是魔法，是红黑树高度 `≤ 2·log₂(N+1)` 的数学保证。范围查询走的就是从根到叶的树下降。本章 ⑩ 用 -O2 实测 `find` 的每次比较与跳转，让你对"一次查找到底走了几步"有量级感。
5. **C++17 的 `try_emplace`/`insert_or_assign`/`extract`/`merge` 补了哪些老疼点？** 更准确地说：它们解决了"查找和插入分两趟导致的不必要拷贝与分配"，以及"想把节点从一棵树搬到另一棵却不复制元素"。本章 ⑨ 的 operator[] 查找-插入路径、⑬ 源码分别展示这些 API 如何省下开销。
6. **透明比较器（`is_transparent`）怎么避免不必要的临时 Key？** `m.find("literal")` 若比较器不透明，会先构造一个临时 `std::string` 再去比；用 `is_transparent` 的 `std::less<>` 直接拿字面量比，省一次堆分配。本章 ⑮ 专门展开这条优化路径及其边界。
7. **工程上到底该上 `map` 还是 `unordered_map`？** 结论先行：需要**有序遍历、稳定迭代器、自定义比较器（有序）**时选 `map`；只求飞快哈希查找、不在意顺序时选 `unordered_map`。数据库索引、配置注册表、符号表、路由表各归其所。本章 ⑫ 工业案例 + ⑲ 性能用数据帮你在两棵树间做出不拍脑袋的选择。
...
有序索引、配置注册表、路由表与符号表是 `std::map` 的主场：编译器的符号表、游戏/服务的配置项注册表、网络设备的路由/ACL 表、编辑器的大纲视图都依赖有序 `map`。它也是实现 LRU 缓存、区间树（如内存分配器的空闲块管理）的常用基石。许多既要求 O(log N) 又要求有序遍历的系统，首选仍是 `map` 而非 `unordered_map`。

- **跨行业实例（数据库/存储引擎）**：LevelDB / RocksDB 的 `MemTable` 与 SSTable 索引层用「有序映射」思想（红黑树或跳表）支持 `lower_bound` 范围扫描；其 `Version` / `VersionEdit` 的元数据也用有序结构管理文件层级——这是「`map` 的有序性直接支撑 LSM 树范围查询」的真实工程。
- **跨行业实例（电信/路由设备）**：路由器与交换机的 ACL/路由策略表（如用 C++ 编写的控制平面）用 `std::map` 表达「前缀/优先级」有序查找，保证「最长前缀匹配、按序遍历」的语义；其节点稳定特性也利于在更新时保持其他迭代器有效。
...
- [cppreference: std::map](https://en.cppreference.com/w/cpp/container/map) — 红黑树有序映射与节点稳定的权威定义
- [open-std: WG21 提案索引](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/) — 查证 map 透明比较器、flat_map 等修订的一手来源
- [LLVM 项目仓库](https://github.com/llvm/llvm-project) — libc++ 的 map/__tree 工业实现参考
...
> 交叉引用：集合见 [ch84](../part07_stl/ch84_set.md)；哈希见 [ch38](../part04_memory/ch38_allocator.md)。
...
- **同模块相邻**：[第76章　STL 架构与迭代器概念](../part07_stl/ch76_stl_arch.md)—— 有序关联容器满足双向迭代器
- **同模块相邻**：[第84章　set / multiset：红黑树有序集合](../part07_stl/ch84_set.md)—— set 是其键即值的同质变体
- **同模块相邻**：[第85章　unordered_map / unordered_set：哈希开链集合](../part07_stl/ch85_unordered.md)—— unordered_map 是其哈希无序版本
- **同模块相邻**：[第79章　list / forward_list <span class="badge badge-std">标准</span>](../part07_stl/ch79_list.md)—— list 节点式存储对比
- **跨模块前置**：[第 38 章　分配器（Allocator）模型与 PMR](../part04_memory/ch38_allocator.md)模型与 PMR）—— 红黑树节点经 allocator 分配
- **相邻主题**：[第115章　移动语义与右值引用](../part10_modern/ch115_move.md)—— 插入元素依赖移动语义
