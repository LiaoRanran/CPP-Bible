...
!!! note "类比：span = 借来的望远镜"
    `std::span` 可以**类比**为一只不拥有景物的望远镜：它只指向别人的内存并记住长度。它更**好比**看菜单——你能读遍每道菜，但菜单不拥有厨房。

    > 失效边界：`span` 不拥有内存，底层数组一释放，悬空 `span` 比悬空引用更隐蔽（编译器无从报警）；它也不能延长所指数据的生命周期。
...
`std::span<T, Extent>` 常被介绍成"非拥有视图"，但"视图"二字最容易被误解成"方便的工具"——它的真正价值恰恰是**替你把"指针 + 长度"这一对必须一起出现的参数，焊成一个不会走丢的整体**。本章不重复"span 不拥有内存"这种入门句，而要带着这六笔账往下读：

1. **span 到底"不拥有"什么，凭什么装得下 array/vector/裸数组三种各不相同的源？** 它只存 `{指针, 长度}` 两个标量，不参与任何内存生命周期——这正是它与拥有型容器（`vector`/`array`）以及 `string_view` 的本质分野。本章 ⑥ UML + ⑪ STL 联系把四者的所有权与适用边界一次厘清，⑧ 生命周期回答"它借来的引用什么时候会悬空"。
2. **动态 extent 和静态 extent，差的不只是"写没写 N"？** 静态 `span<T,N>` 把长度焊进类型、更强校验也更好优化；动态 `span<T>` 则保留 `{ptr,len}` 双字布局。二者在内存布局、接口约束与代码生成上的差异，本章 ⑦ 的 ASCII 布局 + ⑩ 的 -O2 反汇编直接摊开给你看。
3. **span 的"零开销"是真零还是促销话术？** 一个极其干净的实证：它只是 `{指针, 大小}`，在 -O2 下访问被完全内联成数组下标，不产生任何额外指令。本章 ⑩ 用 GCC 15.3 反汇编对比 `span` 索引与 `array`/`vector` 索引，让你亲眼确认"多包一层 = 零成本"。
4. **`first/last/subspan` 切片为什么"越界是 UB"而不是抛异常？** 这是设计取舍：为了保持零开销，`subspan` 不做运行期边界检查，越界即未定义行为——用错就是埋伏雷。本章 ⑨ 调用栈/时序图把每次切片的语义讲清，⑯ 易错点则列出这排最常见的引爆点。
5. **现实中 span 到底抹掉了哪类反复出现的 bug？** 网络封包解析、行情快照、序列化——这些场景过去必须手动传递"裸指针 + 长度"，"长度从哪来"全靠盯。span 把这对参数焊成一个类型。本章 ⑫ 工业案例用真实结构体拆解演示，你会看到它如何消灭"长度错位"这一类事故。
6. **span 怎么和 C 风格接口（`void*` + `size_t`）共存？** 它天生为桥接而生：`data()/size()` 一键递给 C 函数，又能从 C 数组、`vector`、`array` 显式构造。本章 ② 前置 + ⑬ 源码分析给出完整互操作链，⑲ 性能则量化为这个"薄壳"付费的真实成本。
...
```mermaid
flowchart TD
    Root["连续内存抽象（视图家族）"] --> Span["span<T> (任意类型)"]
...
```mermaid
flowchart TD
    A["栈上的 span 对象（x86-64，普通对齐）"] --> B["std::span<int>（8 字节：1 个指针）"]
    B --> P["_M_ptr (8B) ──► 堆/栈上的底层数组 a0 a1 a2 a3 sizeof(int)=4"]
    P --> D["Extent 为动态时：size 也存于 span 内，◄── 指向首元素，总大小 = 16 字节"]
    A --> C["静态 extent（如 span<int,4>）：std::span<int,4>（仅 8 字节：指针）"]
    C --> P2["_M_ptr (8B) ──► 编译器已知长度为 4，不占存储"]
```
...
```mermaid
flowchart TD
    T["时间轴"] --> V["vector<int> v(100); // 拥有存储"]
    V --> S["auto s = std::span(v); // s 借用 v 的内存"]
    S --> Use["使用 s ✅ 安全（v 存活）"]
    S --> Pb["v.push_back(...); // 若触发扩容，v 迁移到新内存"]
    Pb --> D["s 现在指向已释放/旧内存 → 悬垂（UB）"]
    V --> De["v 析构 // 存储归还"]
    De --> UB["此后任何使用 s → UB"]
    De --> Ret["return;"]
```
...
```mermaid
flowchart LR
    Caller["调用方"] -->|"s.subspan(1, 2)"| Obj["span 对象 构造新 span{ _M_ptr = s._M_ptr + 1, _M_extent = 2 }"]
    Obj -->|"(仅计算地址，无拷贝)"| Arr["底层数组"]
    Arr -.->|"返回新 span (O(1))"| Caller
    Caller -->|"[后续] 通过新 span 读写元素 -> 直接读写底层"| Arr
```
