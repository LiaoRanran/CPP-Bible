...
!!! note "类比：array = 焊死在表壳里的刻度盘"
    `std::array` 可以**类比**为大小焊死在表壳里的刻度盘：尺寸在编译期就定好，不占额外开销。它更**好比**给原生数组套了一层带 `size()`/`at()` 的安全壳。

    > 失效边界：尺寸编译期固定、永远不能扩容；它不是 `vector`，没有 `push_back`，越界 `operator[]` 也不会被 `at()` 那样抛异常检查（除非用 `at`）。

## ① 我们真正要回答的问题 <span class="badge badge-std">标准</span>

[第 77 章　vector：动态数组](../part07_stl/ch77_vector.md)
[第 82 章　span：连续序列的视图](../part07_stl/ch82_span.md)

`std::array<T,N>` 常被一句话打发成"类型安全的 C 数组"。但若只是"安全一点"，它不值得 C++11 专门补一个库类型——它的真正价值在于**把裸 C 数组的每一处手工负担，都变成编译器免费替你算好的编译期事实**。本章不重复"array 会给你 `size()`"这种入门句，而要带着这七笔账往下读：

1. **array 凭什么能省略内层花括号？** 靠的是它**聚合类型**的身份：`T a[]{...}` 的多层嵌套在 C++11 起可用"省略内层花括号"默认填充。这不是语法糖，而是 `std::array` 唯一内建成员（一个裸数组）的聚合特性。本章 ⑦ 的 ASCII 内存图 + ⑨ 的调用栈/时序图会把"它就是包着一层壳的 C 数组"讲透。
2. **array 真的零开销吗？还是零开销只是宣传？** 直接看证据：`sizeof(array<T,N>)` 必须等于 `N*sizeof(T)`，因为标准要求它不引入额外存储。本章 ⑦ 给出它与 C 数组**比特级一致**的布局比对，⑩ 用 GCC 15.3 反汇编证明访问 `array` 与访问 `T[N]` 生成相同的指令——这是"零开销抽象"里最干净的一次实证。
3. **array 比裸数组多出的那几样，到底防住了什么？** `size()` / `begin()` / `end()` / `at()`（越界抛异常）/ 值语义拷贝赋值 / **不退化为指针**——它们逐一封堵裸数组最常见的三类事故：越界读写、函数传参时静默退化成指针、拷贝变成浅拷贝。本章 ⑲ 性能 + ⑰ 易错点把这些"防御"的成本和边界说清。
4. **结构化绑定和 tuple 接口，是怎么"长"到 array 上的？** `std::get<N>` / `std::tuple_size` / `std::tuple_element` 让定长数组也能像元组一样解构。本章 ⑭ 专门讲这套接口为何可行（编译期 N 已知是前提），以及它如何把 `array` 接进泛型代码。
5. **C++20 的 `std::to_array` 补了哪个缺？** 从 `T c[N]` "提升"为 `std::array` 时，既要推导 N 又要妥善处理值类别与退化，手写很容易错；`to_array` 把它封成一行。本章 ② 前置 + ⑬ 源码分析给出它与 `data()/front()/back()` 互操作 C 接口的完整路径。
6. **`array<T,0>` 为什么存在？** 标准禁止 0 长度的 C 数组，却允许 `array<T,0>`——它是"没有元素的合法类型"，在模板元编程里承担 `N==0` 的边界场景。本章 ⑫ 工业案例会让它实际跑起来，而非停留在怪谈。
7. **什么时候该用 array 而不是 vector？** 结论先行：长度**编译期已知、栈上定长**时用 array，换取**无堆分配 + 缓存友好**；长度运行期变化才该上 vector。二者的寿命、容量与 allocator 语义完全不同，一旦选错就是"能跑但随时会炸"。本章 ⑲ 用 microbenchmark 量化栈上定长的收益，⑯ 面试题则把这条取舍转成高频追问。

带着这几笔账往下读，每一节都会回到它们：⑦ 布局图回答"零开销"，⑧ 生命周期回答"谁在栈上"，⑯ 面试题把取舍转成追问，⑱ 最佳实践告诉你哪类场景才是 array 的主场。
