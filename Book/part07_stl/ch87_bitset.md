...
> 前置：[第80章　array 与固定数组](../part07_stl/ch80_array.md)（固定大小数组）、[第65章　类型特性 Type Traits —— 编译期类型自省与分发](../part06_templates/ch65_type_traits.md)（整型特性）、[第77章　vector：扩容、失效、allocator 协作](../part07_stl/ch77_vector.md)（vector\<bool\> 特化对比）
> 后续：[第88章　optional / expected / variant：可空与可辨别联合](../part07_stl/ch88_optional_variant.md)（值语义包装）、[第155章　SIMD / AVX 向量化（C++/硬件）](../part14_perf/ch155_simd.md)（位级并行）、[第124章　libstdc++ 架构与阅读入口（C++）](../part11_source/ch124_libstdcxx.md)（阅读入口）
...
!!! note "类比：bitset = 一排带灯开关的面板"
    `std::bitset` 可以**类比**为一排每个开关控制一盏灯的面板：第 i 个比特就是第 i 个开关。它更**好比**把 32 个散装 `bool` 打包进一个字里。
...
> 交叉引用：位操作见 [ch30](../part03_language/ch30_volatile.md)；整数类型见 [ch19](../part03_language/ch19_variables.md)。
...
- **同模块相邻**：[第76章　STL 架构与迭代器概念](../part07_stl/ch76_stl_arch.md)—— 定长位集是该架构的编译期定长组件
- **同模块相邻**：[第80章　array 与固定数组](../part07_stl/ch80_array.md)—— array 是定长值序列，bitset 是定长位序列
- **同模块相邻**：[第89章　tuple / pair / any / function / bind](../part07_stl/ch89_tuple_any.md)—— tuple 等定长异构组件类比
- **对比展示**：[第 38 章　分配器（Allocator）模型与 PMR](../part04_memory/ch38_allocator.md)模型与 PMR）—— bitset 通常不使用 allocator（栈/静态存储），对比展示 STL 内存后端
...
- `[cppref:cpp/utility/bitset]`（T1）cppreference `cpp/utility/bitset` —— 离线 `C:\Users\ASUS\Desktop\cppb参考资料\cppreference\`
