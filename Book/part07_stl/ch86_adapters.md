...
[第 78 章　deque：双端队列](../part07_stl/ch78_deque.md)
[第 77 章　vector：动态数组](../part07_stl/ch77_vector.md)

`stack`/`queue`/`priority_queue` 常被当成"现成容器"，但它们真正的身份更微妙——**它们一律不是容器，而是"只露出少数接口的容器包装"**。这个"不是"字，决定了它凭什么能换底层、为什么没有迭代器、又为什么几乎零成本。本章要带着这八笔账往下读：

1. **"容器适配器不是容器"——这句话到底在说什么？** 它不拥有存储，只是**包住一个有底层的容器**、只把最小接口暴露给你。`stack` 把 `deque` 藏起来只留 `push/pop/top`。理解这点，你就明白为什么它"看起来小、用起来安全"。本章 ⑤ 适配器关系流程图 + ⑦ ASCII 布局把"包装与底层"这对关系画清。
2. **为什么 `stack`/`queue` 默认用 `deque`，`priority_queue` 却默认用 `vector`？** 因为需求不同：栈/队列要"两端都可能高效"，`deque` 两端 O(1)；优先队列要"中间也能随机索引"，只有 `vector` 满足，配合堆做树形跳查。这不是随便选的，是接口最小集倒逼的结果。本章 ⑬ 源码分析 + ⑲ 性能给出这三份默认底层的依据。
3. **用 `Container` 模板参数替换底层，怎么判断"这个容器当得了底层"？** 得满足底层接口上的最小集约束——比如 `stack` 要求底层具备 `push_back/pop_back/back/size`，所以 `vector` 行、`list` 也行、数组不行。本章 ② 前置 + ⑬ 源码把"接口最小集"这条硬约束讲清，你就能自行判断任何候选容器合不合格。
4. **`priority_queue` 的二叉堆到底是什么？"大顶堆"默认，想小顶怎么办？** 它用堆（binary heap）保证 `top` 是极值、插入 `O(log n)`。默认 `less` 是大顶堆，想要小顶可以传 `std::greater` 或自定义比较器。本章 ⑨ 调用栈（一次插入的堆上升）+ ⑩ 汇编把它落到肉眼可见的层。
5. **为什么适配器一律没有迭代器、不能遍历？** 这是**故意收紧**：宁可让你"看不到内部顺序"，也不让你依赖未指定的顺序。`priority_queue` 的内部堆数组虽然物理有序可遍历，但标准明说遍历结果无定义，防止你写死实现细节。本章 ⑧ 生命周期 + ⑮ 面试题解释这条"少即是多"的取舍。
6. **`emplace` 的"零拷贝原地构造"，比 `push` 好在哪？`swap`/比较运算符又贵不贵？** `emplace` 把实参直接转发给底层元素的构造函数，在容器内存里就地构造，省一趟临时对象搬移；因此大对象进 `priority_queue` 应该用 `emplace`。`swap` 与比较运算符只比对底层容器本身，几乎不含成本。本章 ⑬ 源码 + ⑯ 易错点给出能落地的 `emplace` 姿势。
7. **服务器/调度/图算法里，`priority_queue` 怎么当主角，又何时该换手写堆？** 它的主场是"每次都要取当前最优"——请求优先级调度、Dijkstra、贪心。与手写堆的权衡在于：`priority_queue` 不支持"改任意元素优先级并下沉/上浮"，那类动态更新你只能自己实现堆。本章 ⑫ 工业案例（请求优先级调度）给出后者不可替代的场景。
8. **libstdc++ 里 `bits/stl_stack.h`、`bits/stl_queue.h` 到底写了什么？** 两个包一层的 Headers，代码量极小——因为"水"都在底层容器里。读懂它能确认"适配器真不增加运行时开销"，也让你对"它为什么能这么廉价"有了源头答案。本章 ⑬ 源码分析 + ⑳ 跨语言对比把这个"空壳"背后的设计哲学讲完。
