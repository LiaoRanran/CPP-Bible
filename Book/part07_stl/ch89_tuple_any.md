...
> 标准基：ISO/IEC 14882:2023（C++23）。立场分层：`[标准]` 语言/库规定 · `[实现]` 编译器/库实现 · `[平台·x86-64]` 操作系统/ABI · `[经验]` 工程共识。｜层级：L2 进阶

## ⓪ 历史动机：tuple / any 的来龙去脉
...
!!! note "类比：tuple = 异质定长收纳盒，any = 魔术袋"
    `tuple` 可以**类比**为快递的「多格定长收纳盒」：格子数与每格类型都在编译期定好。而 `any` 更**好比**一个能装任何东西的魔术袋——但取物时必须说清你要的是哪种，说错就报错。
...
- **跨行业实例（数据库驱动/ORM）**：SQLite3、libpq 的 C++ 封装用 `std::tuple` 打包「一行多列异构字段」经 `std::apply` 转发给回调，把「列类型各异」的处理从手写模板变成编译期解包；这是 `tuple` 在数据库结果映射中的真实用法。
- **跨行业实例（测试/反射框架）**：GoogleTest 的 `std::tuple` 参数化测试（`TEST_P` 的底层类型列表）、Qt/Boost 的「任意类型属性存储」用 `std::any` 存「运行期才确定类型的属性值」；其「类型擦除逃生舱」语义让框架能持有未知类型而不失类型安全（取用时 `any_cast` 失败抛 `bad_any_cast`）。
...
- `get<N>` 直接访存 ≈ 1.0ns`[微架构·x86-64][UNVERIFIED]`；`any_cast` 含类型比较 ≈ 0.3ns`[微架构·x86-64][UNVERIFIED]`
