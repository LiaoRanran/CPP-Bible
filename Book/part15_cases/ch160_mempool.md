...
!!! note "类比：内存池 = 专属裁缝"
    内存池可以**类比**为「专属裁缝」——通用 malloc 是给一百万种身材做衣服的成衣厂（元数据 / 锁 / 碎片代价高），内存池只给你这一种身材量体裁衣，又快又省又可控尾延迟。固定大小块池更**好比**停车场划好固定车位——来车直接进格，离场即释放，绝不发生「车位大小不合」的碎片。
    换个角度：固定块池也**类似于**按号入座的影院——座位尺寸统一，入场退场都是 O(1)，无需现场量地登记。

    > 失效边界：专用池只赢在「你的那一种请求」上——一旦对象尺寸 / 生命周期变杂，固定块池会退化为浪费或溢出；通用分配器（哪怕 mimalloc）虽慢些但适应一切，池与通用分配器是互补而非替代，游戏 / 实时系统要的是尾延迟可控而非平均更快。
...
```asm
; 节选自 Examples/_ch160_mempool_a1.asm
; Pool::~Pool()  (30 条指令)
push    rdi
push    rsi
push    rbx
