...
- **1980–90 年代**：Doug Lea 的 `dlmalloc` 成为开源世界事实标准的通用分配器，奠定了分箱与边界标记的范式。[史]
- **1998**：C++98 把"分配器（allocator）"写进 STL，允许容器脱离 `::operator new` 自行定制内存来源——这是标准层面第一次为"换分配器"留门。[史]
- **2000 年代**：Google 的 `tcmalloc`、Jason Evans 的 `jemalloc` 用线程缓存把多线程分配性能推上一个台阶，但它们仍是"通用"的。[史]
- **2017**：C++17 引入 PMR（Polymorphic Memory Resources），用 `std::pmr::memory_resource` 把"内存池"做成可插拔的运行时策略，取代了旧 allocator 模型中大量样板。[史]
...
- `mimalloc`（Microsoft，2019）用极致精简的元数据再次刷新了通用分配器性能，但专用池在 hot path 上仍不可替代。
- （待续：无锁内存池、与 hazard pointer 配合的安全回收可在此追加。）
