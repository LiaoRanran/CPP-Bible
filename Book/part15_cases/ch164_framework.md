...
- **1990 年代初**：MFC / Qt / wxWidgets 把"框架接管主循环"变成桌面开发的常态。[史]
- **2002–2003**：Java 的 Spring 把控制反转做成了显式的"依赖注入（DI）"容器，框架不再只管 UI，还管对象的生死与装配。[史]
- **2000 年代后期**：插件化、配置驱动、事件总线等模式成熟，框架从"GUI 壳"演化为"应用骨架"。
- **现代 C++**：结合 RAII、依赖注入容器（见第141章）与构建期代码生成，C++ 也能搭出轻量而强类型的框架。
