...
> "库是你调用它；框架是它调用你。"——这句被讲了三十年的俏皮话，点破了两者权力的颠倒。
...
> 表注（0.2）：框架的演进主线是"控制权从调用方转移到框架"——DI 与代码生成是这条主线在现代 C++ 上的两项关键落地。
...
!!! note "类比：框架 = 餐厅（它调用你）"
    框架 vs 库可以**类比**为「餐厅 vs 厨具」——库是你调用的厨具（你掌勺、你写 main），框架是餐厅（它接管主循环、它来调用你写的菜）；这正是「好莱坞原则：别打给我们，我们会打给你」。约定优于配置更**好比**自动挡——Rails 把前者推到极致，少写即得。
    换个角度：约定优于配置 vs 显式即正义也**类似于**自动挡 vs 手动挡——C++ 文化天然偏手动挡（宁可多写也要看得见、控得住）。

    > 失效边界：框架的「魔法」是双刃剑——新手用得爽，一旦出怪问题就完全看不懂「到底谁调用了我这段代码」；C++ 里模块与静态反射正让「框架屏蔽了什么」更易被类型系统看见，但手写迷你框架最大的价值正是把黑盒里的控制反转拽回你眼前。
...
```mermaid
flowchart LR
  C["checkout 拉源码"] --> G["g++ -O2 编译示例"] --> T["run tests 跑 _ch164"] --> U["upload art 产物归档"]
...
| 关联章节 | 场景 | 组合方式 |
|---|---|---|
| [第163章](../part15_cases/ch163_net.md) | 键值查找/缓存 | 本章提供概念，第163章提供实现 |
| [第159章](../part15_cases/ch159_threadpool.md) | TCP服务器/HTTP客户端 | 本章提供概念，第159章提供实现 |
...
框架是「把控制流与扩展点交给用户」的架构形态。下面按领域展开：

| 领域 / 类别 | 代表系统 · 生态 | 它承担的角色 | 规模 · 行业地位 | 备注 / 标准互动 |
|---|---|---|---|---|
| 桌面 / 游戏 | Qt / Unreal Engine（控制反转 + 事件循环 + 插件/模块） | 工业标杆框架 | 大型应用/游戏 | Unreal 模块+反射支撑大型游戏 |
| 类库即框架 | POCO（URI/Net/Util 可组合 + 插件注册） | 可组合子系统 | 老牌 C++ 框架 | 自带插件注册机制 |
| 可扩展库 | OpenCV（注册式插件动态挂解码器/后端） | 轻量可扩展范例 | 计算机视觉库 | 注册式插件 |
| 浏览器 / 数据库 | 插件/扩展点（V8 嵌入 / PostgreSQL 扩展） | 框架思维延伸 | 引擎级扩展 | 扩展点即框架 |
| C++ 应用框架坐标 | Qt / Unreal / Chromium（Mojo+Blink）/ ROS 2（rclcpp+DDS）/ POCO·ACE | 各场景框架实现 | 工业级事实集合 | 信号槽/反射/moc/IPC 各异 |
| 游戏 / 图形 | Unreal/Unity(C++)/Godot(4 C++ 核心)/bgfx/skia | 自带内存池/任务系统/日志 | 渲染后端标配 | 见 ch159–161 |

> **表注（㉒.2）**：上表前 4 行是「框架思维在各领域的体现」，后 2 行是「C++ 应用框架坐标与游戏/图形后端」；框架的本质是「控制反转（IoC）——框架调用你的代码而非你调用框架」，插件/扩展点/反射都是为此服务，Qt 的 moc、Unreal 的反射体系都是为 IoC 而生。

**一条判读**：框架的价值在「约定优于配置、扩展点清晰」，代价是学习曲线与耦合；小项目用轻量库（POCO/自研）即可，大型应用/游戏才值得上 Qt/Unreal 这类重型框架；选型核心是比较「框架提供的扩展机制」是否匹配你的插件/模块需求。

### ㉒.3 生产踩坑：框架/插件的误用

| 误用 / 坑 | 后果 | 解法（关联章节） |
|-----------|------|------------------|
| **跨插件 ABI 破坏** | 不同编译器/标准库构建主程序与插件，虚表/异常/STL 布局不一致直接崩溃 | 固定工具链，或只暴露 C ABI + 抽象接口 |
| **过度抽象** | 为"未来可能"堆接口层，编译期耦合与运行时间接调用双增、可读性降 | YAGNI，见 ⑯ |
| **生命周期/所有权混乱** | 插件与宿主互相持有裸指针，卸载顺序错即 UAF | RAII + 明确所有权，见 ⑩ |
| **全局状态污染** | 插件偷偷改全局配置/日志，宿主行为不可预测 | 把上下文显式传入，禁止隐式全局 |
...
**最佳实践**：框架的扩展点优先用接口（纯虚/CRTP）而非宏；生命周期用对象树/智能指针统一，避免裸 `new` 散落；注册表用 `flat_hash_map` + 静态初始化顺序规避（参考 [ch38](../part04_memory/ch38_allocator.md)）。

> 交叉引用：内存池见 [ch160](../part15_cases/ch160_mempool.md)；错误传播见 [ch146](../part13_engineering/ch146_error_handling.md)。
...
| **生命周期** | RAII 管理对象树，框架持有根、统一析构（关联 第39章） |
| **扩展点** | 回调 / 策略模式 / 模板方法（关联 第135/136/137章） |
| **整合** | 线程池 + 日志 + 内存池 + JSON 配置 + 网络 = 可运行后端骨架 |
| **零开销扩展** | CRTP / 策略模板做编译期静态扩展（关联 第51/71章） |

> 表注（面试要点）：五条对应第②（框架 vs 库）、第⑩（RAII）、第⑪（扩展点）、第⑨（完整实现）、第⑬（性能）的核心结论。
