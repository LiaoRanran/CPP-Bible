...
!!! note "类比：线程池 = 劳务派遣公司"
    线程池可以**类比**为「劳务派遣公司」——预先养好一批工人（worker 线程），任务来了直接派，而不是每来一个活就现招现裁（每任务一线程）；招人 / 裁人的固定开销（栈、TLS、调度实体）正是吞吐被吃光的元凶。工作窃取更**好比**几个同事各自有活时，闲的人去「偷」忙的人手里的任务。
    换个角度：工作窃取（work-stealing）也**类似于**几个同事各自有活时，闲的人去「偷」忙的人手里的任务——比「一个中心队列大家抢」更少锁争用。

    > 失效边界：线程池不是银弹——池太小扛不住峰值、太大反而因上下文切换 / 缓存争用变慢；C++ 标准至今没有 Executors，池得自己搭，调度策略（中心队列 vs 工作窃取）选错会引入新瓶颈而非消除瓶颈。
...
| [第160章](../part15_cases/ch160_mempool.md) | STL算法回调/异步任务 | 本章提供概念，第160章提供实现 |
| [第116章](../part10_modern/ch116_perfect_forwarding.md) | 无锁队列/计数器 | 本章提供概念，第116章提供实现 |
...
- **跨模块延伸**：[第93章　线程与异步：thread / future / async](../part07_stl/ch93_thread_async.md)
- **跨模块延伸**：[第94章　stop_token 与协作取消 <span class="badge badge-std">标准</span>](../part07_stl/ch94_stop_token.md)
- **跨模块延伸**：[第158章 性能反模式与陷阱](../part14_perf/ch158_perf_antipatterns.md)
- **跨模块延伸**：[第157章 Compiler Explorer 实战](../part14_perf/ch157_compiler_explorer.md)
...
- `[book:concurrency:ch9]`（T4）C++ Concurrency in Action（Williams） · ch9 —— 提取文本 `docs/references/external/books/cpp-concurrency.txt`
