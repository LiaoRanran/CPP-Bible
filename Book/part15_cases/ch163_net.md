...
- **1983**：BSD socket 问世，奠定 C 接口的网络编程范式，C++ 至今仍直接沿用。[史]
- **1999**：Dan Kegel 写下著名的 "The C10K Problem" 文章，点名"默认并发模型撑不住一万个并发连接"，直接催生了整个异步/事件驱动时代。[史]
- **2000 / 2002**：FreeBSD 的 `kqueue` 与 Linux 的 `epoll`（2.5.44 内核）让单线程就能高效地"盯着"成千上万路 IO；Windows 则有完成端口（IOCP）。[史]
- **2005–2006**：Christopher Kohlhoff 的 Boost.Asio 把"Proactor/Reactor + 异步回调"封装成跨平台 C++ 库，成了现代 C++ 网络的事实入口。[史]
- **2020+**：C++20 协程让"异步代码长得像同步代码"成为可能，正在改写网络层的写法。[史]
...
- C++ 标准化委员会长期讨论把网络纳入标准（Networking TS），与 Executors/sender 提案纠缠至今，尚未进标准。
- （待续：io_uring 对 Linux 网络栈的颠覆、QUIC/HTTP3 可在此追加。）
