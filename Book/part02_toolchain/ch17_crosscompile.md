...
## ⓪ 历史动机：交叉编译与嵌入式工具链（C++）的来龙去脉

> 当你为一台只有 32KB 内存、没有屏幕的芯片写 C++，"在本机跑一下"就成了奢侈——交叉编译由此而生。
...
!!! note "类比：交叉编译 = 孤岛送餐"
    交叉编译可以**类比**为「在自家厨房做好菜、送到没有灶台的孤岛去吃」——目标机（ARM Cortex-M，32KB 内存、无屏）根本装不下编译器，只能在宿主机（x86-64）上生成它的代码；三元组（arm-none-eabi 等）把「为谁编」与「在哪编」解耦。看不见的契约（ABI、soft / hard float）更**好比**插头形状——指令集对了、浮点约定错了，板子照样点不亮。
    换个角度：现代嵌入式多用「完整 C++ + 关异常 / RTTI」而非裁剪子集，也**类似于**买整车再按需拆配置，而非买缩水版——工程手段胜过了换语言。

    > 失效边界：交叉编译的真正难点不在编译，而在 ABI、浮点约定与运行时能否在板子上跑起来——这些「看不见的契约」比指令集本身更易翻车；RISC-V 等新三元组虽成熟，但 sysroot 与库版本错配仍会让二进制上不了板；选错 toolchain 文件比选错 CPU 更致命。
...
下表把 C++ 交叉编译方案的真实工程坐标按「交叉编译方案 × 代表设备 × 它承担的角色 × 规模地位 × 标准互动」并列摆开；它们的最大公约数就是「手机、车机、电视、路由器——你身边的嵌入式系统几乎都经交叉编译诞生」。

| 交叉编译方案 | 代表设备·生态 | 它承担的角色 | 规模·行业地位 | 备注 / 标准互动 |
|---|---|---|---|---|
| Buildroot | 路由器 / IoT 网关 / 机顶盒 | 小镜像快速构建 | 轻量嵌入式常见 | 简单、出镜像快 |
| Yocto / OpenEmbedded | 汽车座舱（GENIVI / AGL）/ 工业设备 | 长期可重现镜像 | 厂商级维护 | 层化、可重现 |
| crosstool-NG | 嵌入式团队自造工具链 | 造 GCC / glibc 交叉链 | 定制工具链 | 生成 `<triplet>-gcc` |
| Autotools 三元组 | 遗留 / POSIX 软件（`--host`） | Linux 移植底层语法 | 无数遗留软件 | `--host` / `--build` |
| 汽车座舱·ADAS | GENIVI / AGL（基于 Yocto） | 中控 / 数字座舱底座 | 每辆智能汽车 | 交叉编译生于中控 |
| 消费电子·IoT | 树莓派 / OpenWrt（ARM / MIPS） | 产出设备镜像 | [据记载] 手边设备 | 交叉工具链产出 |

> **表注（㉒.2）**：本表据各方案官方文档与设备事实整理，意在呈现交叉编译的「产业坐标」而非穷举。手机、车机、电视、路由器几乎都经交叉编译诞生。`sysroot` 指错、host 工具误用是经典坑（见 ㉒.3）。

**一条判读**：选型的真实分野是「镜像复杂度 + 可重现要求」——Buildroot 赢在简单、Yocto 赢在可维护，Autotools 三元组是历史遗产仍无处不在。无论哪套，交叉编译的本质风险都是「编译环境与目标环境不一致」：sysroot 指错、host 工具串味，都会让「链接成功、运行崩溃」成为标配。
