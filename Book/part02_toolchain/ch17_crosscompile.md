...
- **汽车座舱与 ADAS**：GENIVI/AGL（基于 Yocto）是车机与数字座舱的通用底座，交叉编译活在每辆智能汽车的中控系统里。
- **消费电子与 IoT**：树莓派（Raspberry Pi OS 用交叉工具链构建）、路由器 OpenWrt 均经交叉编译产出 ARM/MIPS 镜像——[据记载]你手边的嵌入式设备几乎都出自交叉工具链。
