...
- **数据库引擎**：SQLite、RocksDB 用 perf/火焰图定位 B-tree 与 compaction 热路径，性能剖析活在存储引擎的每次版本调优里。
- **游戏与渲染**：Unity/Unreal 与主机游戏用 VTune/PIX 做帧时间剖析，定位 GPU/CPU 瓶颈——[据记载]游戏工业是 perf/profiler 的最大消费群之一。
