...
- **标准库自身**：libstdc++/libc++ 的 `<version>` 与 `__cpp_*` 宏清单由 SD-6（P0096）对齐，构建配置活在标准库实现内部，决定「哪些特性对用户可见」。
- **CI 守门**：Google/LLVM 等大型项目的 CI 用 `__cplusplus` + 特性宏组合做「能力探测」，决定启用 C++20 Modules 还是回退头文件——构建配置活在超大型代码库的迁移闸门上。
