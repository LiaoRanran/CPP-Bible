...
```cpp
// constexpr lambda（C++17 才标准，此处 14 仅示意编译）
constexpr auto l=[](int x){ return x*2; };
```
```cpp
// 泛型 lambda 递归（需 ycomb，简化示意）
...
```cpp
// 字符串字面量到 string_view（17 特性，此处占位）
#include <string>
std::string s="hi";
```
```cpp
