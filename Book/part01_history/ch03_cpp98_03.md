...
- **存量关键系统**：大量航空航天、汽车 ECU、金融核心（交易网关）、电信交换机仍运行 C++98/03 代码库，因重认证/重测试成本极高而不敢升级。
- **老编译器固化**：嵌入式工具链（某些厂商 SDK 仅提供 C++03 编译器）与游戏主机（PS3 的 Cell 工具链）把 C++03 当"事实上最高标准"。
- **LLVM/Clang 起点**：Clang 早期严格以 C++98/03 子集实现，便于在旧工具链上自举。

- **金融与航空存量**：众多交易网关、航电软件（DO-178C 认证）仍以 C++98/03 运行，原因是重新认证/重测试成本极高——[据记载]这类系统「能跑就不敢动」，成为 C++98/03 长期存活的真实坐标。
- **Bloomberg BDE 基线**：Bloomberg 的开源基础库 BDE（`bdl`/`bsl`）早期以 C++03 编写，2010 年代才逐步迁移到 C++11；它是大型金融基础设施「慢升级」的代表。见 [BDE 源码](https://github.com/bloomberg/bde)。
