...
!!! note "类比：C++ = 给跑车装豪华座椅"
    C++ 的诞生可以**类比**为「给跑车装上豪华座椅」——C 是那台贴地飞驰、零负担的跑车（速度），Simula 的「类 / 对象」是豪华座椅（抽象表达力），Stroustrup 把两者缝一起：既快又有组织。让 C 成为 C++ 子集更**好比**语言界的「向下兼容」——老代码几乎不改就能重编，老程序员几乎不重学就能上手。
    换个角度：把 C 继承来的裸指针 / 未定义行为也**类似于**二手跑车自带的隐患——保留了速度，也保留了老毛病，靠 RAII / 所有权这些现代安全网才能兜住。

    > 失效边界：为兼容 C，C++ 也继承了 C 的粗糙（裸指针、未定义行为），至今仍有争议；且「C 是 C++ 子集」早已名存实亡（VLA / _Complex 等未进 C++），把 C 习惯照搬到 C++ 会错失现代安全机制。
...
这一章不是"带你认识 C++ 的历史背景"——那段编年史你在维基百科就能翻到。它要替你把几笔账算清楚，而这些账直接决定你后面怎么理解 C++ 的每一个特性：

1. **C 在 1970s 已经统治系统编程，为什么还要造 C++？而且为什么不是另起炉灶？** 这是全章最关键的一个判断。答案藏在"兼容"与"迁移成本"里：Bjarne 的目标从来不是发明一门"更好的新语言"，而是给 C 生态一张**渐进式升级的梯子**——当时的代码库、编译器、程序员，一个都不能丢。想通这一点，你才能理解为什么 C++ 至今有那么多"历史背负"（下文的 ③、⑪ 会反复回到它）。
2. **C 为 C++ 留下的哪些遗产是无法剥离的底色？** 值语义、指针、数组、struct 聚合、乃至"编译-链接"的底层模型——这些不是 C++ 的"小尾巴"，而是它的地基。本章 ②、⑦、⑧ 会告诉你：为什么 `int a[10]`、`Point p{3,4}`（见下示例 2）到今天仍是 C++ 的 First-Class 公民，而某些现代语言再也给不了你这个的精度。
3. **「C with Classes」（1979–1983）到底引入了什么？它离现代 C++ 还有多远？** 类、继承、早期虚函数在那个阶段已经出现，但那是站在 C 编译器之上"手工缝合"的类——对象的构造、析构、虚表都还手写。弄懂这段过渡（⑨ 的 CFront 编译模型会给线索），你会突然看明白现代 C++ 那些"魔法"到底为你扛走了多少脏活。
4. **「C++ 是 C 的向后兼容超集」这句话，在什么意义上成立、什么意义上是误导？** 语法层面它基本成立——`#include <cstdio>`、`printf`、`struct Point` 至今原样可编译；但语义层面并非无脑超集：UB、例外池、链接规则都有精心刻画的边界。别把这句话当口号，它是一份带着"少数例外"的契约（⑰、⑱ 给出边界清单）。

带着这几笔账往下读，每一节都会回到它们：⑩ 用 GCC 15.3 反汇编 `printf`/`Point` 同构示例，给你"C 与 C++ 生成同一份机器码"的第一手证据；⑪ STL 联系、⑫ 工业案例告诉你这套遗产在生产里怎么继续发挥价值。
...
- ch19–ch32（语言基础）—— 本章的 C 遗产是后文所有语法的基础。
- ch35–ch44（内存管理）—— `malloc`/栈/堆模型源头在 C。
- ch45–ch52（面向对象）—— 类/继承/虚函数在此萌芽。
