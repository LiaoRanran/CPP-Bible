...
!!! note "类比：C++11 = 憋了八年的大扫除"
    C++11 的「还债式大爆发」可以**类比**为一次憋了八年的大扫除——把内存泄漏（裸指针）、样板爆炸（类型声明）、并发缺席三笔旧账一次性结清；移动语义更**好比**把「深拷贝」换成「偷指针」，性能跃升。auto / lambda / 智能指针把样板与裸指针「赶下桌」也**好比**用洗碗机替代手洗——初用者怕「藏了类型」，但模板迭代器时代手写类型既累赘又易错，推导才是趋势。
    换个角度：C++ 选「提供工具、由你负责」而非 Rust 的所有权编译期杜绝，也**类似于**给你安全带但不强制系——泄漏风险仍在程序员肩上。

    > 失效边界：一次性还债过多致 ABI / 行为剧烈变更，后续版本被迫改「小步快跑」正是吸取这次教训；且安全网是「可选」的，不用智能指针 / 不遵 RAII，旧账依旧在你代码里。

> **一句话结论**：C++11 是一次「还技术债」式大爆发：移动语义改写资源管理，auto/lambda/智能指针把样板与裸指针赶下桌，并首次把并发带进标准。

> 历史注脚：C++11 不是凭空爆发，而是 1985–2011 间"实现主导、标准缺席"十余年后的集中清算——Stroustrup 在 HOPL 论文里把"零开销抽象 + 渐进演化"列为 C++ 的设计常量（[hopl:cpp1991]），移动语义/智能指针正是这套哲学在资源管理上的兑现，而非追随带 GC 的语言。代价也写在那条常量里：为不与旧代码决裂，C++11 只"加工具、不强制"——不用智能指针、不遵 RAII，旧账依旧在你代码里（见 ch47/ch39）。
...
这一章不讲"C++11 新增了哪些关键字"——那张清单在《Effective Modern C++》里就有几百条。它要替你把三笔账算清楚，而这些账决定了你写 `auto x = 42` 时到底在做什么：

1. **C++98 用了十年，为什么 C++11 一上来就是"范式级"变革？** 因为痛点不是语法不够，而是**所有权语义缺失**：裸指针 + 手工 `new/delete` 让资源释放无法说清"谁负责"；`auto_ptr` 的拷贝语义诡异到几乎是埋雷；没有移动语义，`vector.push_back` 一次拷贝可能把大对象整个复制。C++11 的全部核心特性（移动语义、右值引用、`unique_ptr`、智能指针）都是**同一次所有权革命**的不同侧面（本节下方示例 1：`auto`/范围 `for`/统一初始化只是这场革命的语法入口）。
2. **移动语义到底解决了什么问题，值得单开一章？** 它不是性能微优化，而是把"临时对象"改造成**可廉价转移的资源胶囊**。先想通"拷贝你要保留，移动你只用一次"，再看右值引用、完美转发（见 ③ 入口）就不会晕。这是全章最需要想明白的一条。
3. **「现代 C++」和「旧式 C++」的边界到底画在哪？** 不在语言版本，而在**默认所有权模型**：现代 C++ 默认值语义 + 智能指针 + RAII，而旧式写法默认裸指针 + 手工管理。C++11 的贡献是把"现代实践"从**可选的最佳实践**锻造成**语言级支撑**——`unique_ptr` 不是推荐，而是 `delete` 的默认替代（本章 ⑦ 的 ASCII 内存图会把这两套模型的差距直接画出来）。
...
> 测试环境：AMD Ryzen 9 7940HX（16C/32T）；本机 MinGW-W64 GCC 15.3.0；`g++ -O2 -std=c++20`；`std::chrono::steady_clock` 计时，5 轮取中位；`volatile` sink 防死代码消除。本附录目的：用主控实测锁死的真实微秒，量化 `vector<string>` 在「拷贝构造（深拷贝）」与「移动构造（仅搬指针）」两种语义下的开销差距，并给出非显然根因。**绝对毫秒随机器而变，加速比才是可移植信号。**
