...
```cpp
// std::print 着色输出（本机 GCC13 未实现 <print>，用 cout 演示）
#include <iostream>
void p9(){ std::cout << "ok\n"; }
```
```cpp
...
```
```cpp
// 契约语法示意（C++26 候选，需 -fcontracts）
// void g9(int x) [[pre x>=0]] {}
```
...
```cpp
// 静态反射示意（C++26 提案）
// constexpr auto mems = std::meta::members_of(^S);
```
```cpp
// 模块化标准库示意（需 -fmodules-ts）
// import std;
```
...
```cpp
// constexpr 虚函数（C++20 已允许）
struct V9{ constexpr virtual int f() const { return 1; } };
```
```cpp
// 反射：枚举值名示意
// constexpr auto name = std::meta::name_of(^E::A);
```
...
```
```cpp
// 错误栈踪（C++23 入标准，链接需 -lstdc++_exp）
// #include <stacktrace>  // auto st=std::stacktrace::current();
```
...
```
```cpp
// 简化 coroutine 关键字 co_await/co_yield/co_return
```
...
```cpp
// 编译期反射元函数示意
// template<class T> using info = std::meta::info_of<T>;
```
...
```cpp
// 协程与生成器标准库
// std::generator<int> seq26();
```
```cpp
// 扩展 if constexpr 模式匹配
#include <variant>
#include <string>
