...
- **嵌入式/车规**：长期锁定 C++14 甚至 C++11（编译器认证慢），用 `string_view`/`optional` 都要评估。
- **服务端/云**：普遍 C++17/20，紧跟编译器两年内的特性（如 `std::format`/`ranges`）。
- **库作者（header-only）**：常以 C++14/17 为最低要求以最大化兼容；而二进制发行库需固定 ABI 与编译器。

- **游戏/AAA 与跨平台引擎**：Unreal Engine 5 在 MSVC 工具链上仍以 C++17 为默认语言档，Unity 的 C++ 底层在各平台构建矩阵中锁定特定标准档；大型引擎的“最小可行标准”通常落后服务端 3–5 年，因为要兼顾主机/移动/旧编译器认证。
- **高频交易与低延迟栈**：部分 HFT 系统固守 C++17/20，用 `-std=c++2a` 按需开启单个特性，并以 `__cpp_*` 特性宏做“特性门控”渐进引入 `std::format`/`<ranges>`，避免一次性触碰 ABI。
