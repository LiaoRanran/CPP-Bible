...
### 0.2 关键转折（编年）
模块化的第一次尝试其实失败得很彻底：C++98 的 `export` 关键字想做"模板独立编译"，设计雄心十足，却因实现成本过高几乎无人真正实现，C++11 起被弃用移除。<span class="badge badge-history">史</span> 这条教训直接塑造了后来 Modules 的取向——宁可与 `#include` 保守共存，也不追求模型纯粹。此后 Daveed Vandevoorde 等人的模块提案几经修订打磨，直到 **C++20（2020）** 才正式落地 `module`/`import`/`export` 三件套，提供可增量编译、无宏污染的语义导入单元。<span class="badge badge-history">史</span>
...
Modules 入标只是起点，真正的硬仗在"工具链落地"与"生态迁移"两头。<span class="badge badge-history">史</span> 各家编译器进度参差：MSVC 最早提供较完整支持，Clang 紧随其后，GCC 在 10 代之后逐步补齐模块接口编译与 `import` 解析，但三者在模块分区、宏导出等边角上至今仍有差异——这正是"标准已定、实现未齐"的典型状态。

标准自身则在持续推进：**C++23** 引入 `import std;` 与 `import std.compat;`，让标准库本身能以模块形式导入，`<iostream>` 之类被逐 TU 反复文本包含的噩梦才算有了官方示范解法；<span class="badge badge-history">史</span> **C++26** 继续推进模块化的标准库分发与构建系统集成，目标是让"编译防火墙"从选修变成默认。<span class="badge badge-history">史</span>

<span class="badge badge-comment">评</span> 但存量代码的迁移工具至今仍是痛点：动辄百万行的代码里 `#include` 与宏深度交织，自动化改写工具（如 Clang 的模块映射生成）尚不成熟，使得"渐进迁移"说着容易做着难。<span class="badge badge-anecdote">轶</span> 这也呼应了 0.2 那条老教训——C++98 的 `export` 本为"导出模板"而生，却因实现成本极高几乎无人真正实现，最终被废弃；Modules 的设计者刻意避开这条老路，选择与 `#include` 长期共存。
...
!!! note "类比：modules = 预编译好的零件包"
    C++20 `modules` 可以**类比**为「预编译好的零件包」：你 `import` 一个模块，直接拿到编好的接口，不必每次从头重读文本。它更**好比**「书的目录索引」替代「把整本书复印进每一章」。

    > 失效边界：模块不是头文件，`#include` 与 `import` 语义不同；模块的内部私有实现不会泄漏，且宏不会跨模块传播——老代码若依赖宏穿透 `#include` 会悄然失效。
...
C++20 引入 Modules 时把代码组织拆成两条路：`export module` 定义**接口单元**（对外可见的声明序列化进 BMI），`import` 从 BMI 读取这些声明而不是把文本展开进来。`[标准]` 关键在于 Modules 并不打算一夜之间取代头文件生态——它与 `#include` **刻意设计成可共存**，让你能按模块粒度渐进迁移，而不是被迫全量重写。`[经验]`
...
接口单元以 `export module` 开头，是模块对外的"门面"；实现单元写作 `module X;`（无 `export`），只服务本模块内部。`[标准]` 这条区分带来一个直接的工程收益：把不导出的内部实现放进实现单元后，导入方连看都看不到——既不污染其命名空间，也让这些实现的后续改动**不触发下游重编**（因为 BMI 没变）。`[经验]`
...
`export` 的粒度相当自由：可修饰单个声明、整个命名空间，也可以用 `export { }` 聚合块一次列出多个声明。`[标准]` 实践中优先选聚合块——它把"这个模块究竟公开了什么"收敛到一处，读代码的人和 Code Review 都能一眼看清导出面，远比散落各处的单条 `export` 好维护。`[经验]`
...
`import` 只把模块的**导出名字**引入当前作用域，且**不引入任何宏**——因为宏活在预处理期，而模块工作在语义期之后，两者根本不在同一层。`[标准]` 这一条直接根治了头文件最臭名昭著的问题：宏泄漏。`<windows.h>` 的 `min`/`max` 宏曾把无数项目的 STL 代码搅坏，逼着大家在包含它之前先 `#define NOMINMAX` 或事后 `#undef`；模块则从机制上让宏无法跨边界传播。`[经验]`
...
模块 `math` 的 `square` 在目标文件里编码为 `_ZW4math6squarei`——其中 `W4math` 是模块名 `math` 的编码、`6squarei` 是 `square(int)` 的签名编码。`[实现·GCC15.3.0]` 真正有说服力的是 `use_mod` 直接 `jmp` 到这个符号，汇编里**找不到任何 `#include` 产生的头文本展开**。这从二进制层面证明了模块导入是**语义引用**而非文本复制：`square` 的声明从 BMI（二进制模块接口）读取，编译一次、多处复用。`[标准]`
...
分区写作 `module big:io`，主接口用 `export import :io` 把各分区再导出、重组成对外统一的模块 `big`。`[标准]` 分区的价值在于"内部可拆、外部仍是一体"：模块作者能按功能把大模块拆成多个文件分别维护，而导入方只需 `import big`，认知负担与单文件模块毫无区别。`[经验]`
...
`module;` 之后、`export module` 之前的区域叫**全局模块片段**：放在这里的 `#include` 照常做文本展开，但其中的宏被关在片段内，不会泄漏给导入方。`[标准]` 这是封装传统 C 头的唯一正确姿势——C 头几乎必然带宏，直接放进模块体会污染下游，放进全局片段则"过而不留"。`[经验]`
...
模块是**发布单元**，命名空间是**逻辑分组**，两者正交——一个模块完全可以导出多个命名空间。`[标准]` 所以不必让模块名与命名空间一一对应；工程惯例是模块名取项目/库粒度（`math`、`networking`），命名空间取代码组织粒度，各管各的反而更清晰。`[经验]`
...
C++23 提供两个命名模块：`std` 与 `std.compat`，后者额外暴露 C 兼容宏以照顾遗留代码。`[标准]` 其中 `import std;` 是迁移 Modules **投入产出比最高的一步**——标准库只编译一次、全工程复用 BMI，省掉的正是 `<vector>`、`<string>` 这类大头被逐 TU 反复文本解析的开销。`[经验]`
...
`import <header>` 把传统头当作**头单元**导入，兼具模块的隔离性与头文件的兼容性。`[标准]` 这给了渐进迁移一条完整路径：内部库优先改成真模块，标准库直接 `import std`，实在动不了的第三方 C 头则用头单元或全局模块片段包一层。`[经验]`
...
模块只改变符号**名字的编码前缀**（加入模块名，`_Z6squarei` 变成 `_ZW4math6squarei`），调用约定、参数传递、内存布局一概不变——模块是编译期组织机制，不是 ABI 机制。`[实现·GCC15.3.0]` 这带来一个很实际的好处：模块代码可以与非模块代码正常链接，只要符号解析得上；也就是说模块化**不必"全有或全无"**，可以一个库一个库地推进。`[平台·Windows]`
...
Modules 给构建系统加了一道硬约束：必须先编译模块接口生成 BMI，再编译使用方——依赖序从"文件级"升级为"模块依赖级"。现代 CMake（3.28+ 的 `FILE_SET CXX_MODULES`）与 Ninja 已原生支持，旧 Make 则仍需手写规则。`[经验]` 这也是迁移中最痛的地方：**难点在构建系统改造，而不在语法本身**——语法半天能学会，把构建图重排好可能要几周。`[经验]`
...
实践中最常见的两个坑：一是**忘记写 `export`**（声明了却在导入方不可见，编译器只报"未声明"，很容易被误判成没 `import`）；二是**模块依赖必须是有向无环图**（DAG）——A `import` B 与 B `import` A 不可并存，成环会直接编译失败。`[经验]` 至于全局状态，模块的 ODR 规则与头文件一致：导出的变量在所有导入单元中是同一个实体，不会每个 TU 各来一份。`[标准]`
...
Modules 用 BMI 缓存语义信息，把"每个 TU 重解析一遍"变成"一次解析、多次读取"，这是编译速度上的根本改进。`[标准]` 大型项目（数百头文件）迁移后编译时间通常降 30%–70%，但这个收益高度依赖一个前提：**依赖链是否真的模块化了**——只要关键路径上还有一个大头文件在走 `#include`，文本重解析就会把收益吃掉大半（附录 F 同此结论）。`[经验]`
...
GCC 15.3.0 用 `-fmodules-ts` 支持这条流程；`use_mod` 最终生成 `mov ecx,7` 加 `jmp _ZW4math6squarei`，证明模块导入在链接期被解析成真实符号，与普通函数调用毫无二致。`[实现·GCC15.3.0]` 三家编译器**语言语法一致、构建命令各异**：Clang 用 `-std=c++20 -fmodules`（成熟度最高），MSVC 用 `/std:c++20 /interface` 配 `.ixx` 扩展名。`[平台·Windows]`
...
导出模板时，模板的**定义**必须在接口单元里可见——因为实例化发生在导入方，看不到定义就无法实例化（道理与头文件里必须包含模板定义完全相同）。`[标准]` 好消息是模块内模板不必再搞"声明放 `.hpp`、定义放 `.tpp`"的分离套路，定义直接写在接口单元里，代码组织反而更整洁。`[经验]`
...
`constexpr` 与 `inline` 在模块中语义不变，仍满足 ODR——多个 TU 导入后拿到的是同一实体，不会因为"每个 TU 各展开一份"而产生多份定义。`[标准]` 模块让 `inline` 变量的分发路径更明确了：它随 BMI 走，不再依赖"谁 `#include` 了这个头"这种隐式的文本传播。`[经验]`
...
三家语法一致，但构建方式与 BMI 细节各不相同，而且**跨编译器共享 BMI 完全不可行**——BMI 不是标准格式，甚至连同一编译器的不同版本之间都可能互不兼容。`[平台·Windows]` 所以模块迁移前要先统一团队的编译器与版本，否则极易陷入"我机器上能编、CI 上编不过"的经典惨剧。`[经验]`
...
Modules 的回报几乎全在**编译期**：运行期它与 `#include` 生成完全等价的代码，不要指望 `import` 能让程序跑得更快。`[经验]` 收益大小取决于头文件解析成本在总构建时间里的占比——Boost、Qt、自研大框架这类"头文件巨兽"收益最明显；小项目原本就没多少重复解析可省，收益有限。需要提醒：上面代码块里的 8.2s / 2.1s 是**不同场景下的量级示意**，不是本机实测；本章 D5.1 给出的本机实测加速比是 **1.42×**，引用数字时请以实测为准。`[经验]`
...
C++ Modules 补齐了与 Rust/Java/C# 同级的语义化模块能力，同时保留了一条别的语言没有的退路——与 C 头互操作（头单元 / 全局模块片段）。`[标准]` 迁移顺序建议按"收益 ÷ 风险"排：**内部库 > 标准库 `import std` > 第三方 C 头（头单元）**，切忌一上来就全量改写。`[经验]`
...
`[史]` C/C++ 自 1973 年前后就沿用「预处理器文本包含」模型——`#include` 本质是逐字粘贴头文件文本。这套模型之所以能活四十多年，是因为它在当年足够简单、且与 C 的编译模型天然契合；代价则被工程界反复承受：同一份声明在每个 TU 里被重复解析、宏随文本四处泄漏、物理包含顺序变得敏感而脆弱。

`[史]` 期间最接近"模块化"的官方尝试是 C++98 的 `export` 关键字，想做「模板独立编译」，结果是一场被多方（Dag Brück、Gabriel Dos Reis 等人的论文）证实失败的实验——实现复杂、收益甚微，最终在 C++11 被移除。这次失败极其关键：它让委员会意识到，"激进重构"在 C++ 这种体量的存量生态里根本推不动。

`[史]` 于是现代 Modules 改走渐进路线：由 Daveed Vandevoorde 等人自 2012 年前后持续提案推进（N3347、N3440 等），核心思路是「编译期语义导入、不再粘贴文本」，最终在 C++20 落地为 `module`/`import`/`export`；C++23 进一步提供 `import std;` 与 `import std.compat;`，让标准库本身也成为模块，标志着头文件时代开始退场。

`[轶]` 最能说明痛点的是一个广为流传的工程吐槽：`#include <windows.h>` 会泄漏成百上千个宏（`min`/`max`/`small` 等），迫使无数项目在包含它之前先 `#define NOMINMAX`、或事后写一串 `#undef`。Modules 的宏隔离正是冲着这类「祖传痛点」去的——它不是为了让代码更好看，而是要消灭一整类极难定位的编译期破坏。
...
理想很丰满，构建很骨感——下面五条踩坑有个共同根因：**模块把"文本拼接"换成了"对编译产物的依赖"**，于是所有原本被文本模型悄悄掩盖的隐式耦合，都会在迁移时集中爆发。

**宏隔离是双刃剑**。不泄漏宏是 Modules 的卖点，但正因如此，原本依赖"头文件里某个宏已被预定义"的旧代码一旦改成模块，就会暴露出大量隐式宏依赖并直接编译报错——这些依赖在 `#include` 时代是靠文本传播悄悄满足的，从没有人显式声明过它们的存在。

**BMI 改变了构建图的形态**。模块必须先编译成 BMI 才能被 `import`，构建图从「文件级」升级为「模块依赖级」，老构建脚本的并行度安排与缓存键设计都要推倒重来（⑫）。

**工具链版本错配放大了这个约束**。不同编译器、甚至同一编译器的不同版本，BMI 二进制格式都可能不兼容（⑱），所以 CI 必须锁定工具链，否则就会出现"我机器上能编、CI 上编不过"的经典惨剧。

**头单元与真模块极易混淆**。`import "foo.h";`（头单元）和 `import foo;`（模块）行为并不相同：前者保留头语义、后者读 BMI。初学者常把 C 头当成模块 `import`，结果符号不可见或踩到 ODR 问题。

**工具链支持存在真空期**。clangd、Doxygen、Coverity 等工具对 Modules 的支持长期不完整，迁移后一度出现"能编译，但 IDE 与静态分析看不懂"的尴尬窗口——这往往比编译错误更消耗团队耐心，也是许多团队观望的真实原因。
