...
- 汽车（ISO 26262）、航空（DO-178C）、医疗、航天嵌入式等「安全攸关」领域，运行时契约是证明系统行为、定位故障边界的关键工具。
- NASA JPL 的 C++ 编码规范、MISRA C++ 等都在用「断言 + 前置检查」填补语言级契约的缺失；低延迟系统也用契约替代部分运行时校验以保留可观测性。
- **铁路/工业控制（IEC 61508 SIL）**：与汽车 ISO 26262 同族的工业功能安全标准，用运行时契约界定「传感器输入合法区间 / 执行器输出边界」，是证明安全相关系统行为的关键。
- **加密/安全库（OpenSSL、libsodium 的边界检查）**：密码学实现用「前置条件 + 断言」保证「密钥长度 / 缓冲区边界」不被越界，契约式检查是抵御侧信道与内存破坏的第一道运行时防线。
