...
> 立场约定：标注 `[标准]`/`[实现·GCC13]`/`[平台·x86-64 Win64 ABI]`/`[经验]`。
...
- `[实现·GCC13]`：GCC 在 `-O0` 也做 RVO——因为它属于「两阶段对象生命周期重写」，不依赖优化级别。
...
真实汇编（`g++ -std=c++23 -O2 -S -masm=intel Examples/_ch117_rvo.cpp`）——`make` 把构造体直接写进 `rcx`（Win64 ABI 的隐藏返回槽指针），**没有任何拷贝/移动构造调用**：

```asm
; 关键证据：_Z4makev（make）直接向返回槽写入，无 mov 复制、无 call 拷贝构造
_Z4makev:
	.seh_endprologue
	mov	rax, rcx                 ; rcx = 调用者提供的返回槽指针（Win64 隐藏参数）
	mov	DWORD PTR [rcx], -889275714   ; 0xCAFEBABE 直接写入返回槽 = 构造就地完成
	ret
; main 甚至被常量折叠，根本不再调用 make：
main:
	sub	rsp, 40
	call	__main
	mov	eax, -889275714         ; 结果被折叠为常量
	add	rsp, 40
	ret
```

- `[实现·GCC13]`：第 12 行的 `return b;` 被重写为「在 `rcx` 指向的内存上执行 `Big` 的构造」，`b` 与 `x` 共享同一地址——这就是 RVO 的本质：**两个名字、一块内存**。
...
真实汇编（`-O0`，符号最清晰）——`make_forced` 真的 `call` 了拷贝构造函数 `_ZN3BigC1ERKS_`（`Big::Big(const Big&)`）：

```asm
; 关键证据：_Z11make_forcedRK3Big 实际调用拷贝构造（对照 ⑤ 的零调用）
_Z11make_forcedRK3Big:
	mov	QWORD PTR 16[rbp], rcx
	mov	QWORD PTR 24[rbp], rdx
	mov	rdx, QWORD PTR 24[rbp]   ; rdx = &src（实参地址）
	mov	rax, QWORD PTR 16[rbp]   ; rax = 返回槽指针
	mov	rcx, rax
	call	_ZN3BigC1ERKS_         ; ← 真实调用拷贝构造函数 Big::Big(const Big&)
	nop
	mov	rax, QWORD PTR 16[rbp]
	ret
```

- `[实现·GCC13]`：这里 `call _ZN3BigC1ERKS_` 不可省略，因为 `src` 来自调用方，`b` 是一个**独立的新对象**，二者内存不同——这正是「跨存储复制」无法消除的铁证。
...
- `[实现·GCC13]`：GCC 在「多分支返回不同对象」时通常不省略，`-O2` 也会插入拷贝/移动（取决于能否静态证明路径唯一）。
...
真实汇编（`-O2`）：`compute` 把 `result.a[0] = sel` 直接写进 `rcx`（返回槽），`result` 与调用者的 `x` 共用同一槽位：

```asm
; 关键证据：_Z7computei（compute）直接在返回槽写入，具名对象 result 被折叠
_Z7computei:
	.seh_endprologue
...
- `[实现·GCC13]`：第 14 行的 `return result;` 没有生成任何拷贝/移动指令——`result` 的存储被**复用**为返回槽，`edx`（参数 `sel`）直接落位。这就是「同一栈槽复用」。
...
- `[实现·GCC13]`：GCC 13 在 `-std=c++23` 下严格遵循上述条文；`return std::move(local);` 因变成右值而不触发强制省略，印证 ⑦。
...
- `[实现·GCC13]`：`-O2` 下被省略的拷贝彻底消失（见 ⑤/⑪ 汇编），失败的拷贝表现为 `memcpy` 内联或 `rep movs`。
