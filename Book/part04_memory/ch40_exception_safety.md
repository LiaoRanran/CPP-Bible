...
- **医疗器械固件（IEC 62304 / 植入式设备）**：植入式医疗器械的 C++ 固件普遍禁用异常，改用错误码返回，因异常展开在确定性硬实时约束下不可预测且增大认证体积——与 JPL 飞行软件同源的「禁异常」工业实践。
- **汽车 AUTOSAR / 实时 ECU**：AUTOSAR 规定错误通过 `E_OK`/`E_NOT_OK` 等返回码表达，而非异常，以满足 ISO 26262 对确定性故障处理的认证要求。
