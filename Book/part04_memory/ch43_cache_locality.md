...
## ⓪ 历史动机：CPU 缓存与局部性的来龙去脉

> 当 CPU 比内存快了一百倍，"内存墙"成了程序真正的瓶颈——局部性是被逼出来的性能学科。

### 0.1 起源（谁·何时·为何）
1960–70 年代，存储层次（寄存器→缓存→主存→磁盘）随硬件分级出现；1980 年代 CPU 与主存速度差距拉大，缓存未命中（cache miss）成为主导延迟的因素。<span class="badge badge-history">史</span> "内存墙（memory wall）"一词在 1990 年代被广泛使用，描述 CPU 速度远超内存带宽的现实。<span class="badge badge-history">史</span><span class="badge badge-comment">评</span> C++ 作为贴近硬件的语言，其数据结构布局直接决定缓存命中率。<span class="badge badge-history">史</span>

### 0.2 关键转折（编年）
- **1990s**：多级缓存（L1 / L2，后 L3）成标配；缓存行（cache line，常 64 字节）成为数据搬运单位。<span class="badge badge-history">史</span>
- **2000s**：x86 多核让"伪共享（false sharing）"成为并发性能杀手。<span class="badge badge-history">史</span>
- **C++17**：`std::hardware_destructive_interference_size` 标准化缓存行大小提示。<span class="badge badge-history">史</span>

### 0.3 设计哲学之争
C++ 不替你做数据布局优化，但给你"控制布局"的全部权力（结构体成员顺序、SOA / AOS、`alignas`）。<span class="badge badge-history">史</span><span class="badge badge-comment">评</span> 这与"零开销"一致：编译器不偷偷重排你的结构体，性能责任在作者——换来了可预测性。<span class="badge badge-comment">评</span>

### 0.4 史料补遗与持续编年

0.2 停在 C++17 标准化 `std::hardware_destructive_interference_size`。此后"缓存提示"从宏常量走向语言内建。<span class="badge badge-history">史</span>

- **C++20 `std::assume_aligned`（P1007）**：把"此指针按 N 字节对齐"作为编译器可依赖的假设，解锁向量化与去边界检查；是 0.3 "把布局控制权交给你"的精细化工具。<span class="badge badge-history">史</span>
- **`[[likely]]` / `[[unlikely]]`（C++20）影响分支与预取**：虽主司分支预测，但常与缓存友好的数据布局配合，减少冷热数据交错带来的缓存污染。<span class="badge badge-history">史</span>
- **NUMA 与硬件预取成为新现实**：多路服务器的"内存远近"直接决定延迟量级，`std::pmr`（ch38）/ 自定义池（ch44）开始感知 NUMA 节点；硬件预取器（stride / stream）让"顺序访问"的局部性红利更稳，但不规则随机访问仍易踩内存墙（见 ch35）。<span class="badge badge-history">史</span><span class="badge badge-comment">评</span>
- **行业思潮**："面向数据设计（Data-Oriented Design）"在游戏 / 模拟领域盛行，以 SoA 替代 AoS、按访问重排字段，本质是把 0.1 "内存墙"变成可编程杠杆。<span class="badge badge-history">史</span><span class="badge badge-comment">评</span>
...
|------|----------|----------|--------------------------|
| 寄存器 | ~数百 B | ~0.1 ns（1 周期级）`[微架构·x86-64][UNVERIFIED]` | ~0.3 周期 |
| L1d / L1i | 32–64 KB/核 | ~1 ns（3–5 周期）`[微架构·x86-64][UNVERIFIED]` | ~3–5 周期 |
| L2 | 256 KB–1 MB/核 | ~4 ns（12–20 周期）`[微架构·x86-64][UNVERIFIED]` | ~12–20 周期 |
| L3 | 8–64 MB（共享） | ~10 ns（30–50 周期）`[微架构·x86-64][UNVERIFIED]` | ~30–50 周期 |
| 主存（DRAM） | 8–256 GB | ~100 ns（200–400 周期）`[微架构·x86-64][UNVERIFIED]` | ~200–400 周期 |
| SSD（NVMe） | TB 级 | ~10–100 μs（10⁴–10⁵ 周期）`[微架构·x86-64][UNVERIFIED]` | ~10⁴ 周期 |
| 磁盘（HDD） | TB 级 | ~5–10 ms`[微架构·x86-64][UNVERIFIED]` | ~10⁷ 周期 |
...
**【平台】** 【实现-推断】 TLB（Translation Lookaside Buffer）是**页表项的缓存**——把虚拟页号→物理页框的映射缓存起来，避免每次访存都走多级页表（4 级页表遍历可达 ~100 ns 级 `[微架构·x86-64][UNVERIFIED]`）。TLB 很小（L1 dTLB 常仅 32–64 项），且是**全相联/少路组相联**。
...
**【标准】** 现代 CPU 是**流水线 + 乱序执行**，遇条件分支需"预测"走哪边；预测失败（mispredict）要清空流水线、重填，代价约 10–20 周期 `[微架构·x86-64][UNVERIFIED]`。C++20 引入 **`[[likely]]` / `[[unlikely]]`** 属性给编译器分支偏好提示，帮助生成更优代码布局与预测hint。
...
## ⑲ 综合实战：缓存友好的计数器与内存池 <span class="badge badge-exp">经验</span>
...
## ⑳ 速查表与检查清单（跨语言补充）<span class="badge badge-exp">经验</span>

**练习题**（已升级为「真实场景 + 引用参考」框架：保留原考察技能，场景改写为工程应用）

1. **真实场景：行主/列主遍历矩阵 cache miss 差一个数量级。** 你遍历 `mat[row][col]` 时按列外层循环，命中率极低。请说明语言层对数组布局的保证。
   - <span class="badge badge-std">标准</span> 多维数组按行主（最后一个下标变化最快）连续存储；语言层只保证这种连续性，具体缓存命中由硬件决定。
   - <span class="badge badge-ref">引用</span> ISO/IEC 14882:2023 §[dcl.array]（数组的连续存储与下标序）；cppreference "Array" 词条。

2. **真实场景：AoS → SoA 提升打包效率。** 你把 `struct {float x,y,z;}` 数组改成三个独立 `float` 数组，SIMD 加载更密。请结合布局/填充说明收益来源。
   - <span class="badge badge-std">标准</span> 连续数组元素在内存中相邻；把同字段聚到一起减少了跨字段的填充与无关数据加载，提高了缓存行利用率。
   - <span class="badge badge-ref">引用</span> ISO/IEC 14882:2023 §[class.mem]（成员布局与填充）/ [dcl.array]（连续存储）；cppreference "Data-oriented design" 词条。

3. **真实场景：false sharing 用 `alignas` 隔离。** 你给每线程的计数器加 `alignas(64)`，避免互相使对方的 cache line 失效。请说明对齐增强的合法性。
   - <span class="badge badge-std">标准</span> `alignas` 可要求比类型自然对齐更强的对齐（不超过实现上限），常用于把一个对象推到独立 cache line 起点。
   - <span class="badge badge-ref">引用</span> ISO/IEC 14882:2023 §[dcl.align]（alignas 说明符）；cppreference "alignas" 词条。
...
| 关联章节 | 场景 | 组合方式 |
|---|---|---|
| [第42章](../part04_memory/ch42_strict_aliasing.md) | 键值查找/缓存 | 本章提供概念，第42章提供实现 |
| [第44章](../part04_memory/ch44_memory_pool.md) | 无锁队列/计数器 | 本章提供概念，第44章提供实现 |
| [第36章](../part04_memory/ch36_stack_heap.md) | 多态插件/框架扩展 | 本章提供概念，第36章提供实现 |
| [第154章](../part14_perf/ch154_cache_opt.md) | 泛型库/编译期计算 | 本章提供概念，第154章提供实现 |
...
### ㉒.1 历史渊源补强：缓存与局部性从哪来
<span class="badge badge-history">史</span> 「cache」一词由 **Maurice Wilkes** 在 1940 年代设计 EDSAC 二代存储时提出（他称其为 "slave memory"，用于隐藏磁鼓延迟）；**IBM System/360 Model 85（1970）** 是第一台商用化引入 CPU 缓存的主机。多级缓存（L1/L2）随 **Intel Pentium Pro（1993）** 进入 x86 主流；**L3** 在 Pentium 4 / 多核时代成为片内共享层。<span class="badge badge-history">史</span> 「false sharing（伪共享）」这一术语随 SMP 多处理器扩展被提出——多核争用同一缓存行里的不同变量，使缓存一致性流量爆炸，是 2000 年代后大规模并行服务的头号性能杀手。<span class="badge badge-anecdote">轶</span> 早期手册把 cache 当成「透明加速」，直到 multi-core + NUMA 时代工程师才被迫理解缓存行边界，否则「改一个计数器让程序慢 10 倍」成为经典面试题。<span class="badge badge-comment">评</span> 缓存不是「更快的内存」，而是一套**空间/时间局部性的博弈系统**；C++ 程序员必须主动布局数据，而非依赖硬件。
...
<span class="badge badge-history">史</span> C++11 引入**内存模型与原子**，把「数据竞争 / 可见性 / 一致性」写进标准，缓存一致性从「实现细节」变成**可推理的语义契约**；`std::atomic` 的 `memory_order` 直接对应缓存一致性流量。<span class="badge badge-history">史</span> **C++17** 通过 **P0154（Hardware Interference Size）** 给出 `std::hardware_destructive_interference_size` 与 `std::hardware_constructive_interference_size`（头文件 `<new>`），让「按缓存行对齐以规避伪共享」首次成为**可移植的标准设施**而非平台魔数。<span class="badge badge-history">史</span> **C++20** 通过 **P0476** 把 `[[likely]]` / `[[unlikely]]` 属性纳入标准，给分支预测器可移植提示（见 ⑫）。<span class="badge badge-comment">评</span> 标准始终在「暴露硬件真实代价」与「保持可移植」之间权衡：缓存行大小仍是实现定义（不是常量），所以 `hardware_*_interference_size` 是「查询」而非「假定」。
- <span class="badge badge-history">史</span> 分支预测提示属性由 **P0479R0→…→R5（C++20，`[[likely]]`/`[[unlikely]]`）** 标准化（与 P0476 的 `bit_cast` 是不同提案），给分支预测器可移植提示。ISO 条款 `[atomics]`（C++11 内存模型）把「数据竞争 / 可见性 / 一致性」首次写进标准，使缓存一致性从实现细节变成可推理的语义契约；而 `hardware_*_interference_size`（P0154R0→R1，位于 `<new>` 的 `[basic.stc.dynamic]`）仍是「实现查询」而非「常量假定」——委员会在暴露硬件真实代价与保持可移植之间持续权衡。
...
- **同模块接续**：[第 41 章 智能指针全解（unique_ptr / shared_ptr / weak_ptr / enable_shared_from_this）](../part04_memory/ch41_smart_pointers.md)—— 控制块的缓存局部性决定解引用开销。
- **同模块接续**：[第 38 章　分配器（Allocator）模型与 PMR](../part04_memory/ch38_allocator.md)—— pmr 池化减少碎片、改善空间局部性。
- **同模块接续**：[第 35 章  C++ 程序的内存模型与操作系统视角](../part04_memory/ch35_memory_layout.md)—— 段/页与 CPU 缓存层级的关系。
- **同模块接续**：[第 36 章　栈（stack）与堆（heap）的深度对比](../part04_memory/ch36_stack_heap.md)—— 栈连续分配天然缓存友好。
- **相邻主题**：[第 45 章　C++ 面向对象总览与对象模型基础](../part05_oo/ch45_oop_object_model.md)—— 对象内存布局与访问局部性。
- **前置基础**：[第122章　PMR 与多态分配器](../part10_modern/ch122_pmr.md)—— 池化资源降延迟的底层机制。

## 附录 I：工业实战复盘（I.实战）[I: Practice]

### 工业案例（真实可查证）

- **链表的缓存不友好性**：`std::list` 每个节点单独分配（通常调用 `operator new`），相邻节点地址不连续→每次遍历都是 DRAM 访问（~100ns `[微架构·x86-64][UNVERIFIED]`）而非 L1 命中（~1ns `[微架构·x86-64][UNVERIFIED]`）。这是「数据局部性比算法复杂度更决定性能」的经典案例。
...
<span class="badge badge-std">标准</span> 数组行优先存储；访问模式应顺应内存布局以利用空间局部性。
<span class="badge badge-ref">引用</span> ISO/IEC 14882:2023 §[dcl.array]（多维数组按行优先布局）；cppreference "std::cache_aligned_allocator" 与硬件预取相关讨论；Ulrich Drepper "What Every Programmer Should Know About Memory" 详述空间局部性与 cache miss。
...
<span class="badge badge-std">标准</span> false sharing：不同变量共享缓存行导致核间无效化；`alignas(缓存行)` 隔离解决。
<span class="badge badge-ref">引用</span> ISO/IEC 14882:2023 §[dcl.align]（`alignas` 控制对齐）；cppreference "alignas"；Intel "Avoiding and Identifying False Sharing" 白皮书；C++ Core Guidelines CP.22（保持共享数据间隔一缓存行）。
...
<span class="badge badge-std">标准</span> SOA 提升数据局部性与 SIMD 利用率，以结构可读性换取吞吐；常用于粒子/数值热点。
<span class="badge badge-ref">引用</span> ISO/IEC 14882:2023 §[dcl.array]；cppreference "std::simd"(C++26 前为实验 TS) 与数据布局讨论；Mike Acton "Data-Oriented Design" 强调按访问模式而非对象语义排布内存；游戏引擎 ECS（如 Unity DOTS、EnTT）普遍采用 SOA。
...
## 可视化速查图（Mermaid 补充）[C: Low-level / E: Performance]

> 本节以图示补全 ⑳ 速查表与 ⑲ 实战的"为什么"——缓存层级与 false sharing 是性能维度的直观入口。

### 图 1 · 存储器层级与访问延迟（量级，x86-64 典型值）

```mermaid
graph LR
    CPU["CPU 核心"] --> L1["L1d 约4周期 32KB"]
    L1 --> L2["L2 约12周期 256KB"]
    L2 --> L3["L3 约40周期 共享"]
    L3 --> MEM["主存 约200周期"]
```

### 图 2 · False Sharing：同一 cache line 上的两线程写冲突

```mermaid
flowchart TD
    A["线程0 写字段 x"] --> B["x 与 y 落在同一 64B cache line"]
    C["线程1 写字段 y"] --> B
    B --> D["MESI 协议下该 line 在核间反复失效"]
    D --> E["本应并行的写被串行化 性能塌缩"]
```
...
flowchart TD
    A{"热点循环遍历多维数组?"}
    B{"按行主序连续访问?"}
    C["转置或改用行主序遍历 (列主序 5-30x 慢)"]
    D{"结构体只遍历部分字段?"}
    E["改用 SoA (Struct of Arrays)"]
    F{"多核并发写相邻计数/字段?"}
    G{"有 false sharing 风险?"}
    H["用 alignas(64) 或 per-thread 归约"]
    I{"大工作集随机访问?"}
    J{"TLB miss 严重?"}
    K["用大页 (huge pages) 或分批"]
    L{"跨 NUMA 节点分配?"}
    M["用 first-touch / NUMA 亲和分配"]
...
> 决策流说明：缓存优化的第一决策是"让最内层循环沿内存连续方向走"——行主序连续访问 vs 列主序可差 5-30×；只遍历部分字段改用 SoA。多核写相邻数据先排查 false sharing（alignas(64) 或 per-thread 归约）；大随机工作集考虑大页降 TLB miss；跨节点分配用 first-touch / NUMA 亲和；指针追逐热路径尽量改数组索引或分块。

## 附录 K：缓存局部性知识图谱（D6 维度）

```mermaid
flowchart TD
    V1["存储层次 L1/L2/L3/DRAM"] --> V2["Cache Line 64B"]
    V2 --> V3["空间局部性"]
    V2 --> V4["时间局部性"]
    V3 --> V5["行主序 vs 列主序"]
    V5 --> V6["SoA vs AoS"]
    V1 --> V7["MESI 缓存一致性"]
    V7 --> V8["False Sharing"]
    V8 --> V9["NUMA first-touch"]
    V1 --> V10["TLB 与大页"]
    V10 --> V11["Prefetch 硬件/软件"]
    V3 --> V12["数据布局 填充/对齐"]
    V4 --> V13["分支预测 likely/unlikely"]
    V12 --> V13
    V6 --> V11
    V14["vtable/ICache 虚调用"] --> V1
```
...
| 边 | 起点 → 终点 | 依赖含义 |
|---|---|---|
| 1 | V1 → V2 | 各级缓存以 cache line（常 64B）为传输/一致性单位 |
| 2 | V2 → V3 | 一次取行带来相邻字，故连续访问享有空间局部性 |
| 3 | V2 → V4 | 近期访问的行暂留缓存，再次访问命中即时间局部性 |
| 4 | V3 → V5 | 空间局部性要求最内层循环沿连续维度（行主序） |
| 5 | V5 → V6 | 只遍历部分字段时 SoA 比 AoS 更紧凑、可向量化 |
| 6 | V1 → V7 | 多核下 cache line 经 MESI 协议保持一致性 |
| 7 | V7 → V8 | 不同核写同一 cache line 不同字即 false sharing |
| 8 | V8 → V9 | NUMA 跨节点访问叠加一致性流量，放大 false sharing 代价 |
| 9 | V1 → V10 | 页表项经 TLB 缓存，miss 触发页表遍历 |
| 10 | V10 → V11 | 大页/预取降低 TLB miss 与冷缺失 |
| 11 | V3 → V12 | 结构体填充/对齐影响每行能装多少热点对象 |
| 12 | V4 → V13 | 分支预测依赖执行的时间局部性，与缓存命中相关 |
| 13 | V12 → V13 | 字段布局影响分支与数据预取效果 |
| 14 | V6 → V11 | SoA 连续内存便于硬件预取与自动向量化 |
| 15 | V14 → V1 | 虚调用经 vtable 取指走 I-cache，受缓存层级影响 |

### K.2 跨章闭环表

| 目标章 | 关联主题 | 闭环关系 |
|---|---|---|
| ch19 | 变量与存储期 | .data/.bss 静态对象与 TLS 布局决定缓存行占用基线 |
| ch41 | 智能指针 | 控制块缓存局部性影响解引用开销，对象与元素分布影响命中率 |
| ch45 | 对象模型 | 对象内存布局直接决定字段访问的局部性 |
| ch154 | 缓存优化（性能篇） | 本章基础在性能篇被量化与基准化（perf/measuring） |
| ch122 | PMR 与多态分配器 | 池化分配器改善空间局部性、减少碎片 |
| ch48 | 动态内存 | 堆分配散落导致 dynamic 对象局部性差、易 cache miss |
| ch47 | 虚函数 | vtable 间接取指经 I-cache，与分支预测交互 |

## 附录 D5：真实基准与性能分析 — 行主序与缓存行（GCC 15.3.0）

> 测试环境：AMD Ryzen 9 7940HX（16C/32T）；本机 MinGW-W64 GCC 15.3.0；`g++ -O2 -std=c++23`；`std::chrono::steady_clock` 计时，5 轮取最佳；`volatile` sink 防死代码消除。本附录目的：用主控实测锁死的真实毫秒，量化同一份矩阵在行优先（连续）与列优先（跨行大步长）遍历下的缓存友好度差异，并给出非显然根因。**绝对毫秒随机器而变，加速比才是可移植信号。**

### D5.1 基准结果

对象为 4096×4096 `int` 矩阵（64MB，超过本机 L3，逼迫缓存行为显现）。
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(a) 绝对耗时（随机器而变，仅作量级参考）">
  ...
  <line x1="80" y1="225.8" x2="640" y2="225.8" stroke="#C44E52" stroke-width="1.2" stroke-dasharray="5 4"/>
  <text x="640" y="221.8" text-anchor="end" font-size="10.5" font-family="Georgia, serif" fill="#C44E52">基线 3.96ms</text>
  <rect x="188.0" y="225.8" width="64.0" height="74.2" fill="#9A9A9A"/>
  <text x="220.0" y="219.8" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#9A9A9A">3.96ms</text>
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">row_major（行主序，连续访问）</text>
  <rect x="468.0" y="100.8" width="64.0" height="199.2" fill="#C44E52"/>
  <text x="500.0" y="94.8" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">40.40ms</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">col_major（列主序，stride=16KB）</text>
</svg>
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(b) 相对倍数（可移植信号：基准=1.00×）">
  ...
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">row_major（行主序，连续访问）</text>
  <rect x="468.0" y="175.0" width="64.0" height="125.0" fill="#C44E52"/>
  <text x="500.0" y="169.0" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">10.19×</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">col_major（列主序，stride=16KB）</text>
</svg>
...
2. **16KB 步长把工作集打爆 L1/L2，并触发 set 别名冲突。** 4096 行的列遍历意味着同一列要反复穿越整个矩阵；工作集远超 L1/L2 容量，命中率骤降。步长 4096×4B = 16KB 是 2 的幂，通常（可能）落入同一 cache set 的少数几路，发生 set 别名/冲突未命中——这是"2 的幂步长遍历大数组"的经典坑。

3. **TLB 压力是诚实的旁因。** 4096×4096 `int` = 64MB，远超单页（通常 4KB）覆盖；列遍历的随机大跨度访存让 TLB 频繁换页，叠加 cache miss 进一步拖慢。行主序的连续访问则更友好地复用页表项。
...
int main() {
    const int N = 256;                 // 缩小规模，保证 CI 下 -O0 秒级完成
    std::vector<int> m((size_t)N * N);
    for (int i = 0; i < N; ++i)
        for (int j = 0; j < N; ++j)
            m[(size_t)i * N + j] = (i * 31 + j * 7) % 1000;

    long long row_sum = 0;
    for (int i = 0; i < N; ++i)
        for (int j = 0; j < N; ++j)
            row_sum += m[(size_t)i * N + j];

    long long col_sum = 0;
    for (int j = 0; j < N; ++j)
        for (int i = 0; i < N; ++i)
            col_sum += m[(size_t)i * N + j];

    std::cout << "row_sum : " << row_sum << std::endl;
    std::cout << "col_sum : " << col_sum << std::endl;

    // 功能正确性断言：两种遍历只是顺序不同，求和必相等（绝不断言时间/倍数/sizeof）
    assert(row_sum == col_sum);
    return 0;
}
```
...
- 计时取 5 轮最佳（而非中位），因为缓存基准对冷启动更敏感，最优轮次最能反映稳态缓存行为。
