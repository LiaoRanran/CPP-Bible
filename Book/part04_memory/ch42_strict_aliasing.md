...
- **金融协议引擎（QuickFIX / 二进制 FIX 消息）**：把网络来的二进制 FIX 消息体重解释为协议结构体，必须用 `std::memcpy`/`std::bit_cast` 而非 `(Header*)buf` 强转，否则开 `-O2 -fstrict-aliasing` 后字段读错——金融低延迟撮合里「合法双关」是刚需。
- **GPU / 图形驱动（Mesa / Vulkan 用户态驱动）**：Mesa 内部大量类型双关处理硬件寄存器描述符与打包格式，部分路径被迫 `-fno-strict-aliasing` 或 `__attribute__((may_alias))`——「靠 UB 偷偷双关」在驱动层仍有残留工业现实。
