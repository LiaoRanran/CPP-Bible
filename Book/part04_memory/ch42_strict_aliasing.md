...
| 关联章节 | 场景 | 组合方式 |
|---|---|---|
| [第41章](../part04_memory/ch41_smart_pointers.md) | 无锁队列/计数器 | 本章提供概念，第41章提供实现 |
| [第43章](../part04_memory/ch43_cache_locality.md) | 泛型库/编译期计算 | 本章提供概念，第43章提供实现 |

## 相关章节（交叉引用）

- **同模块接续**：[第 37 章 动态内存分配原语：`operator new` / `operator delete`](../part04_memory/ch37_new_delete.md)—— 类型双关与 aliasing 规则约束内存重解释。
- **同模块接续**：[第 40 章　异常安全（Exception Safety）](../part04_memory/ch40_exception_safety.md)—— 激进优化影响异常处理元数据体积。
- **同模块接续**：[第 44 章 内存池（Memory Pool）从零实现](../part04_memory/ch44_memory_pool.md)—— 池内对象类型转换的 aliasing 边界。
- **同模块接续**：[第 35 章  C++ 程序的内存模型与操作系统视角](../part04_memory/ch35_memory_layout.md)—— 同一块内存的多种类型解释视图。
- **同模块接续**：[第 36 章　栈（stack）与堆（heap）的深度对比](../part04_memory/ch36_stack_heap.md)—— 栈/堆内存的类型重解释一致性。
