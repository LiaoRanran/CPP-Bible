...
- **航天飞行软件（NASA JPL C++ Coding Standard）**：JPL 的飞行软件编码标准明确限制甚至禁止在深空任务里使用堆（`new`/`delete`），因为不可预测的碎片在无法重启的航天器上不可接受——全部用栈 + 静态池，这是「禁用堆是安全攸关行业的硬规定」而非风格偏好的真实案例。
- **电信 / 5G 基站（Ericsson / Nokia 实时信号处理）**：基站的实时信号处理链路用栈式/池式分配保证确定性尾延迟，避免堆碎片导致通话掉线或调度抖动——与 HFT 同源的「确定性内存」需求。
