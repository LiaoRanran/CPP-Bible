...
下表把「栈 / 堆选择」从语法概念拉到行业决策层，左三行是「用堆但把它管好」，右三行是「干脆不用堆」，依据是同一句话：堆的不确定性代价能否被系统容忍。

| 领域/类别 | 代表系统·生态 | 它承担的角色 | 规模·行业地位 | 备注 / 标准互动 |
| --- | --- | --- | --- | --- |
| 系统软件 / 运行时 | glibc `ptmalloc2`（源自 Doug Lea 的 `dlmalloc`）、Google `tcmalloc`、Facebook `jemalloc` | 通用堆分配器地基 | 几乎所有 C/C++ 程序背后 | 碎片与锁竞争直接决定服务端尾延迟 |
| 游戏 / 实时引擎 | Unreal / Unity 帧分配器 · 栈式临时内存 | 帧末一次性回收对象，避免每帧海量 `new`/`delete` 击穿堆 | 帧预算守卫 | ch44 对象池（第 ⑫ 节）/ 第 ⑦ 节 `alloca` 的产品化 |
| Web 浏览器 | Chromium（PartitionAlloc）+ V8 | 不同安全等级对象放进独立堆区 | 缩小 UAF 攻击面 | 堆布局在安全上的工业级应用 |
| 嵌入式固件 | MCU（无 MMU） | 关闭堆，全栈 + 静态区 + 内存池（ch44） | 确定性系统 | 堆碎片在无 MMU 下可能致命 |
| 航天飞行软件 | NASA JPL C++ 编码标准 | 深空任务禁 / 限堆，全栈 + 静态池 | 不可重启航天器 | 禁堆是安全攸关硬规定，非风格偏好 |
| 电信 / 5G 基站 | Ericsson / Nokia 实时信号处理 | 栈式 / 池式分配保确定性尾延迟 | 防通话掉线 / 调度抖动 | 与 HFT 同源的「确定性内存」需求 |

> **表注（㉒.2）**：上表把「栈 / 堆选择」从语法概念拉到行业决策层。注意 PartitionAlloc 与 JPL 恰好构成两个极端：前者把堆布局当作安全武器（隔离 UAF 攻击面），后者把堆当作必须清除的危险源（不可预测碎片在航天器上不可接受）。三行「用堆」与三行「不用堆」的依据是同一句话——堆的不确定性代价能否被系统容忍。

**一条判读**：栈 / 堆不是「谁快用谁」，而是「系统能否为不确定性买单」。服务端能容忍堆（靠 `tcmalloc`/`jemalloc` 把代价压到可忽略），嵌入式 / 航天 / 基站不能（确定性是硬约束）。判断一个项目该走栈式池还是通用堆，先看它的「最坏情况延迟预算」而不是「平均吞吐量」。
...
### 图 1 · 进程虚拟地址空间典型布局（自上而下 = 由高地址到低地址）

```mermaid
graph TB
    K["内核空间 高地址"] --> S["栈 向低地址增长"]
    S --> HP["堆 向高地址增长"]
    HP --> G["全局静态 .data .bss"]
    G --> C["代码 .text 低地址"]
```
...
```mermaid
flowchart TD
    S["要创建一个对象"] --> D1{"大小 / 生命周期确定"}
    D1 -->|"小且作用域内"| STK["栈：自动分配，零碎片"]
    D1 -->|"大或运行期确定"| HP{"是否跨作用域共享"}
    HP -->|"否"| STK
    HP -->|"是"| D2{"谁负责释放"}
    D2 -->|"RAII 持有"| RAII["智能指针管理堆对象"]
    D2 -->|"裸管理"| RAW["裸 new（高风险）"]
    STK --> PERF["比较分配开销"]
    RAII --> PERF
    RAW --> PERF
    PERF -->|"热路径 / 高频"| POOL["用内存池降低 new 成本"]
    PERF -->|"普通"| OK["完成"]
    RAW -->|"悬垂 / 泄漏"| FIX["改 RAII 或容器"]
    FIX --> D2
    OK --> LOOP["回溯：能否留在栈"]
    LOOP -->|"可逃逸分析"| ESC["保持在栈避免堆分配"]
    ESC --> S
```
> 决策流说明：以"大小/生命周期是否确定 → 栈优先，跨作用域 → RAII 管理堆"为主线；裸 new 出问题即回到 RAII 收敛。

## 附录 K：栈与堆知识图谱（D6 维度）

```mermaid
flowchart TD
    C1["栈 stack"] -->|"承载"| C2["自动存储期对象"]
    C3["堆 heap"] -->|"承载"| C4["动态存储期对象"]
    C2 -->|"零碎片"| C5["分配快"]
    C4 -->|"需释放"| C6["分配器管理"]
    C7["RAII"] -->|"桥接"| C4
    C7 -->|"指向"| C3
...
| C2 → C5 | 栈分配几乎无碎片且快 |
| C4 → C6 | 堆对象需分配器释放 |
| C7 → C4 | RAII 桥接堆对象生命周期 |
| C7 → C3 | RAII 智能指针指向堆 |
| C8 → C1 | 逃逸分析决定留在栈 |
| C9 → C3 | 栈容量不足时转堆 |
| C10 → C6 | 内存池优化堆分配 |
| C11 → C7 | 智能指针是 RAII 实现 |
| C12 → C4 | 频繁小块分配产生碎片 |
| C13 → C1 | 调用栈帧就是栈本身 |
...
| 上游章 | 下游章 | 传递的知识 |
| --- | --- | --- |
| ch20 引用与指针 | ch36 | 栈/堆地址通过指针统一访问 |
| ch28 生命周期与 UB | ch36 | 栈对象作用域结束即销毁 |
| ch35 内存布局 | ch36 | 栈/堆区域是地址空间的下游 |
| ch37 operator new/delete | ch36 | 堆对象经 operator new 创建 |
| ch38 分配器 | ch36 | 分配器在堆上切分内存 |
| ch39 RAII 与规则 | ch36 | 智能指针把堆对象纳入栈式管理 |
| ch41 智能指针 | ch36 | unique_ptr / shared_ptr 是堆管理归口 |

## 附录 L：栈 / 堆 / 内存池 决策流（D3 维度）

```mermaid
flowchart TD
    A["需要一块内存"] --> B{"对象生命周期<br>是否已知且短?"}
    B -->|"是"| C["优先栈上分配<br>自动回收零开销"]
    B -->|"否"| D{"是否数量大 /<br>尺寸运行时确定?"}
    D -->|"是"| E["堆上分配<br>operator new"]
    D -->|"否"| F{"是否同尺寸对象<br>高频创建销毁?"}
    F -->|"是"| G["使用内存池<br>减少碎片与系统调用"]
    F -->|"否"| E
    C --> H{"是否可能<br>异常退出作用域?"}
    E --> H
    G --> H
    H -->|"是"| I["用 RAII / 智能指针<br>保证释放"]
    H -->|"否"| J["裸管理可接受<br>单路径短生命周期"]
    I --> K{"是否跨线程<br>共享该内存?"}
    J --> K
    K -->|"是"| L["加锁或线程局部<br>避免数据竞争"]
    K -->|"否"| M["单线程独占即可"]
    L --> N["完成：分配策略落地"]
    M --> N
    J --> N
...
> 决策流说明：已知短生命周期优先栈；运行时尺寸或大体量走堆；同尺寸高频对象用内存池降碎片；异常路径以 RAII/智能指针兜底，形成分配策略闭环。

## 附录 M：栈 / 堆 / 内存池 知识图谱（D6 维度）
...
| 栈 → 栈帧 | 每次调用建立栈帧 |
| 栈帧 → 自动回收 | 作用域结束自动回收 |
| 堆 → 动态分配 | 动态分配经 operator new |
| 动态分配 → 系统调用 | new 触发系统调用 |
| 系统调用 → 碎片化 | 频繁分配产生碎片 |
| 内存池 → 固定尺寸块 | 池预切固定尺寸块 |
| 固定尺寸块 → 减少系统调用 | 池复用降低系统调用次数 |
| 动态分配 → RAII 管理 | 堆对象借 RAII 管理 |
| RAII 管理 → 智能指针 | 智能指针是 RAII 落地 |
| 堆 → 线程局部存储 | TLS 提供线程独占堆区域 |
...
| 场景 | 耗时 ms | 备注 |
|---|---|---|
| heap `new`/`delete` int ×10M | 480.446 | 单 int 最坏情形 ≈48ns/次 `[实验·本机实测][VERIFIED]` |
