...
```asm
; === 真实汇编（GCC 15.3.0, x86_64-w64-mingw32, -O0）===
...
|---|---|---|---|---|
| **libstdc++**（GCC 15.3.0, x64） | **32 字节** | **16 字节**（`_M_max_size = sizeof(_Nocopy_types)`） | 目标满足 *trivially copyable* 且 `size ≤ 16` 且对齐 ≤ 8 时存本地，否则堆分配 | ✅ 源码+实测（⑳ 第 119 行、`prog_34`） |
...
> ```
> 四个条件**全满足**才存本地；否则走 `_M_create(..., false_type)` → `new _Functor(...)`（第 162–163 行）。
...
#include <utility>
#include <functional>
// std_function.h:588-594  —— operator() 通过 _M_invoker 间接调用（类型擦除核心）
...
// std_function.h:433-456  —— 构造：选择 handler，绑定 manager + invoker
...
- **[KP21]** `std::function` 要求目标可拷贝（static_assert，见 ⑳ 第 441 行）→ unique_ptr 捕获装不进
...
## 附录 H：lambda 捕获的汇编代价（ASM-26-lambda-capture）[E: Low-level]

> 编译：`g++ -std=c++26 -O2 ch26_lambda_capture_test.cpp -o ch26_lambda_capture_test.exe`；反汇编 `objdump -d -M intel -C`（GCC 15.3.0 / Win64）。证据：`_asm_demo/ch26_lambda_capture_test.cpp/.s`。
> 设计关键：用不透明指针 `int* p`（来自 `[[gnu::noinline]] get_field()`）使"被捕获的指针值"编译期未知，并在捕获后插入 `bump(p)` 别名写，逼出按引用捕获**真实的二次解引用**——若目标可证明不变，GCC 会把引用捕获也折叠为直接加载（"按引用捕获在目标可证明不变时零开销"）。

### 闭包对象真实布局（运行期 `sizeof` 实测）

| 捕获形式 | 闭包 `sizeof` | 说明 |
|----------|:---:|------|
| `[]` 无捕获 | **1** | 空类型（C++ 要求非空，占位 1B） |
| `[val=*p]` 按值捕获值 | **4** | 持有快照 `int` |
| `[&r]` 按引用捕获 | **8** | 持有 `int*`（指针，比值大 1 倍） |

### 调用点真实汇编（节选 `.s`）

```asm
; f0 无捕获：调用点直接常量折叠，零闭包
mov    ecx,0x2a           ; 42
call   sink(int)
; f1 按值捕获值：捕获时把 *p 载入 r9d(闭包字段)，调用点仅寄存器搬运
mov    r9d,DWORD PTR [rax]   ; 捕获时: *p -> r9d
...
mov    ecx,r9d            ; 调用点: 快照值已在寄存器，0 次内存访问
call   sink(int)
; bump(p)：不透明别名写，*p 被改写 —— f2 必须实时重读
call   bump(int*)
; f2 按引用捕获(&r = p)：调用点经指针实时解引用
mov    ecx,DWORD PTR [r8]   ; r8 持有闭包内 int* = p，取 *p(已被 bump 改写)
call   sink(int)
```

**关键发现**

1. **闭包体积**：按引用捕获持指针（8B）是按值捕获值（4B）的 **2 倍**；无捕获仅占位 1B。高频捕获大对象（结构体）按值会复制整个对象进闭包。
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(a) 绝对耗时（随机器而变，仅作量级参考）">
  ...
  <line x1="80" y1="259.2" x2="640" y2="259.2" stroke="#C44E52" stroke-width="1.2" stroke-dasharray="5 4"/>
  <text x="640" y="255.2" text-anchor="end" font-size="10.5" font-family="Georgia, serif" fill="#C44E52">基线 8.22ms</text>
  <rect x="188.0" y="259.2" width="64.0" height="40.8" fill="#9A9A9A"/>
  <text x="220.0" y="253.2" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#9A9A9A">8.22ms</text>
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">模板 + 捕获 lambda（内联，基线）</text>
  <rect x="468.0" y="166.8" width="64.0" height="133.2" fill="#C44E52"/>
  <text x="500.0" y="160.8" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">26.86ms</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">std::function + 捕获 lambda（类型擦除）</text>
</svg>
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(b) 相对倍数（可移植信号：基准=1.00×）">
  ...
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">模板 + 捕获 lambda（内联，基线）</text>
  <rect x="468.0" y="137.9" width="64.0" height="162.1" fill="#C44E52"/>
  <text x="500.0" y="131.9" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">3.27×</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">std::function + 捕获 lambda（类型擦除）</text>
</svg>

> 图注：捕获 lambda 装入 `std::function` 触发类型擦除（堆分配闭包 + 间接调用），比模板内联慢 **3.27×**；热路径优先模板或 auto 参数保留内联。
...
基准源码见库根 `_bench_d5_26_lambda.cpp`。
