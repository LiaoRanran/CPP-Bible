...
> 标准基：ISO/IEC 14882:2023（C++23）｜预计阅读：7 h｜前置：ch19（存储期/链接/ODR）、ch20（引用与指针悬垂）、ch21（const/volatile 与写 UB）、ch25（联合 active member UB）、ch27（reinterpret_cast 与严格别名）、ch40（异常安全）、ch42（严格别名与优化）、ch61（并发数据竞争）｜后续：ch42（严格别名）、ch61（数据竞争）｜难度：★★★★★｜层级：L2 进阶
...
## 附录 J：对象生命周期与未定义行为（UB）决策流（D3 维度）

```mermaid
flowchart TD
    S["对象从哪里来：变量 / 临时量 / new / 全局"] --> D1{"存储期判定"}
    D1 -->|"自动存储期"| A["自动存储期：作用域结束自动析构"]
    D1 -->|"静态或线程存储期"| ST["静态 / 线程存储期：程序生命周期内存活"]
    D1 -->|"需要跨作用域共享"| DY{"是否跨作用域共享"}
    DY -->|"否"| A
    DY -->|"是"| N["动态存储期：new 分配堆对象"]
    N --> OW{"所有权由谁持有"}
    OW -->|"RAII"| R["unique_ptr / shared_ptr 持有所有权"]
    OW -->|"裸指针"| H["手写裸指针（易悬垂 / 泄漏）"]
    R --> AC{"是否跨线程访问"}
    H --> AC
    AC -->|"否"| USE["正常使用对象（读 / 写 / 取址）"]
    AC -->|"是"| DR{"有同步保护"}
    DR -->|"否"| UB1["data race 导致未定义行为"]
    DR -->|"是"| USE
    USE --> SA{"是否做类型双关"}
    SA -->|"否"| CHK["编译期 / 运行期校验"]
    SA -->|"是"| UB2["strict aliasing 违规导致未定义行为"]
    CHK --> SAN["UB Sanitizer 捕获越界 / 泄漏 / 对齐"]
    SAN -->|"发现问题"| FIX["修复并回到来源复核"]
    FIX --> S
    UB1 --> FIX
    UB2 --> FIX
    H -->|"悬垂 / 重复释放"| FIX
```
> 决策流说明：以"存储期 → 所有权 → 并发与别名安全"为主线，任一环节触发 UB 都回卷到来源复核；RAII 是收敛所有分支的归口。

## 附录 K：对象生命周期与未定义行为（UB）知识图谱（D6 维度）

```mermaid
flowchart TD
    C1["对象生命周期"] -->|"启用"| C2["存储期：自动 / 静态 / 线程 / 动态"]
    C2 -->|"决定"| C3["所有权"]
    C3 -->|"实现为"| C4["RAII"]
    C4 -->|"消除"| C11["悬垂引用 / 指针"]
    C5["未定义行为 UB"] -->|"源于"| C6["strict aliasing"]
    C5 -->|"源于"| C7["data race"]
    C5 -->|"源于"| C8["类型双关 type punning"]
    C6 -->|"关联"| C8
    C9["UB Sanitizer"] -->|"检测"| C5
    C10["临时量生命周期"] -->|"是 C1 的特例"| C1
    C11 -->|"导致"| C5
    C12["移动与拷贝语义"] -->|"影响"| C3
    C2 -->|"区分"| C10
    C4 -->|"依赖"| C12
    C13["视图类型 std::string_view"] -->|"不拥有"| C3
    C14["生命周期延长规则"] -->|"约束"| C10
```
### K.1 概念依赖逐边解读
...
| 边 | 含义 |
| --- | --- |
| C1 → C2 | 生命周期概念落到具体存储期分类 |
| C2 → C3 | 存储期决定对象该由谁持有所有权 |
| C3 → C4 | 所有权在 C++ 中通过 RAII 实现 |
| C4 → C11 | RAII 自动释放，消除悬垂与泄漏 |
| C5 → C6 | UB 的常见来源之一是 strict aliasing 违规 |
| C5 → C7 | 无同步的并发访问产生 data race UB |
| C5 → C8 | 非标准类型双关触发 UB |
| C6 → C8 | strict aliasing 与类型双关本质相关 |
| C9 → C5 | Sanitizer 在运行期捕获各类 UB |
| C10 → C1 | 临时量生命周期是对象生命周期的特例 |
| C11 → C5 | 悬垂访问直接表现为 UB |
| C12 → C3 | 移动 / 拷贝改变所有权归属 |
| C2 → C10 | 存储期区分临时量与具名对象 |
| C4 → C12 | RAII 依赖移动 / 拷贝的语义正确性 |
| C13 → C3 | 视图类型不持有所有权，仅借用 |
| C14 → C10 | 生命周期延长规则约束临时量存活区间 |

### K.2 跨章闭环表

| 上游章 | 下游章 | 传递的知识 |
| --- | --- | --- |
| ch20 引用与指针 | ch28 | 引用 / 指针语义是悬垂与生命周期分析的基础 |
| ch21 const 与类型族 | ch28 | const 与别名规则直接影响 strict aliasing |
| ch27 类型转换 | ch28 | reinterpret_cast 是类型双关 UB 的高发区 |
| ch41 智能指针 | ch28 | unique_ptr / shared_ptr 是 RAII 的具体落地 |
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(a) 绝对耗时（随机器而变，仅作量级参考）">
  ...
  <line x1="80" y1="256.1" x2="640" y2="256.1" stroke="#C44E52" stroke-width="1.2" stroke-dasharray="5 4"/>
  <text x="640" y="252.1" text-anchor="end" font-size="10.5" font-family="Georgia, serif" fill="#C44E52">基线 8.86ms</text>
  <rect x="141.3" y="256.1" width="64.0" height="43.9" fill="#9A9A9A"/>
  <text x="173.3" y="250.1" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#9A9A9A">8.86ms</text>
  <text x="173.3" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 173.3 314.0)">栈分配 (RAII auto)</text>
  <rect x="328.0" y="165.4" width="64.0" height="134.6" fill="#C44E52"/>
  <text x="360.0" y="159.4" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">27.14ms</text>
  <text x="360.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 360.0 314.0)">unique_ptr 堆</text>
  <rect x="514.7" y="166.6" width="64.0" height="133.4" fill="#55A868"/>
  <text x="546.7" y="160.6" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#55A868">26.89ms</text>
  <text x="546.7" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 546.7 314.0)">裸 new/delete</text>
</svg>
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(b) 相对倍数（可移植信号：基准=1.00×）">
  ...
  <text x="173.3" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 173.3 314.0)">栈分配 (RAII auto)</text>
  <rect x="328.0" y="148.1" width="64.0" height="151.9" fill="#C44E52"/>
  <text x="360.0" y="142.1" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">3.06×</text>
  <text x="360.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 360.0 314.0)">unique_ptr 堆</text>
  <rect x="514.7" y="149.5" width="64.0" height="150.5" fill="#55A868"/>
  <text x="546.7" y="143.5" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#55A868">3.03×</text>
  <text x="546.7" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 546.7 314.0)">裸 new/delete</text>
</svg>

> 图注：堆分配（无论 `unique_ptr` 还是裸 `new`）比栈对象慢约 **3×**——差在分配/释放的固定成本，而非 RAII 本身（`unique_ptr` 零抽象开销）。
