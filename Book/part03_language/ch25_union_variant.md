...
### 0.1 起源（谁·何时·为何）
C 的 `union` 来自 1970 年代，让多个成员共用一块存储以省内存——代价是"读非活跃成员"是未定义行为，全靠程序员记住当前装的是谁。[史] Stroustrup 把它带进 C++，但 C++ 的对象模型（构造 / 析构 / 类型）让 union 越发危险：非平凡类型放进去，析构谁？[史][评]

### 0.2 关键转折（编年）
- **C++98**：union 对含非平凡构造的类型限制极严。[史]
- **C++11**：放宽匿名 union 与带非平凡成员的限制，但仍不管理"活跃成员"。[史]
- **C++17**：`std::variant`（带"活跃成员"记账、访问者模式、`valueless_by_exception` 状态）入标准，与 `std::optional` / `any` 共同组成安全值族。[史]
