...
## 附录 J：友元与访问控制 决策流（D3 维度）

```mermaid
flowchart TD
    A["定义新操作"] --> B{"需要访问<br>私有成员?"}
    B -->|否| C["定义为非成员<br>非友元函数"]
    B -->|是| D{"是否为对称双操作数?<br>如 + - == "}
    D -->|是| E["定义为非成员 friend<br>支持左操作数隐式转换"]
    D -->|否| F{"左操作数是否为<br>本类类型?"}
    F -->|是| G["定义为成员函数<br>this 即左操作数"]
    F -->|否| H["定义为非成员 friend<br>让左操作数可隐式转换"]
    C --> I{"是否仅需<br>单一类型重载?"}
    I -->|是| J["普通函数即可"]
    I -->|否| K["考虑模板 / 重载集"]
    E --> L{"是否破坏封装<br>暴露太多内部?"}
    L -->|是| M["改用接口函数<br>减少 friend"]
    L -->|否| N["保留 friend"]
    G --> O{"是否应声明为<br>const 成员?"}
    O -->|是| P["const 限定成员"]
    O -->|否| Q["非 const 成员"]
    M --> R["完成：封装安全"]
    N --> R
...
> 决策流说明：从是否需要私有访问切入，对称双操作数优先选非成员 friend 以支持左侧隐式转换；若友元暴露过多内部状态则回退到接口函数，形成封装安全的闭环。

## 附录 K：友元与访问控制 知识图谱（D6 维度）

```mermaid
flowchart TD
    T1["访问控制"] --> T2["public 接口"]
    T1 --> T3["private 成员"]
    T1 --> T4["protected 成员"]
    T3 --> T5["友元函数"]
    T3 --> T6["友元类"]
...
| 边 | 含义 |
| --- | --- |
| 访问控制 → private 成员 | 私有成员限定友元与成员的访问范围 |
| private 成员 → 友元函数 | 友元突破私有访问边界以访问内部状态 |
| 访问控制 → 成员函数 | public 成员是类对外常规接口 |
| 友元函数 → 对称运算符重载 | 双操作数对称运算符常用非成员友元 |
| 对称运算符重载 → 重载决议 | 重载集驱动运算符的选择 |
| 友元类 → 封装边界 | 友元类需谨慎以免破坏封装 |
| 封装边界 → 封装 | 封装是访问控制的最终目标 |
| 非成员函数 → 接口抽象 | 非成员函数构成松耦合接口 |
| 对称运算符重载 → 模板友元 | 模板友元处理泛型对称运算符 |
