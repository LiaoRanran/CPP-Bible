...
- **生物信息（BLAST / SeqAn）**：序列比对的核心 Smith–Waterman / Needleman–Wunsch 就是带罚分的「编辑距离/动态规划」，思想与 LCS 同源，每天在基因组比对里跑数十亿次。
- **地图与导航（Google Maps / 高德 / Waze）**：路径规划用 Dijkstra / A*（Dijkstra 的加权扩展）在路网图上求最短路径，是经典图算法在亿级节点上的工业落地。
