...
> 本节补可查证的真实项目引用（L2 文件级），每条均指向具体源码，是本章算法选型论断的工业佐证。

- **GCC libstdc++ `stl_algo.h`**：STL 算法全景——`std::sort`（introsort: L1940-L2010）、`std::find`（L185-L210）、`std::nth_element`（L4950-L5010）。对照本章 §② 算法分类体系。
  → <https://github.com/gcc-mirror/gcc/blob/master/libstdc++-v3/include/bits/stl_algo.h>
- **LLVM libc++ algorithm 族**：PDQSort（`sort.h`, L3880-L3980）、堆算法（`sift_down.h`）、二分（`lower_bound.h`）——独立头文件架构，其 `std::ranges::sort` 已在 LLVM 17+ 的部分 pass 中替代迭代器对。
  → <https://github.com/llvm/llvm-project/tree/main/libcxx/include/__algorithm>
- **Intel oneTBB `parallel_sort.h`**：本章 §⑨ 执行策略（`std::execution::par`）的工业先驱——工作窃取 + 分治。
  → <https://github.com/oneapi-src/oneTBB/blob/master/include/oneapi/tbb/parallel_sort.h>
- **Chromium `base::` 算法用法**：[chromium/chromium](https://github.com/chromium/chromium) —— Chromium 在热路径用 `std::ranges` 重写排序/查找，是「⑭ 选型」的工业范本；`base::STLSetDifference` 等封装即基于标准算法。
- **ClickHouse 查询执行器（并行排序）**：[ClickHouse/ClickHouse](https://github.com/ClickHouse/ClickHouse) —— ClickHouse 的聚合阶段用 `std::sort` + 并行执行策略做外部归并排序，对应「⑨ 并行策略」。
- **Boost.Algorithm（Boost 社区）**：[boostorg/algorithm](https://github.com/boostorg/algorithm) —— `boost::algorithm::is_sorted`/`hex` 等扩展是标准算法的工业补充，对应「⑪ 迭代器失效规则」之外的工具箱。
- **folly 并行排序（Facebook）**：[facebook/folly](https://github.com/facebook/folly) —— `folly::sort`/`folly::parallel_for` 体现「⑨ 执行策略」的工业落地。
- **跨章关联**：排序细节→ `Book/part08_algorithms/ch96_sorting.md`；堆算法→ `Book/part08_algorithms/ch98_heap.md`；理论→ `Book/part08_algorithms/ch101_algo_theory.md`。
- **常见陷阱**：`std::find` 在 `list` 上用非成员版本会退化为 O(n²)——本章 §①/§⑬ 的迭代器类别→复杂度选型决策是关键。
