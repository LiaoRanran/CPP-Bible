...
!!! note "类比：模板 = 按需刻章的模具"
    ...
    > 失效边界：真实模具对什么面团都压得出形；模板却在类型不满足其操作（如 `max_val` 要求 `T` 支持 `<`、可拷贝）时直接实例化失败——不会"压出畸形饼干"，而是编译错误。
...
模板常被当成"写一次、处处用"的语法糖，但**模板真正的身份是编译期代码生成器**——你写的是配方，编译器按配方在实例化点现做一份。不搞懂这个"生成"过程，你会把模板当宏用、把报错当玄学。本章要带着这五笔账往下读：

1. **「模板」「模板参数」「模板实参」「实例化」四个词，到底谁是谁？** 模板是配方，模板参数是配方里的占位符，模板实参是你填进去的料，实例化是编译器按配方+实参**现场生成一份具体代码**。这四者的关系一旦含糊，后面所有报错都读不懂。本章 ② 速查 + ③ 核心结构先把这四者钉死。
2. **隐式实例化 / 显式实例化 / 显式特化 / 显式实例化定义——四个"化"怎么区分？** 隐式实例化是编译器按需自动生成；显式实例化定义是你主动要求"现在就生成这份"；显式特化是"这份实参组合我不用通用配方、我另写一份"；三者触发时机与用途完全不同。本章 ④ 实例化机制 + ⑦ 标准把四者的边界讲清。
3. **两阶段查找（Phase 1 / Phase 2）为什么是模板报错难读的根源？** 不依赖模板参数的符号在**定义处**（Phase 1）就查，依赖模板参数的符号要等到**实例化点**（Phase 2）才查——所以同一个名字，模板里和实例化后可能指向完全不同的东西。本章 ④ 专门拆解这条"延迟查找"规则。
4. **从 mangled 符号能反推出什么？** 编译器把模板实例化"编码"进符号名（如 `_Z3addIiET_S0_S0_`），读得懂 mangled 名，你就能在链接错误里一眼看出"哪个实例化没生成、哪个实参类型对不上"。本章 ⑩ 汇编/符号证据用 GCC 15.3 的真实符号带你练这套眼力。
5. **非类型模板参数（NTTP）和模板模板参数，边界到底在哪？** NTTP 是"把值当参数"（如 `std::array<int, 5>` 的 `5`），模板模板参数是"把模板当参数"（如 `std::vector` 本身）。两者都让模板更灵活，也各自有严格约束（NTTP 类型受限、模板模板参数要匹配签名）。本章 ③ 核心结构 + ⑪ STL 模式给出能落地的边界判断。
