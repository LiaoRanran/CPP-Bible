...
// 文件：C:/Qt/Tools/mingw1310_64/lib/gcc/x86_64-w64-mingw32/13.1.0/include/c++/type_traits
// 行号：62（struct integral_constant）
...
// 文件：同上 type_traits，行号：2235（struct conditional）/ 2240（偏特化 false 分支）
...
// 文件：C:/Qt/Tools/mingw1310_64/lib/gcc/x86_64-w64-mingw32/13.1.0/include/c++/bits/utility.h
// 行号：164（struct integer_sequence）/ 172（make_integer_sequence）
...
**源码阅读路线**
- `type_traits` → `integral_constant`（行号：62）→ `conditional`（2235）→ `conjunction`（217）：看标准库如何用 TMP 搭建 type_traits。
...
底层: TMP vs constexpr的编译时间
  TMP factorial<10>::value → 10个模板实例化 → ~50ms
  constexpr factorial(10) → 1次函数调用(编译期) → ~5ms
  → 结论: constexpr比TMP快10×
...
- **后续依赖**：`Book/part05_oo/ch51_crtp.md`（第51章　CRTP 与静态多态（Curiously Recurring Template Pattern））—— 本章为其前置，建议后续延伸阅读。
...
**底层深度**：TMP 的关键瓶颈是编译器模板实例化内存。Boost.MPL 的 `mpl::fold<Sequence, Init, Op>` 在 50 元素序列上生成约 2,550 个中间类型（每个 `Op::apply<T,S>` 产生独立特化），GCC 13.1 `-ftime-report` 显示 ~450MB 实例化内存。等效的 `boost::hana::fold` 在 constexpr 函数中通过 `for` 循环惰性求值，仅需 1 个模板特化（`hana::fold_impl`），编译器内存 ~15MB。这就是 C++11/14/17 从"类型计算"转向"值计算"（`constexpr`）的根本驱动力——编译速度提升约 20–100×。
