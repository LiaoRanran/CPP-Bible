...
```cpp
// 文件：C:/Qt/Tools/mingw1310_64/lib/gcc/x86_64-w64-mingw32/13.1.0/include/c++/bit
// 行号：426（template popcount）
template <typename _Tp>
constexpr int popcount(_Tp __x) noexcept {
    return std::__popcount(__x);          // 行 289：调用 __builtin_popcount
}
// 行号：289（__popcount）
...
```cpp
// 文件：C:/Qt/Tools/mingw1310_64/lib/gcc/x86_64-w64-mingw32/13.1.0/include/c++/array
// 行号：200（operator[] non-const，C++17 起 constexpr）
...
2. `<bit>` `popcount`/`__popcount`（行 426/289）：constexpr 包装编译器内建的范式。
3. `<array>` `operator[]`（行 200）：constexpr 容器随机访问，支撑编译期建表。
...
- **后续依赖**：`Book/part01_history/ch05_cpp14.md`（第05章　C++14：小幅完善）—— 本章为其前置，建议后续延伸阅读。
...
- **相邻主题**：`Book/part06_templates/ch71_policy.md`（第71章　策略设计 Policy-Based Design）—— 编号相邻、主题接续。
