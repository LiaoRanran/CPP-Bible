...
理解标签分发的内存模型，关键在于"标签类型是空的"。**标签是空类型**：`true_type`/`false_type` 继承自 `integral_constant`（空），迭代器标签也是空 struct，它们构成的内存开销受 EBO 与空基类优化（ch52），实例（如 `true_type{}`）通常占 **0 字节**。
...
STL 把标签分发用到了"性能要命"的地方。**`std::advance` / `std::distance`** 内部靠 `iterator_category` 标签分发：随机访问迭代器直接走 `+= n`（O(1)），输入迭代器则退化为逐次 `++`（O(n)）——同一个接口，复杂度由标签在编译期钉死。**`std::copy` / `std::fill`** 对它更进一步：对 `random_access` 迭代器用 `memmove` 批量搬运（标签感知后的优化），对其他类别逐元素循环，把"能否整块移动"这个布尔性质也交给了标签。
...
标签分发的边界在哪里，要和它的三个近邻划清楚。**与 `if constexpr`（ch69）**：前者用多个 overload + 公共入口、靠"最具体匹配"在编译期选路；后者是单函数里用编译期分支、未取分支被丢弃。关键差别是——**迭代器层级只能靠标签表达**：标签继承链能让派生标签隐式向上匹配基标签重载（示例 13 的 `if constexpr` 写法对 forward 迭代器会错落到 else，缺了 O(1) 优化，而标签分发能按最具体选）。
...
反模式基本围绕"在错误的位置做本应编译期的决定"。头号问题是**用虚函数做本可编译期确定的分发**：当类型性质在编译期就已知时，却上运行时多态——vtable 查表 + 间接调用，白白丢掉 `-O2` 折叠机会（对比 ⑩ 的零开销证据），示例 18 正是这样一个反面载体。
...
标签分发在真实工程里最常见的落点，是"在编译期把类型性质翻译成最优实现"。**序列化库**是一个典型：按 `is_arithmetic<T>::type` 标签选路径——数值类型走二进制定长编码，复合类型走递归反射（示例 19），公共的 `serialize(T)` 入口隐藏了这条分支，新增类型不改入口。
...
易错点大多源于"标签是类型、不是值"这一特殊性的误用。**标签类型必须可默认构造**：`Tag{}` 这种转发要求标签有可访问的默认构造（空 struct 恰好都满足），含状态的标签则要传实例而非统一 `{}`。**`typename` + `::type`** 是模板内的高频翻车点（⑬ 已述），模板内取 `std::is_xxx<T>::type` 漏 `typename` 必报错。
...
这四个问题把标签分发与相邻机制的分界问清楚了。**"和普通函数重载有何不同？"**——普通重载靠实参的**值类型**区分；标签分发额外加一个"标签类型"形参，由类型萃取或标签层级在编译期决定，公共入口因此能隐藏分支。**"为什么迭代器标签要是继承链而不是 enum？"**——因为继承链让 `random_access` 能**隐式向上转型**匹配 `forward`/`input` 重载，重载决议自动选最具体；enum 表达不了这种"is-a"层级，示例 26 用 vector 迭代器演示了"向上也能匹配 base、但选最深的 random_access"。
...
把反模式反过来，就是一套可照抄的**最佳实践**，五条彼此呼应。**公共入口只做"取标签 + 转发"**，实现细节全放带标签重载的 `impl`（示例 27），接口因此干净、单测可分点。**布尔性质分发直接用 `std::is_xxx<T>::type`**，不要自己发明 `true/false` 标签——库内已有、兼顾一致性。**迭代器相关算法务必用标签分发按类别优化**，随机访问 O(1)、其余 O(n)，这是标准库同款做法。
...
性能账要分"运行期、编译期、体积、对比虚函数"四笔算。**运行期**：标签分发在 `-O2` 下**零分支、零间接调用**（⑩ 证据），与手写单路径性能等价；即便 `-O0`，也不过一次标签对象构造（空类型常被优化掉）加一次直接调用，仍优于虚函数（没有 vtable 查表）。**编译期**：标签类型在实例化时钉死，重载决议在编译期完成，运行期零路由计算。
...
- `[cppref:cpp/types/integral_constant]`（T1）cppreference `cpp/types/integral_constant` —— 离线 `C:\Users\ASUS\Desktop\cppb参考资料\cppreference\`
