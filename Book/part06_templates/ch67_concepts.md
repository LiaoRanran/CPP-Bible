...
> 用途：工业级讲解 C++20 Concepts 与 requires 子句，含手写 concept、标准库 concept 源码剖析、与 SFINAE 的 ABI 等价性、MinGW GCC 13.1.0 真实汇编证据。
> 作者：CPP-Bible 工程
> 版本：v3.0（2026-07-08）
...
```asm
; _Z12use_conceptsv （MinGW GCC 13.1.0, -O2）—— 约束分派已被编译期消除
_Z12use_conceptsv:
    sub     rsp, 24
...
```asm
; _asm_tpl_concepts_O0.asm 节选（MinGW GCC 13.1.0, -O0）
...
`std::integral` / `std::same_as` 的真实定义（文件：`C:/Qt/Tools/mingw1310_64/lib/gcc/x86_64-w64-mingw32/13.1.0/include/c++/concepts`，行号：100 `integral` / 62 `same_as`）：
...
- **源码阅读路线**：`<concepts>` 行号：100（`integral`）、62（`same_as`）；`ch66_sfinae.md`（SFINAE 对照）、`ch62_specialization.md`（偏特化是约束载体）、`ch65_type_traits.md`（trait 是 concept 的底层）、`ch68_tmp.md`（编译期计算）、`ch75_template_diag.md`（约束失败报错对比）。
