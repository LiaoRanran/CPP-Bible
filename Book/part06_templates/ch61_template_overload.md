...
```cpp
// 阶段1：建立候选集（名称查找 + 模板实参推导）
// 阶段2：筛选可行集（实参可隐式转换、无 SFINAE 失败）
// 阶段3：从可行集选最佳（按转换序列等级排序；模板按偏序打破平局）
```

```cpp
void log(int);                       // (A) 非模板
template <typename T> void log(T);  // (B) 泛化
template <typename T> void log(T*); // (C) 更特化
void use() {
    int x;
    log(42);     // 候选 A/B/C；A 非模板优先 → 选 A
    log(&x);     // 候选 B/C（A 不可行：int*→int 非隐式）；C 比 B 特化 → 选 C
}
```
...
```asm
; _asm_tpl_overload.asm 节选（MinGW GCC 13.1.0, -O2）
_Z1fi:                          ; f(int) 非模板
...
```cpp
#include <vector>
// std::swap 主模板 + 容器特化：vector 有更高效 swap（O(1) 交换指针）
std::vector<int> a, b;  std::swap(a, b);   // 选中 vector 特化版 swap
```

```cpp
// std::begin / std::end：对数组、容器、initializer_list 多候选
int arr[3];  std::begin(arr);              // 数组重载
```

```cpp
// std::distance：对随机迭代器 O(1)、其他 O(n) 多实现
```

```cpp
#include <utility>
// std::forward 与 std::move：通过引用折叠与重载实现完美转发
```
...
void proc(tag_int, int) { }  void proc(tag_str, const char*) { }
```

```cpp
// 约束重载：C++20 用 requires 区分
template <typename T> requires std::integral<T> void f(T) {}
template <typename T> requires std::floating_point<T> void f(T) {}
```

```cpp
// 尾置返回类型 + decltype 参与决议
template <typename T> auto f(T x) -> decltype(x + 0) { return x; }
```

```cpp
// 转发引用重载谨慎：会与拷贝/移动构造冲突，用 enable_if 约束
template <typename T> void wrap(T&&) requires (!std::is_same_v<std::decay_t<T>, Self>) {}
```
...
#include <cstddef>
#include <array>
// 案例：std::swap 对 std::array 的特化更高效（不逐元素交换控制块）
template <typename T, std::size_t N>
void swap(std::array<T,N>& a, std::array<T,N>& b) noexcept { a.swap(b); }
```

```cpp
#include <vector>
// 案例：序列化框架按类型分派
template <typename T> void serialize(T, Output&);          // 默认
template <> void serialize<int>(int, Output&);             // 特化更快
template <typename T> void serialize(const std::vector<T>&, Output&);  // 容器特化
```

```cpp
// 案例：RPC stub 生成器对 POD 与复杂类型用不同序列化重载
```
