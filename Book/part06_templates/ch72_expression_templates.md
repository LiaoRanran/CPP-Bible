...
在标准库里，ET 的落地有一个明显的分野：**数值容器 `std::valarray` 处在半用半不用之间，其余 STL 容器则刻意不用**。`valarray` 的运算符（`operator+` 等）按语义返回**新的 `valarray`**（立即求值），本身**不是** ET；但它的内部 `_Expr` 模板（如 `operator+=` 接受 `_Expr` 形参）保留了一部分惰性优化空间。标准对它的选择是"返回 `valarray`"，图的是语义简单、结果可预期（见 ⑮ 对比）。
...
表达式模板很少独行，它的五种变体合起来才是工业里的真身。**ET + CRTP**（ch51/57）是标准骨架：`Expr<E>` 用 CRTP 向下转型提供静态 `operator[]`/`size()`，免虚函数、零开销地给节点统一接口。**ET + SIMD**（ch19/43）补上最后一块性能拼图：`operator=` 的单遍循环被 `-O2` 自动向量化为 AVX，Eigen 更进一步用手写 intrinsic 节点榨干带宽（衔接 ⑩ 的向量化观测）。
...
工业落地上，ET 的成色由 Eigen 与 Blaze 这类数值库兑现。**Eigen** 把 `MatrixXd C = A * B + D * E;` 编译为单个 kernel 并自动向量化，全程零中间矩阵——ET 正是 Eigen 性能的核心；**Blaze** 与之同源，并在 ET 之上叠加"智能表达式优化"（依据矩阵结构选择最优求值顺序）。二者还常与 140 章 Policy-Based Design 的 policy 组合联用，定制表达式求值策略（对照 ⑫）。
...
ET 的易错点，根子都在"代理是借来的生命周期"。**代理生命周期**是第一铁律：`a+b+c` 产生的 `Sum` 代理只持有操作数的 `const` 引用，必须在操作数存活期间求值（立即 `u = ...` 或显式 `eval`），绝不能让代理越过操作数作用域存活（⑬）。由它直接推出第二个陷阱 **`auto` 捕获代理**：`auto e = a + b;` 把 `Sum` 代理存进 `e`，一旦 `a`/`b` 后续被修改或析构，`e` 求值结果即出错或悬垂（示例 29）——应立刻 `u = e` 或让 `auto u = a + b;` 中的 `u` 物化为具体类型。
...
四个 FAQ 把 ET 的能力边界说圆。**「ET 为什么能消除临时对象？」**——因为 `operator+` 返回的是**代理**（只存操作数引用，约 16 字节），不构造结果数组，直到 `operator=` 才单遍写入目标，中间数组全部跳过（⑩）。**「ET 和 valarray 的 `_Expr` 一样吗？」**——不完全：valarray 运算符按标准返回**新 `valarray`**（立即），`_Expr` 只是内部有限优化；ET（Eigen 式）的 `operator+` 返回延迟代理，才真正做到零临时。
...
最佳实践就是一套从骨架到调用面的稳健套路。**接口层**：`Expr<E>` 基类用 CRTP 提供静态接口（`operator[]`/`size()`），`Sum`/`Prod`/`Scale`/`Const` 等所有节点统一继承它（⑫）；`operator+`/`operator*` **按值返回代理**且代理只持 `const` 引用（不拷贝操作数）——正是 ⑬ 强调的合法返回形态。**赋值与调试层**：`operator=` 接收 `const Expr<O>&` 单遍遍历求值，并暴露 `.eval()` 显式物化便于调试（示例 32/33）。
...
性能的一头是**运行期收益**：ET 把 `u = a + b + c` 的**分配从 5 次降到 4 次、遍历从 3 次降到 1 次**（⑩ 实测），且单遍循环可被自动向量化（AVX），大向量上加速显著（Eigen 可达数倍）。加速的机理在**内存带宽**：单遍遍历对缓存友好（ch43），减少中间数组的反复读写——朴素实现多遍把同一数据读进读出（示例 34/35）。
