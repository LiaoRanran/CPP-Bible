...
⟶ Book/part11_source/ch128_boost.md（Boost 库生态）—— Boost.uBLAS 是另一个 ET 数值线性代数实现
⟶ Book/part12_patterns/ch140_policy_pattern.md（Policy-Based Design）—— Blaze 用 policy 组合定制表达式求值策略
...
⟶ Book/part11_source/ch124_libstdcxx.md（libstdc++ 实现剖析）—— 标准库容器的运算符语义在此统一实现
...
⟶ Book/part14_perf/ch153_cpu_micro.md（CPU 微架构与微基准）—— ET 加速需用微基准如实测量化，避免印象式估算
⟶ Book/part14_perf/ch156_compiler_opt.md（编译器优化）—— 单遍循环能否被自动向量化取决于循环形式与别名分析
...
- **Eigen**（GitLab 官方仓库 `libeigen/eigen`）是表达式模板的工业级标杆：矩阵/向量运算 `u = a + b * c` 被编码成惰性类型，赋值瞬间融合成单个循环，性能追平甚至超过手写循环，被广泛用于机器人、CV、数值仿真。
- **Blaze、Armadillo、Boost uBLAS**：同属表达式模板数值库阵营；Blitz++ 是鼻祖。
- **Boost.Spirit**：把表达式模板思想用到解析器组合子，「写一套 EBNF 风格的中缀表达式」在编译期生成递归下降解析器，是 ET 在非数值领域的经典应用。
- **自动微分（Sacado，Sandia 国家实验室）**：用表达式模板实现前向/反向 AD 的惰性导数累加，被 PDE 约束优化、气候建模等 HPC 科学计算广泛采用。
- **有限元（deal.II）**：张量与线性代数运算采用 ET 式惰性求值，把多重循环融合成单趟遍历，是学术/工业级有限元仿真的内核技术。
