...
// 行号：746（operator= 拷贝赋值模板）
vector& operator=(const vector& __x);     // 立即逐元素拷贝，非延迟
// 行号：868/878（begin()）/ 888/898（end()）：非 ET 遍历基础，返回迭代器
...
```cpp
// 文件：C:/Qt/Tools/mingw1310_64/lib/gcc/x86_64-w64-mingw32/13.1.0/include/c++/valarray
// 行号：439（operator+= 成员，返回 valarray&，就地立即修改）
