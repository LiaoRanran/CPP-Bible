...
- **电信/网络设备（DPDK 数据面）**：DPDK 的 `rte_ring`（无锁环形队列）跑在用户态网络数据面，用 CAS + 内存屏障实现多生产者/多消费者无锁收发，是 5G 基站与路由器转发面低延迟的底座。
- **区块链/共识节点（状态机复制）**：共识节点为追求吞吐与确定性，用无锁队列在「网络收包 → 共识 → 状态机应用」之间传递消息，避免锁在竞争下造成确定性抖动。
