...
## 附录 J：协程 vs 回调 vs 多线程 选型 决策流（D3 维度）

```mermaid
flowchart TD
    S0["起点：需要异步/并发执行"] --> D1{"是否 IO 密集且大量并发?"}
    D1{"是否 IO 密集且大量并发?"} -->|"是"| D2{"是否需线性化异步代码?"}
    D1{"是否 IO 密集且大量并发?"} -->|"否"| D3{"是否 CPU 密集?"}
    D2{"是否需线性化异步代码?"} -->|"是"| A1["协程 co_await"]
    D2{"是否需线性化异步代码?"} -->|"否"| A2["回调 callback"]
    D3{"是否 CPU 密集?"} -->|"是"| D4{"是否需真并行?"}
    D3{"是否 CPU 密集?"} -->|"否"| A3["协程或回调皆可"]
    D4{"是否需真并行?"} -->|"是"| A4["多线程 std::thread"]
    D4{"是否需真并行?"} -->|"否"| A5["协程单线程事件循环"]
    A1 --> D5{"是否需取消/超时?"}
    D5{"是否需取消/超时?"} -->|"是"| A6["协程 + 取消帧"]
    D5{"是否需取消/超时?"} -->|"否"| A7["裸协程即可"]
    A2 --> END["结束：模型选型确定"]
    A3 --> END
    A4 --> END
    A5 --> END
...
> 决策流说明：IO 密集且海量并发首选协程，用 co_await 把回调地狱线性化；需要真并行算力的 CPU 密集任务必须上多线程；回调仅在异步逻辑极简、不愿引入协程栈开销时保留。协程并非免费——它带来帧生命周期与取消复杂度，且仍由单线程事件循环驱动时并不提供并行。

## 附录 K：协程 vs 回调 vs 多线程 选型 知识图谱（D6 维度）

```mermaid
flowchart TD
    C1["异步执行"] --> C2["协程 coroutine"]
    C1 --> C3["回调 callback"]
    C1 --> C4["多线程 thread"]
    C2 --> C5["co_await 挂起"]
    C5 --> C6["promise / future"]
    C6 --> C7["协程帧生命周期"]
    C3 --> C8["回调地狱嵌套"]
    C4 --> C9["数据竞争"]
    C9 --> C10["互斥/原子同步"]
...
| 上游概念 | 下游概念 | 依赖含义 |
| --- | --- | --- |
| 异步执行 | 协程 coroutine | 协程以挂起表达异步 |
| 异步执行 | 回调 callback | 回调以函数表达异步 |
| 异步执行 | 多线程 thread | 线程以并行表达并发 |
| 协程 coroutine | co_await 挂起 | await 让出执行权 |
| co_await 挂起 | promise / future | 挂起点连接产出 |
| promise / future | 协程帧生命周期 | 帧存活覆盖 resume |
| 回调 callback | 回调地狱嵌套 | 回调嵌套难维护 |
| 多线程 thread | 数据竞争 | 共享态并发致竞争 |
| 数据竞争 | 互斥/原子同步 | 同步消除竞争 |
| 协程 coroutine | 事件循环驱动 | 协程由事件循环调度 |
| 协程帧生命周期 | 移动唯一所有权 | 句柄移动且禁用拷贝 |
| 事件循环驱动 | 单线程高并发 | 事件循环支撑海量 IO |
| 互斥/原子同步 | 并行加速 | 同步下安全并行 |
| 移动唯一所有权 | 避免 double-destroy | 唯一句柄防重复销毁 |

### K.2 跨章闭环表

| 上游章 | 下游章 | 传递的知识 |
| --- | --- | --- |
| ch113 | ch120 | 协程应用层模式复用协程选型 |
| ch93 | ch113 | std::async 是协程前身范式 |
| ch108 | ch113 | 协程帧同步靠内存序 |
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(a) 绝对耗时（随机器而变，仅作量级参考）">
  ...
  <text x="640" y="256.8" text-anchor="end" font-size="10.5" font-family="Georgia, serif" fill="#C44E52">基线 3.16ms</text>
  <rect x="188.0" y="260.8" width="64.0" height="39.2" fill="#9A9A9A"/>
  <text x="220.0" y="254.8" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#9A9A9A">3.16ms</text>
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">普通函数累加（基线）</text>
  <rect x="468.0" y="162.9" width="64.0" height="137.1" fill="#C44E52"/>
  <text x="500.0" y="156.9" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">11.06ms</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">协程 Task 累加</text>
</svg>
...
<svg viewBox="0 0 680 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="(b) 相对倍数（可移植信号：基准=1.00×）">
  ...
  <text x="220.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 220.0 314.0)">普通函数累加（基线）</text>
  <rect x="468.0" y="126.4" width="64.0" height="173.6" fill="#C44E52"/>
  <text x="500.0" y="120.4" text-anchor="middle" font-size="11" font-weight="bold" font-family="Georgia, serif" fill="#C44E52">3.50×</text>
  <text x="500.0" y="314.0" text-anchor="end" font-size="10.5" font-family="Georgia, serif" transform="rotate(-32 500.0 314.0)">协程 Task 累加</text>
</svg>
...
基准源码见库根 `_bench_d5_113_coroutine.cpp`。
