...
- **P2-3 校准数据采集（放权的前提，468 杠杆 4）**：不只是成本 trace。完整定义 = ①每颗原子 token 消耗（机器侧+红队侧估算）②红队裁决留痕（每条阻断/高级/建议的最终处置：采纳/驳回+理由/误判）③L1 自检拦截率（writer_selfcheck 拦了多少、漏了多少被红队抓到）④**假阴性率**（机器放行后，人审或后续对抗发现错了的比例）。448 的核心结论：放权的前提是"机器判断与人类判断的一致率"可量化——当前这个数据是 0，所以"自动化"是赌博。10 颗原子后输出第一份校准报告，所有后续放权决策（红队频率、规则质量、自治边界）都基于这份数据。**关键认知（466 中期调整 3）**：ccache 省的是编译时间（秒级），红队 token 才是最大成本（分钟级——一次红队子 agent 53 次工具调用）。成本优化的真正杠杆是**减少红队轮次**（错误左移+红队分层触发），而不是加速编译。trace 数据要能回答"红队占单颗总成本的百分比"，这个数字下降才是真优化。
