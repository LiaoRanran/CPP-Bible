# 470 · 今晚 Agent 干活报告 v2.6（系统安全地基与范式升级）
...
> **版本**：v2.6（十六轮打磨 + 六轮深度内卷批判：v1 初始 → v1.1 系统全貌 → v1.2 五维模型 → v1.3 九漏洞 → v1.4 执行协议 → v1.5 466消化 → v1.6 468消化 → v1.7 461消化 → v1.8 462/383/389消化 → v1.9 内卷修正 → v2.0 305/372消化 → v2.1 内卷批判20项修9项 → v2.2 执行路径模拟修7项 → v2.3 对抗视角+偷懒预防修8项 → v2.4 失败恢复+CI交互修6项 → v2.5 验收标准审查修6项含1真实错误 → v2.6 元规则+降级策略+可执行性歧义修5项）。执行中若本文被外部修改，以你开工时 Read 的版本为准，不要中途换版本。
...
- poison + pytest 绿
- **性能基线**：记录开启重编译后全量 replay（56 卡×2 平台）的耗时。预期从 ~2min 增至 ~5-8min（每次多一次编译）。如果超过 10min，考虑只对 artifact_sha256 与磁盘工件匹配的卡做重编译（不匹配的直接 refute，不需要重编译）——但注意这会留下"攻击者填正确 sha"的窗口，所以默认全量重编译，性能优化留 P2。
