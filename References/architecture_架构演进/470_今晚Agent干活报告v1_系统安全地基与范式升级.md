...
> **版本**：v2.7（十七轮打磨 + 七轮深度内卷批判：v1 初始 → v1.1 系统全貌 → v1.2 五维模型 → v1.3 九漏洞 → v1.4 执行协议 → v1.5 466消化 → v1.6 468消化 → v1.7 461消化 → v1.8 462/383/389消化 → v1.9 内卷修正 → v2.0 305/372消化 → v2.1 内卷批判20项修9项 → v2.2 执行路径模拟修7项 → v2.3 对抗视角+偷懒预防修8项 → v2.4 失败恢复+CI交互修6项 → v2.5 验收标准审查修6项含1真实错误 → v2.6 元规则+降级策略修5项 → v2.7 隐性假设+输入依赖+回归风险修4项）。执行中若本文被外部修改，以你开工时 Read 的版本为准，不要中途换版本。
...
├── tests/                                       # pytest（test_*.py 数量以实跑为准，苦力 430 后新增了多个）
...
**Writer 自检 7 项具体清单（461，全部来自本仓踩过的坑）**——好模型执行 P0 时必须知道 L1 已覆盖了什么，避免重复造轮子：

| # | 自检项 | 来源教训 | 与今晚 P0 的关系 |
|---|---|---|---|
| W1 | 断言字面量必须在 Linux 工件中 grep 得到 | 040/041：断言写运行时拼接标签，Windows 走 sha 静默跳过，Linux refute | P0-A 的前置：重编译前先确认断言在工件中存在 |
| W2 | 夹具注释不写带双引号的完整取值串 | S3 规则抓源码所有双引号字面量含注释，注释里写旧值被误判 | 与 P0 无直接关系，写夹具时注意 |
| W3 | 性能卡 command 不含性能打印（须 -DBENCH_FULL 宏门控双构建） | EV-MEM-045：逐轮纳秒进 run_match 逐字比对，任何机器必 refute | P0-F 零诊断的同族问题 |
| W4 | run_match_keys 不含环境量（nproc/hardware_concurrency/cpu_count） | d8ff94d：hardware_concurrency 进 keys，本地 32 核绿 CI 核数不同必 refute | **= P0-E 的规则化**，P0-E 就是把 W4 从自检升为 gate 规则 |
| W5 | 自旋夹具必须有界（volatile 自旋 + 上界） | CONC-001：真无限循环 replay 超时 600s → refute + CI 白挂 10 分钟 | 与 P0 无直接关系，写夹具时注意 |
| W6 | 元数据口径统一（覆盖所有分配器自身占用） | ALLOC-002：Arena 只算 sizeof、Pool 漏算 8000B 堆数组，方向反转 | 红队/L4 问题，L1 只能 warn |
| W7 | claim 锚方向不锚倍数（含倍数须声明变量域+证伪条件） | PERF-004：同机跨运行倍数 18.86×→8.78×，波动 2×+ | 红队/L4 问题，L1 只能 warn |

**关键认知**：W1/W3/W4/W5 可写成确定性 gate 规则（零 LLM），W2/W6/W7 是建议级。今晚 P0-E 就是 W4 的规则化——你做 P0-E 时不要重新设计，直接把 writer_selfcheck 的 W4 检测逻辑搬成 gate 规则即可。

> 验证金字塔只是系统**五维成熟度**中的"验证"一维。完整框架见第五节：验证（较强）、信任（弱）、度量（弱）、消费（弱）、进化（最弱）。今晚 P0 只加固验证维，其余四维是后续架构批次。

> **成本与左移（468 洞察 3，理解今晚 P0 为什么重要）**：验证层的成本是指数级的——L1 自检拦截一个错误 = 0 token；L2/L3 机器拦截 = 0 token（机器时间）；L4 红队拦截一个口径错误 = ~20000 token（53 次工具调用）；L5 人审 = 用户最稀缺的注意力。第五批三个方向性错误（ALLOC 口径反转/LEAK 观测翻转/PERF 恒真读数）全部在 L4 才被抓到。**今晚 P0 的本质就是"左移"**：P0-A（重编译不变量）把 L4 才能发现的"工件被篡改"下沉到 L3 机器自动检测；P0-B/C/E/F（内容规则）把 L4 才能发现的"cat 证据/恒真断言/环境依赖"下沉到 L2 gate 规则。每下沉一项，红队就少一项负担。这也是为什么 P0 优先做内容/时序规则而不是机械规则——机械规则已经过剩，内容规则才是 L4 红队的主要负担。
...
- **改完工具后立即验证可加载**：每次修改 `gate_engine.py`/`atom_evidence_replay.py`/`poison_drill.py` 后，**立即**跑 `python tools/gate_engine.py --check`（或对应工具的 `--help`/空跑）确认模块能正常导入。苦力 430 遇到过"插入大段代码后破坏了 check_relations_dag 循环导致加载报错"的坑——不要等写完所有代码再验证，每改一个函数就验证一次。如果导入报错，用 `git checkout -- <文件>` 恢复到上一个可加载状态，不要在坏状态上继续堆代码。
...
**WSL 不可用时的降级**：本文多处要求"双平台验证"（Windows + WSL）。如果你的环境没有 WSL（或 WSL 不可用），**只做 Windows 侧**，WSL 侧标注"环境不可用，交 CI 验证"。不要因为 WSL 不可用就跳过整个 P0——Windows 侧的验证仍然有价值。CI（Ubuntu）会覆盖 WSL 侧的验证。在 worklog 中明确列出哪些验证只做了 Windows 侧。

---

## 二、苦力 430 成果衔接（站在它肩膀上，不要重做）

**⚠️ 编号体系澄清（执行前必读，避免混淆）**：本项目历史上有三套独立编号，含义不同但字母重叠：
- **452 对抗探针**：`E01`–`E16`（第四轮对抗发现的逃逸，在 `_adv_v70/probes/`）
- **414 修复方案**：`F01`–`F16`（414 文档设计的修复项，部分被苦力实现）
- **苦力 430 实现**：也用 `F01`–`F09`（苦力实际落地的修复，与 414 的 F 编号大致对应但不完全一致）

本文中**提到"Fxx"时默认指苦力 430 的实现**（如"苦力的 F02"），提到 414 的修复方案时会写"414 Fxx"。提到逃逸时用"452 Exx"。如果看到单独的"F02"且上下文不明确，默认是苦力 commit `e9094d7` 的实现。
...
**最坏情况预案**：如果 P0-A 和 P0-D 都触发回滚（A 降级为 warn、D 不硬切），今晚的两个范式级任务都没做成。此时不要慌——把规则级 B/C/E/F/G 全部做完（这些是独立的，不依赖 A/D），H 只沉淀已被规则级修复拦住的探针（E03/E05/E07/E10/E12），E01 探针标注"P0-A 未闭合，暂不沉淀"。收工时明确写"范式级两项未闭合，规则级五项已闭合"，交人裁决下一轮是否继续攻 A/D。**不要因为 A/D 没成就放弃整个晚上**——规则级五项本身就有实质价值。

**总时间不足时的砍掉顺序（从后往前砍）**：如果整体时间不够（如只剩 1 小时），按以下优先级保留：P0-A（治最严重的 E01 编译后覆写，范式级）> P0-D（治 E07 缩进走私，范式级）> P0-B（cat 证据）> P0-C（判别力）> P0-E（环境量）> P0-F（零诊断）> P0-G（签收+并发）> P0-H（探针沉淀，依赖前面）。被砍掉的 P0 在 worklog 中标注"时间不足未做，设计已在本文中，下一轮执行"。**不要为了赶进度把 P0 做成半成品**——宁可少做 2 个 P0，也要把做了的验证完整。

**模型降级预案（468 攻击 4）**：你是好模型，但白嫖额度可能在执行中途耗尽或被限流。如果发生：
- **不要中断当前 P0**——把当前做到一半的任务的进度、已改文件、未完成项写进 worklog，然后停手。
- **L0-L3 机器层不受影响**——gate/replay/poison/pytest 是纯机器工具，任何模型都能跑。
- **降级顺序**：P0-A（范式级，需要推理）→ P0-D（兼容性评估，需要判断）→ 其余规则级（机械实现，差模型也能做）。如果额度只够做 3 个，优先 P0-B/C/E（规则级，快做快过），把 P0-A/D 留给下一轮好模型。
- **绝对不要**因为额度不够就"简化"P0-A 的实现（比如跳过重编译直接比对 sha）——那会制造假阳性。宁可标记"未完成"也不要做半成品。
...
**EIR 检查（389：自纠错引入新错误率）**：每修一个问题，不仅要验证"问题修好了"，还要验证"没有引入新错误"。本仓真实案例：ALLOC-002 修了口径统一，结果 bitmap reset 不一致（新错误）；LEAK-002 修了 H2，结果观测反转（新发现）。检查点的 `gate --check` 不仅看 block=0，还要看 **warn 数是否增加**——新增 warn 可能是新规则暴露了存量真债（可接受），也可能是你的修复引入了误伤（必须修）。如果 warn 数增加，逐条判断是真债还是误伤，不要"block=0 就完事"。

### 3. 探针可用性前置验证（开工第 0 步，在 P0 之前）

`_adv_v70/probes/` 里的探针是 452 对抗时（基线 9f9652c）构造的，之后苦力改了 F01/F02/F03 等工具，**探针可能已经不能复现了**。开工后、做任何 P0 之前：

**⚠️ 探针目录是 untracked 的**：`_adv_v70/` 不在 git 仓库中（按惯例不入库）。如果你在一个**新克隆的干净环境**中工作，这个目录不存在。先 `ls _adv_v70/probes/` 确认——如果不存在，从原工作副本复制过来，或让用户提供。不要假设探针一定在磁盘上。如果探针确实不可得，跳过前置验证，直接按本文的 P0 优先级执行（E01→A、E03→C、E05→B、E07→D 的对应关系已在本文中写明），在 worklog 中注明"探针目录不可得，未做前置验证"。

1. 选 E01/E03/E07 三个阻断级探针，分别在当前代码上实跑一次（gate + replay）。**探针是 markdown 描述文件（含攻击方式、预期结果、复现步骤），不是可直接运行的脚本**——你需要先 Read 探针文件了解攻击方式，然后根据描述在临时目录构造测试卡/夹具来复现。不要试图"运行"探针文件。
...
8. **把跨编译器输出差异误判为工件篡改（462 差分测试警示）**：P0-A 重编译不变量是"同一编译器、同一档位、同一输入，两次编译输出一致"。**不要**把"MinGW 输出和 WSL g++ 输出不同"当成 refute:artifact_tampered——不同编译器的代码生成本就不同，这是正常的。跨编译器差异更可能是 UB 嫌疑（-O0 vs -O2 不一致 = UB 嫌疑），而不是工件被篡改。P0-A 只比对同一编译器的两次编译，跨编译器比对走 artifact_assert 路径（已有机制）。
9. **用之前跑过的门禁结果冒充 fresh run**：收工时的门禁必须是所有 commit 之后重新跑的。不要说"replay 56/56"但实际上是 3 个 commit 之前跑的——最后一个 commit 可能改变了数字。如果 P0-A 重编译导致全量 replay 超 10min，可以用 `--fast` 但必须注明。
10. **探针复现不了就跳过整个前置验证**：探针可能因为环境变化/苦力修复而无法复现，这是正常的。换备选探针（E05/E10/E12），或在 worklog 中注明"探针不可得，按本文 P0 优先级执行"。不要因为"探针复现不了"就得出"452 的攻击都被修好了"的结论——那是没有证据的判断。
...
- **临时目录必须清理**：用 `tempfile.mkdtemp()` 创建的目录，在 replay 进程退出前用 `shutil.rmtree(tmpdir, ignore_errors=True)` 清理。可以用 `try/finally` 包裹，确保即使编译失败也清理。长期不清理会导致系统 temp 目录积累大量编译产物（56 卡×2 平台×每次几百 KB = 每次 replay 几十 MB）。

### 6. 上下文管理纪律（383，直接影响你执行 8 个 P0 的质量）

好模型执行长任务时，上下文会被工具输出（git status、门禁全量输出、编译日志）污染，导致后面的 P0 质量下降。**PRISM 实证：13× 上下文缩减同时带来 35pp 评判增益——精准的少量信息比塞满整个窗口更有效。**

- **工具输出只留结论**：门禁全跑输出只保留结论行（`replay confirm=56`、`gate block=0 warn=32`、`poison 50/50`），不要把完整逐卡输出留在上下文里。需要细节时重新跑或查文件。
- **按需 Read，不预读**：做 P0-A 时只 Read `atom_evidence_replay.py` 的相关函数，不要预先把 `gate_engine.py`/`poison_drill.py` 全读进上下文。改哪个工具 Read 哪个。
- **每完成一个 P0，输出 200 字摘要**：关键结论（改了什么、门禁结果、未完成项）写进 `_worklog_470.md`，然后"忘记"该 P0 的细节——下一个 P0 不需要上一个 P0 的完整编译日志。
- **70% 阈值主动固化**：当你感觉上下文接近上限（系统提示上下文过大、或你发现自己在重复查找信息），主动输出"固化摘要"（关键决策+未完成项+具体 sha/数字），写进 worklog，然后继续。不要等系统报错才压缩。
- **压缩必须保留关键细节**：通用摘要会丢失"这个 sha 是改夹具后的新 sha"这类关键信息。固化时必须保留：具体 sha/读数、未解决的阻断项、人审裁决结果。
...
**规则级任务（B/C/E/F）的通用回滚条件**：若实现某规则时发现需要改 replay 核心逻辑（如改 run_match 解析、改 artifact_assert 评估流程）而非只加 gate 规则函数，立即停止该任务，降级为 P1 并记录——规则级任务的设计前提是"只加 gate 规则 + 毒样例 + pytest"，动 replay 核心说明问题比预期复杂，需要单独设计。若规则命中存量卡 >5 张且无法区分真问题/误伤，先降 experimental 观察，不要硬升 warn/block。
...
8. **结构化比对的定位（明确：是全文件 sha 不稳定时的降级判据，不是二次确认）**：P0-A 的主判据是"重编译全文件 sha == 卡内 artifact_sha256"。只有当加了确定性 flag 后仍有卡全文件 sha 不稳定时，才对这些卡改用结构化比对。结构化比对一致 = confirm，不一致 = refute。**不要**在全文件 sha 已经不一致时再用结构化比对"二次确认"——那会放宽判据。全文件 sha 不稳定的卡集合必须在 worklog 中列出，交人裁决是否接受结构化比对作为这些卡的判据。
   - **方案 1（首选）**：比对符号表——用 `nm <工件>` 或 `objdump -t` 提取符号名+地址，排序后 diff。符号表比全文件稳定（不受路径/时间戳影响）。**Windows 侧可用性**：MinGW 自带 `nm.exe`/`objdump.exe`（在 `C:\Qt\Tools\mingw1530_64\bin` 或 MSYS2 `mingw64\bin`），先 `where nm` 确认路径。如果 Windows 侧没有 nm，方案 1 在 Windows 侧不可用，直接试方案 2。WSL 侧 `nm`/`objdump` 默认安装。
...
**验收**：
- **阳性对照**：构造 E01 探针（helper 脚本编译后覆写）→ replay 重编译 sha 不一致 → refute:artifact_tampered
- **阴性对照（必须有，防假阳性）**：选 3 张正常卡（工件未被篡改、command 是直接编译器调用）→ 重编译 sha 一致 → confirm 不退化。如果阴性对照也 refute，说明重编译逻辑有 bug（如 flag 注入错误、临时目录路径问题），不要上线。
- 现有 56 卡重编译 sha 一致（或差异可解释：确定性 flag 问题）→ confirm 不退化
- 并发跑 6 轮 replay 无假失败（临时目录隔离）
- poison + pytest 绿
- **性能基线 + 回滚条件**：记录开启重编译后全量 replay（56 卡×2 平台）的耗时。预期从 ~2min 增至 ~5-8min（每次多一次编译）。**如果超过 15 分钟**（CI 超时阈值通常是 20-30 分钟，留余量），触发性能回滚：选项①只对 artifact_sha256 与磁盘工件匹配的卡做重编译（不匹配的直接 refute，不需要重编译——但注意这会留下"攻击者填正确 sha"的窗口）；选项②把重编译设为 CI-only（本地 replay 跳过重编译，只在 CI 上跑）。选哪个在 worklog 中说明。**不要让 replay 超时导致 CI 全红**——性能回滚比"全量重编译但 CI 挂掉"好。默认全量重编译，性能优化留 P2。
- **CI 兼容性（必须考虑）**：P0-A 上线后第一批 CI 结果需要重点监控。CI 环境（Ubuntu Docker）与本地（Windows+WSL）的差异：构建路径不同（`-ffile-prefix-map` 应能处理）、编译器版本可能不同（CI 用 gcc-14，本地用 gcc-15.3/13.3）、可能没有 ccache。如果 P0-A 在本地稳定但 CI 全红，**不要在 CI 上禁用重编译**——先查是编译确定性问题（加 flag）还是环境差异（CI 的 gcc 版本生成不同代码）。如果是后者，P0-A 的重编译只比对同一编译器的两次编译，CI 用 gcc-14 重编译 gcc-14 的工件，应该一致——不一致说明工件是用不同编译器生成的（跨编译器工件），这本身就是值得 warn 的。在 worklog 记录 CI 第一批结果。

### P0-B【高】cat 式证据检测（452 E05）

**问题**：夹具 open 随库数据文件原样打印，输出逐字相等但零计算，run_match confirm。

**修复**：新增 gate 规则 `EV-FIXTURE-NO-ECHO-DATA`（先 `experimental` 只记录不输出，观察命中情况后升 warn，不直接 block——遵循铁律 12 渐进发布）。**experimental 的实现方式**：gate_engine 当前只有 block/warn/advice 三级，没有 experimental 级别。experimental = 规则函数内部收集命中信息（卡 ID + 匹配行号）打印到 stderr 或写入日志文件，但返回 `(0, 0, 0)` 不触发任何 warn/block。观察一轮（全量 56 卡跑一次，看命中哪些卡、是真问题还是误报）后，如果命中合理，改为返回 `(0, 1, 0)` 升 warn。不要为了 experimental 去改 gate_engine 的三级输出结构——那是 P2-2 的事，P0 阶段用"返回零但打印日志"的方式实现。
...
**验收**：E05 探针 warn；存量 56 卡若有同类，列出来交人裁决。**阴性样例（必须有）**：构造一个合法夹具（读文件后经过算术/函数调用计算再输出，如读入数字后 +1 再打印）→ 不被 warn。如果阴性样例也被 warn，说明正则太宽，需要收窄（如把"≤3行"改为"≤1行"或要求中间无任何变量赋值）。
...
- **contains_in**（断言"符号在某区间内存在"）：若符号在太多函数区间都出现，说明它是"背景噪音"，无法证明"该函数有某行为"。这才是"无判别力"。
  ...
  - `k/N ≥ 0.8` 且 `N ≥ 3` → block（新卡）/ warn（存量）。**"新卡"与"存量卡"的区分标准**：新卡 = 不在 `golden_state.json` 的 evidence 基线中（即 `golden_lock show` 列出的 evidence 列表里没有这张卡的 ID）；存量卡 = 在基线中。实现时 gate 规则接收一个 `is_new` 参数（由调用方根据 golden_state 判断），不要在规则内部硬编码路径判断。如果无法判断是否新卡（如 golden_state 缺失），默认按存量处理（warn），不要默认 block——避免误伤。
...
**实现提示**：先 Read `atom_evidence_replay.py` 中 contains_in/absent_in 的现有实现，确认符号区间切分函数名和调用方式。**先验证符号区间切分的准确性**——对 1-2 个工件手动检查函数边界是否正确（如 `main` 函数区间是否包含了 `main` 的全部指令、没有把下一个函数的指令算进来）。如果区间切分有 bug，先修 bug 再做判别力统计，否则统计基础不可靠。再在评估 contains_in 时插入频率统计。不要改 absent_in 的逻辑。

**验收**：
- E03 探针（contains `.cfi_startproc`，在所有函数区间出现）→ block
- 真实机制符号（lock xchg 只在特定函数、fence 只在 writer 函数）→ 放行
- **absent_in 不误判**：构造一个 absent_in 强断言（如 `absent_in: "lock"` 在一个纯计算函数区间）→ 判别力统计不触发（absent_in 不做频率统计），正常评估符号是否真的不在该区间
...
**实施步骤（谨慎，兼容性优先）**：
1. 先写一个**兼容性测试脚本**（不改动现有解析器）：用 safe_load 解析全量 evidence/（56 张）+ atoms/（27 颗，以实跑为准），把结果与现有自定义解析器的结果逐字段 diff。
2. 分类差异：①完全一致 → 安全切换；②safe_load 报错（依赖自定义解析宽容行为，如未加引号中文冒号）→ 列出具体卡，批量修正卡格式；③语义差异（重复键 after-wins 行为不同）→ 加重复键检测；④**类型差异**（自定义解析器把某字段转成了 int，safe_load 转成了 str，或反之）→ 这是最隐蔽的差异，逐字段 diff 时必须比较 `type(value)`，不能只比较 `str(value)`。如果类型差异 >3 张卡，在 gate 规则函数入口加类型归一化（如 `str(x).strip()`），不要改卡格式。
...
**验收**：构造环境量探针（证据卡 run_match_keys 含 `hardware_concurrency=32` 这类非确定性输出值）→ block；CONC-002 合法兜底（`insufficient_cores` 用于守卫不作为逐字 key）放行。**注意**：E07 是缩进走私探针，对应 P0-D，不是 P0-E——不要用 E07 验证 P0-E。
...
**G2 并发隔离**：replay 编译输出从固定 `Examples/atoms/` 改为每进程 `tempfile.mkdtemp()`，编译后原子写入（先 .tmp 再 rename）。若 P0-A 已用临时目录重编译，此处只需确保**主编译路径**也隔离——P0-A 的临时目录只用于重编译校验，主编译（产生 .asm/.out 的那次）仍可能写固定路径，必须一起改。建议实现时把"临时目录编译"做成 replay 的统一函数，主编译和重编译都调用它，避免两套路径。
...
**不需要重复沉淀的探针**（苦力已修复并配了毒样例）：E04（中文/全角 .out 键，苦力 F04）、E06（虚构 .out 留痕，苦力 F06）、E08（poison 退出码，苦力 gate_exit_code）。先 `poison_drill.py --list` 确认这些已有毒样例覆盖，不要重复造。E09（并发）是 replay 基础设施问题（P0-G2），不是 gate 规则能拦的，不配 poison 毒样例，用 pytest 并发测试覆盖。E11（冲突同义词）是 P0-D 的一部分，毒样例合并到 P0-D 的重复键/关系检测中。E16（闪卡 draft）是 P1-2 的功能问题，不配 poison。

**注意**：探针转化时去掉 `_adv_v70/` 的临时路径，用 poison 标准格式；每个毒样例必须验证"修复前能逃逸、修复后被拦"。

**E01 类探针的毒样例格式说明**：E01（helper 脚本编译后覆写）的攻击发生在 replay 执行 command 时，不是 frontmatter 静态检查能拦的。毒样例需要写一个**特殊的 command**：先正常编译，再用 `cp`/`python`/无参 helper 脚本把工件覆写为另一个文件，artifact_sha256 填覆写后的 sha。poison_drill 执行时跑这个 command + replay，验证 P0-A 重编译后 sha 不一致 → refute。如果 poison 框架不支持执行 replay（只跑 gate），则 E01 毒样例标注为"replay 级毒样例"，放在单独目录或用 pytest 模拟，不要硬塞进只跑 gate 的 poison YAML。
...
- **若对应 P0 任务回滚未闭合**（如 P0-A 降级为 warn、P0-D 不硬切），该探针的毒样例**无法验证"修复后被拦"**——跳过转化，标注"对应 P0 未闭合，待下一轮修复后沉淀"。不要硬造一个"验证 warn 级"的毒样例，毒样例的标准是"被拦"（block 或 refute），warn 不算拦住。

**验收（P0-H 必须有，不能只说"转化了"）**：
- `poison_drill.py` 全绿：新增的毒样例全部被对应规则拦截（exit 0）
- 每个新毒样例有对应的阴性样例（合法卡）在 pytest 中放行（反例）
- `poison_drill.py --list` 确认编号连续无跳号
- 毒样例是"最小攻击片段"（不是完整探针的复制粘贴）——用 `wc -l` 检查，每个毒样例 ≤30 行（探针可能 100+ 行，毒样例应裁剪到只含触发规则的部分）
- 在 worklog 列出每个转化的探针→毒样例编号映射表
...
- **P1-3 突变测试最小版（断言判别力的终极验证，461 MUT1）**：P0-C 是静态统计判别力（contains_in 的 text 在多少函数区间出现），突变测试是动态验证——对 C++ 夹具做 3 种突变（关系运算符翻转 `==`→`!=`、常量替换 `1`→`0`、语句删除关键打印行），重跑 replay：如果突变后 replay 仍 confirm，说明断言杀不死突变 = weak 断言。先在 1-2 颗原子上手动跑，验证这个方法能发现 P0-C 统计漏掉的 weak 断言，再决定是否自动化（`tools/mutant_kill.py`）。
- **P2-1 ccache 接入 replay**：环境已就绪（Win `C:\tools\ccache\ccache.exe`、WSL `/usr/bin/ccache`）。给 replay 的 WSL 编译调用前缀 ccache，构建目录固定 /tmp，实测二次 replay 耗时（预期 2min→~10s），记录数字。Windows 侧同理。**⚠️ ccache 与 P0-A 重编译的关系（修正：与 P0-A 不矛盾）**：日常 replay 的主编译路径可以走 ccache 加速，但 **P0-A 的重编译不变量检查必须 CCACHE_DISABLE=1**——重编译的目的是检测工件是否被篡改，走 ccache 缓存命中就恒真了。两者是不同路径：主编译（日常，可 ccache）vs 重编译校验（安全，禁 ccache）。不要在重编译路径开 ccache。
- **P2-2 规则渐进发布**：gate 规则加 `status: experimental|stable|deprecated`，新规则默认 experimental=warn。
- **P2-4 DAG 最小实现（续作不重跑夹具的制度化，461 DAG1/DAG2）**：节点 = 文件产出（夹具.cpp/工件.asm.out/证据卡.md），依赖 = 文件 sha（上游 sha 变了才重跑下游），checkpoint = `_checkpoint.json`（节点名、状态、产出文件、sha、时间戳）。这直接把"续作无需重跑夹具"的经验做法变成机器可执行的协议——replay 检查夹具 sha 未变则跳过重编译。Prefect 3 的数据依赖图验证了这个模式。注意与 P0-A 重编译不变量的交互：重编译检查是"验证工件未被篡改"，DAG 跳过是"夹具未变则不重编译"——两者不冲突，DAG 跳过时仍需 sha 校验。
- **P2-3 校准数据采集（放权的前提，468 杠杆 4 + 305 红队效能度量）**：不只是成本 trace。完整定义 = ①每颗原子 token 消耗（机器侧+红队侧估算）②红队裁决留痕（每条阻断/高级/建议的最终处置：采纳/驳回+理由/误判）③L1 自检拦截率（writer_selfcheck 拦了多少、漏了多少被红队抓到）④**假阴性率**（机器放行后，人审或后续对抗发现错了的比例）⑤**红队效能 6 指标（305）**：阻断证实率（>80%目标）、预测命中率（>50%目标，红队盲读后先预测3个问题再验证）、深层阻断率（方向性/总阻断 >30%）、修复循环数（<1.5轮）、误报率（<10%）、知识产出率（红队发现写入can't-miss或gate规则的比例 >20%）。448 的核心结论：放权的前提是"机器判断与人类判断的一致率"可量化——当前这个数据是 0，所以"自动化"是赌博。10 颗原子后输出第一份校准报告，所有后续放权决策（红队频率、规则质量、自治边界）都基于这份数据。**关键认知（466 中期调整 3）**：ccache 省的是编译时间（秒级），红队 token 才是最大成本（分钟级——一次红队子 agent 53 次工具调用）。成本优化的真正杠杆是**减少红队轮次**（错误左移+红队分层触发），而不是加速编译。trace 数据要能回答"红队占单颗总成本的百分比"，这个数字下降才是真优化。
...
- **归纳跳跃标注（372 v5 P0）**：证据卡证明的是"在环境 E 下观测 O"，但 claim 写的是"机制 M 成立"——从 O 到 M 是归纳跳跃，系统当前没有标注这个跳跃的边界。PERF-003（MinGW 与 Linux 排序相反）、LEAK-002（LSan 报告随 volatile 翻转）、CONC-001（"屏障拦不住消除"被三平台推翻）都是有限观测被写成普遍结论。新增字段方向：`applicability`（tested/not_tested 环境边界）、`evidence_strength`（single-platform < multi-platform < cross-arch < standard-grounded）、`inductive_leap`（什么观测会推翻 claim）。这把 falsification 从"卡内对照实验"升级为"claim 的适用边界管理"。
- **博弈防御（372 v5 最高优先级）**：放权的前提不是"Agent 是善意的"，而是"让骗过门禁比真做对成本更高"。5 种已识别的投机路径：①self_check 跑旧版本再改卡提交 ②claim 写模糊不可证伪 ③断言锚恒真符号 ④falsification 写稻草人对照 ⑤选最简单原子避开难域。机制设计：自检证据带卡+夹具 hash 可独立复算、claim 可证伪性本身受检、断言阴阳配对（反例夹具下必须失败）、选题受覆盖率配额约束。**今晚 P0-A（重编译不变量）和 P0-C（判别力证明）就是博弈防御的第一批落地——不能信任自报，必须独立复算。**
...
5. **门禁全绿才提交**：gate block=0、poison 全拦截+阴性放行、pytest 全过、replay 不退化。**Windows replay 与 WSL 预检严禁并行**（同写 .asm 出假红）。**豁免**：P0-A 的重编译实验在 `tempfile.mkdtemp()` 中做，不写正式 `Examples/atoms/`，因此 Windows 侧重编译实验与 WSL 侧重编译实验**可以并行**——但正式 replay（写 .asm/.out 的）仍严禁并行。
...
9. **安全停止**：任何 P0 任务若发现"修复会破坏 >5 张存量卡且无法快速修正"，**停止该任务**，把现象、受影响卡、可选方案写进 worklog，继续下一个任务，不要硬改。**如果已经改坏了工具导致 gate 加载报错或全量 replay 飘红**，立即 `git checkout -- <文件>` 恢复单个文件到 HEAD 状态，不要在坏状态上继续调试。如果已经 commit 了坏改动，`git revert <commit>` 生成反向 commit（不要 `reset --hard`，会丢失后续 commit）。恢复后在 worklog 记录"该 P0 因 X 原因回滚到 commit Y"。
...
12. **规则渐进发布 + 泛化测试（466 矛盾 2 + 468 攻击 5）**：新规则不要一上来就 block。流程：先 `experimental`（只记录不输出）→ 观察 1 批次（看命中哪些卡、是真问题还是误报）→ 确认精确率后升 warn → 再观察 → 升 block。P0-B（cat 证据）已按此做（先 warn），P0-E/F 等新规则也应遵循。**跳过观察期直接 block 是 warn 信噪比恶化的根因**——466 实测 32 条 warn 中 17 条是误报，就是因为规则没经过观察期。**泛化测试**：一条新规则必须能拦住 ≥2 个不同形态的攻击用例（不是同一个攻击的轻微变种）才能升 stable；只拦一个特例的规则永远停在 experimental，3 批后无新命中则 deprecated 并删除。规则数量要做减法——删掉 1 条低质量规则比加 1 条新规则更有价值。
13. **发现本文错误时的处理（元规则）**：本文经过多轮打磨，但仍可能有与磁盘实际代码不符的描述（如规则编号、函数名、行为描述、探针编号对应关系）。**当你发现本文描述与实际代码/工具行为不符时，以磁盘实际为准**，不要硬按本文执行导致失败。在 worklog 中记录"本文第 X 节描述有误，实际是 Y，已按实际执行"，继续工作。不要因为本文有错误就停止整个任务——本文是指南不是法律，实际代码才是真相。如果错误影响了 P0 的设计（如本文说的规则名不存在），在 worklog 中提出替代方案并继续。
...
- **worklog 零上下文接手标准**：你写的 `_worklog_470.md` 必须让**一个完全没参与今晚工作的新模型**能直接接手。检验方法：worklog 中每个未完成项必须包含①当前代码状态（改了哪些文件、最后一个 commit hash）②未完成的具体原因（不是"没时间了"而是"卡在 X 函数的 Y 行为"）③下一步精确命令（不是"继续做 P0-D"而是"先跑 `python tools/compat_test.py` 看差异分类，然后根据结果决定硬切还是外层包"）。如果一个未完成项缺少这三条，补全后再收工。
10. 未完成项的下一步精确命令。
